...
class InferenceContentSecurityConfig:
    ...
    allowlists; data URIs remain allowed within the configured size budget.
        ...
        Attributes:
            allowed_hosts (list[str] | Unset): Explicit hostname/IP allowlist for inference link downloads. Omission and an
                explicit empty list both deny all hosts; block_private_ips applies in addition to this allowlist. Example:
                ['example.com', 'cdn.example.com', '192.0.2.1'].
            block_private_ips (bool | Unset): Block non-global IP literals, including loopback, private, link-local,
                carrier-grade NAT, reserved, and multicast ranges. DNS results are not currently pinned or revalidated; use an
                allowlist and network egress controls for untrusted deployments. Default: True.
            max_download_size_bytes (int | Unset): Maximum size of downloaded content in bytes Default: 104857600. Example:
                104857600.
            download_timeout_seconds (int | Unset): Reserved for future download timeout enforcement; currently parsed but
                not applied by the scraper.
    ...
    download_timeout_seconds: int | Unset = UNSET
