...
// This interface mirrors the one in go/pkg/antfly/lib/generation for use by pipelines.
...
func LoadTextGenerationPipeline(
	...
	if genaiConfig := readGenAIConfig(modelPath); genaiConfig != nil && genaiConfig.ToolCallFormat != "" {
		...
		// since we can't import go/pkg/antfly/lib/generation here due to circular dependency
	}
