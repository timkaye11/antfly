...
// ValidateCreate validates ExternalInferencePool when creating a new pool.
func (p *ExternalInferencePool) ValidateCreate() error {
	return p.ValidateExternalInferencePool()
}

// ValidateUpdate validates ExternalInferencePool when updating an existing pool.
func (p *ExternalInferencePool) ValidateUpdate(old runtime.Object) error {
	return p.ValidateExternalInferencePool()
}

// ValidateExternalInferencePool performs all validation checks.
func (p *ExternalInferencePool) ValidateExternalInferencePool() error {
	...
	if len(allErrors) > 0 {
		return fmt.Errorf("ExternalInferencePool validation failed:\n  - %s", strings.Join(allErrors, "\n  - "))
	}
