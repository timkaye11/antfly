...
// InferenceProxySpec defines the desired state of InferenceProxy
...
// OperationType represents a Inference API operation
...
// ValidOperationTypes contains all routable Inference API operation values.
...
type RouteDestination struct {
	// Pool is the InferencePool to route to
...
// InferenceProxyStatus defines the observed state of InferenceProxy
type InferenceProxyStatus struct {
...
// InferenceProxy is the Schema for the inferenceproxies API
type InferenceProxy struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`

	Spec InferenceProxySpec `json:"spec,omitempty"`
	// +optional
	Status InferenceProxyStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// InferenceProxyList contains a list of InferenceProxy
type InferenceProxyList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []InferenceProxy `json:"items"`
}

func init() {
	SchemeBuilder.Register(&InferenceProxy{}, &InferenceProxyList{})
}
