...
const REPLAY_STAGES = [
  ...
  { id: "kv", label: "KV", color: "var(--kfam-kv)" },
...
export function ColdOpenFigure({ tokS }: { tokS: string }) {
  ...
  const tokenCount = Math.min(REPLAY_TOKENS.length, Math.floor(tick / (REPLAY_STAGES.length + 2)) + 1);

  return (
    <div className="flex h-full flex-col justify-center gap-6">
      <div className="flex items-center justify-between">
        <span className="font-mono text-[11px] uppercase tracking-wider text-muted-foreground">
          one decode step · replayed
        </span>
        <Button variant="ghost" size="icon" className="size-6" onClick={() => setPlaying((p) => !p)}>
          ...
          <div key={s.id} className="flex flex-1 items-center gap-1.5">
            <div
              className={cn(
                ...
                i === stageIdx ? "scale-105 border-transparent text-background" : "text-muted-foreground",
              )}
              style={{ background: i === stageIdx ? s.color : undefined }}
            >
              {s.label}
            </div>
      ...
      <div className="rounded-lg border bg-muted/30 p-4">
        <div className="min-h-10 text-sm">
          {REPLAY_TOKENS.slice(0, tokenCount).map((t, i) => (
            <span key={i} className={cn(i === tokenCount - 1 && "rounded bg-primary/20")}>
              {t}
            </span>
          ))}
...
export function LayerStackFigure({
  ...
  return (
    <Figure
      ...
      caption={
        emphasis === "attention"
          ? "Shuttered rows slide a window; every 6th layer (solid) attends globally — the 5:1 iSWA rhythm."
          : emphasis === "kv"
            ? `Only ${kvOwners} layers own KV (left column). The rest read a donor's blocks — no K/V projection, no KV writes.`
            : emphasis === "headdim"
              ? "Global layers run head_dim 512; sliding layers run 256 — double the rank exactly where long-range attention lives."
              : undefined
      }
    >
      {layers.map((l) => {
        ...
        const isEmph =
          emphasis === "attention" ? l.global : emphasis === "kv" ? !l.shared : emphasis === "headdim" ? l.global : false;
        const color = l.global ? "var(--kfam-attention)" : "var(--dtype-f16)";
        return (
          <g key={l.idx} opacity={emphasis === "none" || isEmph || emphasis === "attention" ? 1 : l.shared && emphasis === "kv" ? 0.45 : 1}>
            {/* KV ownership column */}
            <rect
              x={4}
              y={y + 1}
            ...
            {l.global && (
              <text x={344} y={y + rowH / 2} fontSize={6} textAnchor="end" dominantBaseline="central" className="fill-background font-mono">
                {emphasis === "headdim" ? "hd 512" : "global"}
              </text>
            )}
            {l.shared && emphasis === "kv" && (
              <text x={352} y={y + rowH / 2} fontSize={6} dominantBaseline="central" textAnchor="end" className="fill-muted-foreground font-mono">
                shared
              </text>
            )}
          </g>
...
export function GraphToFrameFigure({ step }: { step: 0 | 1 | 2 }) {
  ...
  return (
    <Figure
      viewBox="0 0 480 240"
      title={step === 0 ? "the op graph" : step === 1 ? "…grouped into encoder scopes" : "…barriers only at real hazards"}
    >
      {ops.map((op, i) => {
        const x = 12 + i * 66;
        ...
        return (
          <g key={op} className="transition-all duration-500">
            <rect
              ...
              stroke={step >= 1 ? ["var(--kfam-attention)", "var(--kfam-mmsg)", "var(--kfam-fusion)"][inScope] : "var(--kfam-matvec)"}
              strokeWidth={1.25}
            />
            <text x={x + 29} y={y + 15} textAnchor="middle" dominantBaseline="central" fontSize={8.5} className="fill-foreground font-mono">
              {op}
            </text>
          ...
          return (
            <g key={s.label}>
              <path
                ...
                strokeWidth={1}
              />
              <text x={(first + last) / 2} y={86} textAnchor="middle" fontSize={8} className="fill-muted-foreground font-mono">
                {s.label}
              </text>
            </g>
          );
      ...
      {step === 1 && (
        <>
          {[210, 342, 408].map((x) => (
            <line key={x} x1={x} y1={100} x2={x} y2={150} stroke="var(--destructive)" strokeWidth={2} />
          ))}
          <text x={240} y={190} textAnchor="middle" fontSize={9} className="fill-destructive font-mono">
            Q8_0 anchor: 41 encoders · 422 barriers
          </text>
        </>
      )}
      {step === 2 && (
        <text x={240} y={190} textAnchor="middle" fontSize={9} className="fill-foreground font-mono">
          live Q4_0: <tspan className="fill-primary">1 encoder · 143 planned scopes · 0 barriers</tspan>
        </text>
      )}
    </Figure>
  );
