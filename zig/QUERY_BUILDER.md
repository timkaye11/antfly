...
- `constraints.graph_queries` and `constraints.graphs` parse into `query_request.graph_queries`. The deprecated `constraints.expand_strategy` applies only to legacy `graph_searches`; when supplied with canonical graph queries it is ignored with a warning because typed `graph_results` must be consumed explicitly. Graph mode can also infer a graph query from `constraints.graph_index` or a single table graph index, seed it from the assembled lexical, semantic, or fused result set, extract simple `from`/`to` or `between` node keys plus common edge-type hints from the intent, build explicit two-hop or three-hop pattern queries, and use a configured generator for richer graph plans with deterministic validation and fallback.
