...
- The default artifact root mirrors the model namespace: `~/.antfly/inference/artifacts/<owner>/<model>/xla/...`.
- Whole-model PJRT artifact directories refresh a package manifest (`*.antfly-package.json`) that indexes the prefill entry plus compatible decode buckets; whole-model attach is package-first for both prefill and decode bucket discovery, with raw sidecar scanning as fallback compatibility.
- `compiled-target=whole-model` is treated strictly for PJRT: it only attaches when the traced plan is one PJRT-owned compute partition. Multiple PJRT islands are still partitioned execution, not whole-model ownership.
- PJRT manifests carry a semantic binding vocabulary for `input_ids` and past/present KV input/output bindings: `input_ids` maps to the embedding-id execution path, semantic `present.*` output bindings can populate the retained-buffer cache, and semantic `past_*` inputs can bind retained PJRT buffers back into matching executable input slots.
- PJRT artifact loading reports its load mode explicitly: `pjrt_hlo` artifacts compile HLO through the configured plugin at load time, while `pjrt_executable` artifacts deserialize a plugin-native executable without HLO compile-on-load, as a distinct manifest kind rather than a variant of `pjrt_hlo`.
- `PjrtModelRuntime` owns a retained-buffer cache and clears it on runtime reset/deinit; a prefill artifact plus decode artifact share that cache, and packages with semantic prefill `present.*` outputs plus decode `past_*` inputs advertise backend-owned decode state.
- PJRT HLO lowering covers static 2D `fused_rope` (constant cos/sin masks plus a per-head rotation permutation matrix for exact traced sequence shapes) and static batch-1 full-recompute `fused_gqa_causal_attention` (grouped-query KV head repetition, causal masking, softmax).
