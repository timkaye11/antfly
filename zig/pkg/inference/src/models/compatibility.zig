...
fn assessGenerator(architecture: []const u8, expert_count: u32) Assessment {
    if (std.mem.startsWith(u8, architecture, "gemma4") and expert_count > 0) {
        return makeCompatible(architecture, "Gemma 4 mixture-of-experts compact streaming runtime is enabled");
    }
...
test "gemma 4 dense and mixture-of-experts architectures are enabled while unified layout is blocked" {
    var man = manifest_mod.ModelManifest{ .allocator = std.testing.allocator };
    man.model_type = .generator;
    ...
    try std.testing.expectEqual(Level.compatible, assessWithFacts(&man, "gemma4", 128).level);
}
