...
/// Position-id convention used by the embedding table. RoBERTa/XLM-R models
/// reserve the padding index and number non-padding positions from
/// `pad_token_id + 1`; BERT uses a simple zero-based position range.
...
pub fn parseGgufMetadata(view: gguf_metadata.View) ?Config {
    ...
    if (view.getString("bert.family")) |family| {
        ...
        // llama.cpp serializes XLM-R/BGE-M3 with the generic `bert`
        // architecture and no `bert.family`. Its SentencePiece tokenizer
        // contract is nevertheless distinguishable from WordPiece BERT, and
        // position ids must start after the padding index for parity.
        config.model_type = .roberta;
        config.position_id_mode = .roberta_padding;
    }
...
/// llama.cpp's XLM-R GGUF export uses `general.architecture=bert`, but keeps
/// its SentencePiece tokenizer metadata. This is deliberately conservative:
/// ordinary WordPiece BERT models continue to use absolute position ids.
...
fn metaI64(view: gguf_metadata.View, key: []const u8) ?i64 {
    return std.math.cast(i64, view.getU64(key) orelse return null);
}
...
test "XLM-RoBERTa config keeps padding-aware positions and layer norm epsilon" {
    const cfg = try parseConfig(std.testing.allocator,
        \\{"model_type":"xlm-roberta","pad_token_id":1,"layer_norm_eps":0.00001}
    );
    try std.testing.expectEqual(ModelType.roberta, cfg.model_type);
    try std.testing.expectEqual(PositionIdMode.roberta_padding, cfg.position_id_mode);
    try std.testing.expectEqual(@as(i64, 1), cfg.pad_token_id);
    try std.testing.expectApproxEqAbs(@as(f32, 1e-5), cfg.layer_norm_eps, 1e-9);
}
...
test "BGE-M3 style GGUF metadata selects XLM-R padding positions" {
    const allocator = std.testing.allocator;
    const format = @import("../gguf/format.zig");
    const writer = @import("../gguf/writer.zig");
    ...
    const metadata = [_]format.MetadataEntry{
        ...
        .{ .key = "tokenizer.ggml.padding_token_id", .value = .{ .u32 = 1 } },
