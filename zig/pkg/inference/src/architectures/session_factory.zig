...
pub const CudaCapabilityProfile = if (build_options.enable_cuda) cuda_compute_mod.CapabilityProfile else enum {
    clipclap,
    bert_encoder,
    deberta_reranker,
    gliner2,
...
/// Create a Gemma 4 CUDA session whose artifact is qualified for training.
/// The stronger profile is checked before resident model weights are uploaded.
...
/// CUDA Gemma 4 preference training currently builds only the text graph.
/// Unified checkpoints also contain vision/audio towers that cannot be
/// reached by that graph and must not consume scarce device residency. A
/// text-only checkpoint has no wrapper prefix, so retain its complete weight
/// set for compatibility with legacy exports.
fn gemma4CudaTrainingUsesWeight(arch_config: ArchConfig, name: []const u8) bool {
    ...
    // Shared-tail layers consume K/V from their last matching donor layer.
    // Their checkpoint-local K/V projections are therefore unreachable, and
    // Gemma adapter target discovery already applies this same graph rule.
...
test "gemma4 CUDA text training excludes unreachable unified modality weights" {
    ...
    const shared_tail = ArchConfig{ .gpt = .{
        .family = .gemma,
        .weight_prefix = "model.language_model",
        .num_hidden_layers = 42,
        .num_kv_shared_layers = 18,
    } };
    try std.testing.expect(gemma4CudaTrainingUsesWeight(shared_tail, "model.language_model.layers.23.self_attn.k_proj.weight"));
    try std.testing.expect(!gemma4CudaTrainingUsesWeight(shared_tail, "model.language_model.layers.24.self_attn.k_proj.weight"));
    try std.testing.expect(!gemma4CudaTrainingUsesWeight(shared_tail, "model.language_model.layers.41.self_attn.v_proj.weight"));
    try std.testing.expect(gemma4CudaTrainingUsesWeight(shared_tail, "model.language_model.layers.41.self_attn.q_proj.weight"));
}
...
fn createCudaSessionWithRequiredProfile(
    ...
    if (required_profile_override == .gemma4_training and architecture_profile != .gemma4) {
    ...
    var skipped_non_text_count: usize = 0;
    while (it.next()) |entry| {
    ...
    if (required_profile_override == .gemma4_training and skipped_non_text_count > 0) {
        std.log.info(
            "cuda: gemma4 text training admitted {d} weights and skipped {d} unreachable multimodal weights",
            .{ resident_count, skipped_non_text_count },
        );
    }
...
fn overlayGptStructuralConfig(target: *gpt_mod.Config, source: gpt_mod.Config) void {
    ...
    // A colocated config.json may be intentionally minimal (the Google Gemma 4
    // QAT bundles contain only model_type).  In that case the sidecar-derived
    // config has no EOS even though the GGUF tokenizer metadata does.  Preserve
    // any explicit sidecar EOS, but merge every GGUF stop token so generation
    // cannot continue through <eos> as ordinary output.
    mergeGptEosToken(target, source.eos_token_id);
    for (source.extra_eos_token_ids[0..source.extra_eos_token_ids_len]) |token_id| {
        mergeGptEosToken(target, token_id);
    }
}

fn mergeGptEosToken(config: *gpt_mod.Config, token_id: i32) void {
    ...
    if (config.extra_eos_token_ids_len >= gpt_mod.max_extra_eos_token_ids) return;
...
test "overlay gpt structural config merges gguf eos metadata" {
    var target: gpt_mod.Config = .{ .family = .gemma };
    var source: gpt_mod.Config = .{ .family = .gemma, .eos_token_id = 1 };
    source.extra_eos_token_ids[0] = 106;
    source.extra_eos_token_ids_len = 1;

    overlayGptStructuralConfig(&target, source);

    try std.testing.expectEqual(@as(i32, 1), target.eos_token_id);
    try std.testing.expectEqual(@as(u8, 1), target.extra_eos_token_ids_len);
    try std.testing.expectEqual(@as(i32, 106), target.extra_eos_token_ids[0]);

    // An explicit sidecar EOS remains primary, while the artifact EOS is still
    // recognized as an additional stop token.
    var explicit: gpt_mod.Config = .{ .family = .gemma, .eos_token_id = 7 };
    overlayGptStructuralConfig(&explicit, source);
    try std.testing.expectEqual(@as(i32, 7), explicit.eos_token_id);
    try std.testing.expectEqual(@as(u8, 2), explicit.extra_eos_token_ids_len);
    try std.testing.expectEqual(@as(i32, 1), explicit.extra_eos_token_ids[0]);
    try std.testing.expectEqual(@as(i32, 106), explicit.extra_eos_token_ids[1]);
}
