...
test "quant kernel compiler checked-in Metal evidence matches generated source" {
    try std.testing.expectEqual(@as(usize, 10), first_metal_runtime_evidence_count);
    for (first_metal_runtime_evidence) |evidence| {
...
test "quant kernel compiler checked-in Metal blocker evidence matches generated candidates" {
    try std.testing.expectEqual(@as(usize, 15), first_metal_promotion_blocker_evidence_count);
    ...
    try std.testing.expectEqual(@as(usize, 5), metalPromotionBlockerEvidenceCount("unstable_benchmark_timing"));
    try std.testing.expectEqual(@as(usize, 10), metalPromotionBlockerEvidencePathCount());
    try std.testing.expectEqual(@as(usize, 20), first_metal_promotion_blocker_evidence_expected_case_count);
    try std.testing.expectEqual(first_metal_promotion_blocker_evidence_expected_case_count, first_metal_promotion_blocker_evidence_expected_route_ready_count);
    for (first_metal_promotion_blocker_evidence) |evidence| {
...
test "quant kernel compiler registry route summary is golden" {
    ...
    try std.testing.expectEqual(@as(usize, 102), metal.quant_kernel_handwritten_production);
    try std.testing.expectEqual(@as(usize, 10), metal.quant_kernel_generated_production);
    try std.testing.expectEqual(@as(usize, 1120), metal.quant_kernel_unsupported_routes);
    try std.testing.expectEqual(@as(usize, 15), metal.quant_kernel_generated_candidates);
    try std.testing.expectEqual(@as(usize, 15), metal.quant_kernel_fallback_generated_artifact_missing);
...
test "quant kernel compiler conformance manifest serializes the route matrix" {
    ...
    try expectManifestNestedInteger(manifest, "metal_route_summary", "quant_kernel_generated_production", 10);
    try expectManifestNestedInteger(manifest, "metal_route_summary", "quant_kernel_generated_candidates", 15);
    try expectManifestNestedInteger(manifest, "metal_route_summary", "quant_kernel_fallback_generated_artifact_missing", 15);
...
test "quant kernel compiler conformance fingerprints match generated artifacts" {
    ...
    try std.testing.expectEqual(@as(usize, 15), metal_candidates);
}
...
test "quant kernel compiler production Metal source includes only runtime-wired generated kernels" {
    ...
    const q4_disable = std.mem.indexOf(u8, contents, "termite_metal_runtime_promoted_gate(runtime, \"TERMITE_METAL_DISABLE_ANTFLY_Q4_K_SMALL_BATCH\")") orelse return error.MissingMetalQ4Disable;
    const q5_disable = std.mem.indexOf(u8, contents, "termite_metal_runtime_promoted_gate(runtime, \"TERMITE_METAL_DISABLE_ANTFLY_Q5_K_SMALL_BATCH\")") orelse return error.MissingMetalQ5Disable;
