// Copyright 2026 Antfly, Inc.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

// Standalone-only SM89 Flash-prefill v3 experiment for Gemma 4 E2B's locked
// 8Q:1KV contract. One 256-thread CTA owns two query heads and one 16-query
// tile. Warps 0..3 process the first head while warps 4..7 process the second,
// sharing each paged K/V tile. The translation unit is never included in
// production codegen, catalog, artifacts, or runtime dispatch.
//
// Q is staged one head at a time through a single 16xHD half buffer. Each head
// group loads the WMMA A fragments it owns and retains them in registers for
// the entire key loop. The four physical warps per head compute the same eight
// virtual-warp partial score tiles as qualified Flash-v1. Two shared-memory
// phases reduce those partials in the exact virtual-warp 0..7 order, retaining
// Flash-v1's q16/k16 arithmetic and online-softmax recurrence.

#include <cuda_fp16.h>
#include <mma.h>
#include <math.h>
#include <stdint.h>
#include <stddef.h>

#ifndef ANTFLY_FLASH_V3_MIN_BLOCKS_PER_SM
#define ANTFLY_FLASH_V3_MIN_BLOCKS_PER_SM 0
#endif
static_assert(ANTFLY_FLASH_V3_MIN_BLOCKS_PER_SM == 0 ||
        ANTFLY_FLASH_V3_MIN_BLOCKS_PER_SM == 2,
    "minimum blocks per SM must be unconstrained or two");

#if ANTFLY_FLASH_V3_MIN_BLOCKS_PER_SM == 2
#define ANTFLY_FLASH_V3_LAUNCH_BOUNDS __launch_bounds__(256, 2)
#else
#define ANTFLY_FLASH_V3_LAUNCH_BOUNDS
#endif

// Reuse the standalone scalar reference implementations under the symbols the
// existing guard/poison/determinism harness expects. The generated Flash-v1
// entry points are deliberately renamed and never launched.
#define ANTFLY_FLASH_NAMESPACE antfly_flash_prefill_v3_reference_hd256
#define ANTFLY_FLASH_KERNEL antfly_gqa_attention_prefill_flash_f16_sm89_hd256_swa512_f32_v1_unused_v3
#define ANTFLY_FLASH_REFERENCE_KERNEL antfly_gqa_attention_prefill_reference_f16_hd256_swa512_f32_prototype
#define ANTFLY_FLASH_HEAD_DIM 256
#define ANTFLY_FLASH_SLIDING_WINDOW 512
#include "../../../graph/templates/cuda_gqa_flash_prefill_f16_sm89.cuh"
#undef ANTFLY_FLASH_SLIDING_WINDOW
#undef ANTFLY_FLASH_HEAD_DIM
#undef ANTFLY_FLASH_REFERENCE_KERNEL
#undef ANTFLY_FLASH_KERNEL
#undef ANTFLY_FLASH_NAMESPACE

#define ANTFLY_FLASH_NAMESPACE antfly_flash_prefill_v3_reference_hd512
#define ANTFLY_FLASH_KERNEL antfly_gqa_attention_prefill_flash_f16_sm89_hd512_global_f32_v1_unused_v3
#define ANTFLY_FLASH_REFERENCE_KERNEL antfly_gqa_attention_prefill_reference_f16_hd512_global_f32_prototype
#define ANTFLY_FLASH_HEAD_DIM 512
#define ANTFLY_FLASH_SLIDING_WINDOW 0
#include "../../../graph/templates/cuda_gqa_flash_prefill_f16_sm89.cuh"
#undef ANTFLY_FLASH_SLIDING_WINDOW
#undef ANTFLY_FLASH_HEAD_DIM
#undef ANTFLY_FLASH_REFERENCE_KERNEL
#undef ANTFLY_FLASH_KERNEL
#undef ANTFLY_FLASH_NAMESPACE

namespace antfly_flash_prefill_v3_gqa2 {

using namespace nvcuda;

constexpr unsigned kHeads = 8u;
constexpr unsigned kKvHeads = 1u;
constexpr unsigned kHeadsPerCta = 2u;
constexpr unsigned kQueryTile = 16u;
constexpr unsigned kKeyTile = 16u;
constexpr unsigned kPageTokens = 16u;
constexpr unsigned kThreads = 256u;
constexpr unsigned kWarps = 8u;
constexpr unsigned kWarpsPerHead = 4u;
constexpr unsigned kInvalidToken = 0xffffffffu;
constexpr float kNegativeInfinity = -3.402823466e+38F;

enum Status : unsigned {
    kStatusOk = 0u,
    kStatusNullPointer = 1u,
    kStatusMisalignedPointer = 2u,
    kStatusLaunchGeometry = 3u,
    kStatusUnsupportedShape = 4u,
    kStatusUnsupportedPolicy = 5u,
    kStatusInvalidStride = 6u,
    kStatusInvalidPageContract = 7u,
    kStatusMappedPageOutOfBounds = 8u,
};

__device__ __forceinline__ unsigned ceil_div(unsigned value, unsigned divisor) {
    return value / divisor + (value % divisor != 0u ? 1u : 0u);
}

__device__ __forceinline__ bool pointer_aligned(const void* ptr, uintptr_t alignment) {
    return (reinterpret_cast<uintptr_t>(ptr) & (alignment - 1u)) == 0u;
}

__device__ __forceinline__ unsigned physical_token(
    unsigned logical_token,
    const unsigned* block_table,
    unsigned block_count,
    unsigned page_size_tokens,
    unsigned physical_token_capacity
) {
    unsigned long long physical = logical_token;
    if (block_table != nullptr) {
        const unsigned logical_block = logical_token / page_size_tokens;
        if (logical_block >= block_count) return kInvalidToken;
        physical = static_cast<unsigned long long>(block_table[logical_block]) *
                page_size_tokens +
            logical_token % page_size_tokens;
    }
    return physical < physical_token_capacity
        ? static_cast<unsigned>(physical)
        : kInvalidToken;
}

struct VisibleRange {
    unsigned begin;
    unsigned end;
};

template <int SlidingWindow>
__device__ __forceinline__ VisibleRange visible_range(
    unsigned query_pos,
    unsigned kv_seq_len,
    unsigned kv_position_offset
) {
    VisibleRange result{0u, 0u};
    if (query_pos < kv_position_offset) return result;
    const unsigned visible = query_pos - kv_position_offset + 1u;
    result.end = visible < kv_seq_len ? visible : kv_seq_len;
    if constexpr (SlidingWindow != 0) {
        const unsigned window_begin_abs = query_pos + 1u > SlidingWindow
            ? query_pos + 1u - SlidingWindow
            : 0u;
        if (window_begin_abs > kv_position_offset) {
            result.begin = window_begin_abs - kv_position_offset;
            if (result.begin > result.end) result.begin = result.end;
        }
    }
    return result;
}

template <int HeadDim>
__device__ __forceinline__ size_t shared_bytes() {
    // Keep this mirrored by the standalone build script/harness override.
    return
        kQueryTile * HeadDim * sizeof(half) +             // one staged Q head
        kKeyTile * HeadDim * sizeof(half) +               // shared K or V
        kWarps * kQueryTile * kKeyTile * sizeof(float) +  // warp scratch
        kHeadsPerCta * kQueryTile * kKeyTile * sizeof(float) + // scores
        kHeadsPerCta * kQueryTile * kKeyTile * sizeof(half) +  // probabilities
        3u * kQueryTile * sizeof(unsigned) +              // physical/begin/end
        4u * kHeadsPerCta * kQueryTile * sizeof(float) +  // max/denom/alpha/beta
        sizeof(unsigned);                                 // shared error
}

template <int HeadDim, int SlidingWindow>
__device__ void body(
    float* dst,
    const float* q,
    const unsigned char* k,
    const unsigned char* v,
    const unsigned* block_table,
    unsigned batch,
    unsigned q_seq_len,
    unsigned kv_seq_len,
    unsigned num_heads,
    unsigned num_kv_heads,
    unsigned head_dim,
    unsigned query_position_offset,
    unsigned kv_position_offset,
    unsigned sliding_window,
    unsigned total_sequence_len,
    unsigned mask_len,
    unsigned bias_mode,
    unsigned key_row_bytes,
    unsigned base_key_row_bytes,
    unsigned value_row_bytes,
    unsigned block_count,
    unsigned page_size_tokens,
    unsigned format,
    unsigned value_format,
    unsigned physical_token_capacity,
    const unsigned* decode_scalars,
    unsigned char* storage
) {
    static_assert(HeadDim == 256 || HeadDim == 512, "unsupported head dimension");
    constexpr unsigned kVirtualWarpFragments = HeadDim / (kWarps * 16u);
    constexpr unsigned kOutputFragments = HeadDim / (kWarpsPerHead * 16u);
    static_assert(kVirtualWarpFragments == 2u || kVirtualWarpFragments == 4u,
        "unexpected score fragment count");
    static_assert(kOutputFragments == 4u || kOutputFragments == 8u,
        "unexpected output fragment count");

    half* q_shared = reinterpret_cast<half*>(storage);
    half* kv_shared = q_shared + kQueryTile * HeadDim;
    float* warp_scratch = reinterpret_cast<float*>(kv_shared + kKeyTile * HeadDim);
    float* scores = warp_scratch + kWarps * kQueryTile * kKeyTile;
    half* probabilities = reinterpret_cast<half*>(
        scores + kHeadsPerCta * kQueryTile * kKeyTile);
    unsigned* physical = reinterpret_cast<unsigned*>(
        probabilities + kHeadsPerCta * kQueryTile * kKeyTile);
    unsigned* row_begin = physical + kKeyTile;
    unsigned* row_end = row_begin + kQueryTile;
    float* running_max = reinterpret_cast<float*>(row_end + kQueryTile);
    float* running_denom = running_max + kHeadsPerCta * kQueryTile;
    float* row_alpha = running_denom + kHeadsPerCta * kQueryTile;
    float* row_beta = row_alpha + kHeadsPerCta * kQueryTile;
    unsigned* shared_error = reinterpret_cast<unsigned*>(
        row_beta + kHeadsPerCta * kQueryTile);

    const unsigned tid = threadIdx.x;
    const unsigned warp = tid >> 5u;
    const unsigned lane = tid & 31u;
    const unsigned head_in_cta = warp / kWarpsPerHead;
    const unsigned warp_in_head = warp % kWarpsPerHead;

    if (tid == 0u) {
        const bool table_present = block_table != nullptr;
        const bool count_present = block_count != 0u;
        const bool supported_prefix = q_seq_len == 512u
            ? (query_position_offset == 0u || query_position_offset == 512u ||
                  query_position_offset == 1024u || query_position_offset == 1536u)
            : query_position_offset == 2048u;
        unsigned error = kStatusOk;
        if (dst == nullptr || q == nullptr || k == nullptr || v == nullptr) {
            error = kStatusNullPointer;
        } else if (!pointer_aligned(dst, alignof(float)) ||
            !pointer_aligned(q, alignof(float)) ||
            !pointer_aligned(k, alignof(half)) ||
            !pointer_aligned(v, alignof(half)) ||
            (block_table != nullptr &&
                !pointer_aligned(block_table, alignof(unsigned)))) {
            error = kStatusMisalignedPointer;
        } else if (blockDim.x != kThreads || blockDim.y != 1u ||
            blockDim.z != 1u || gridDim.x != kHeads / kHeadsPerCta ||
            gridDim.y != ceil_div(q_seq_len, kQueryTile) || gridDim.z != 1u) {
            error = kStatusLaunchGeometry;
        } else if (batch != 1u || num_heads != kHeads ||
            num_kv_heads != kKvHeads || head_dim != HeadDim ||
            (q_seq_len != 512u && q_seq_len != 3u) || !supported_prefix ||
            kv_seq_len != query_position_offset + q_seq_len ||
            kv_seq_len == 0u || kv_seq_len > 4096u ||
            kv_position_offset != 0u || total_sequence_len != kv_seq_len) {
            error = kStatusUnsupportedShape;
        } else if (sliding_window != SlidingWindow || mask_len != 0u ||
            bias_mode != 0u || format != 2u || value_format != 2u ||
            decode_scalars != nullptr) {
            error = kStatusUnsupportedPolicy;
        } else if (key_row_bytes != HeadDim * sizeof(half) ||
            base_key_row_bytes != key_row_bytes ||
            value_row_bytes != key_row_bytes) {
            error = kStatusInvalidStride;
        } else if (page_size_tokens != kPageTokens ||
            table_present != count_present || physical_token_capacity == 0u ||
            physical_token_capacity % kPageTokens != 0u ||
            (!table_present && physical_token_capacity < kv_seq_len) ||
            (table_present && block_count < ceil_div(kv_seq_len, kPageTokens))) {
            error = kStatusInvalidPageContract;
        }
        *shared_error = error;
    }
    __syncthreads();
    if (*shared_error != kStatusOk) return;

    // Audit every referenced page before writing any output. This retains the
    // fail-closed page contract of the qualified kernel despite the grouped
    // launch geometry.
    if (block_table != nullptr) {
        const unsigned required_blocks = ceil_div(kv_seq_len, kPageTokens);
        for (unsigned logical_block = tid; logical_block < required_blocks;
             logical_block += kThreads) {
            const unsigned long long physical_begin =
                static_cast<unsigned long long>(block_table[logical_block]) * kPageTokens;
            const unsigned long long physical_end = physical_begin + kPageTokens - 1u;
            if (physical_begin > 0xffffffffull || physical_end > 0xffffffffull ||
                physical_end >= physical_token_capacity) {
                atomicCAS(shared_error, kStatusOk, kStatusMappedPageOutOfBounds);
            }
        }
        __syncthreads();
        if (*shared_error != kStatusOk) return;
    }

    const unsigned first_head = blockIdx.x * kHeadsPerCta;
    const unsigned tile_query_begin = blockIdx.y * kQueryTile;
    const unsigned valid_rows = q_seq_len - tile_query_begin < kQueryTile
        ? q_seq_len - tile_query_begin
        : kQueryTile;

    if (tid < kQueryTile) {
        if (tid < valid_rows) {
            const VisibleRange range = visible_range<SlidingWindow>(
                query_position_offset + tile_query_begin + tid,
                kv_seq_len,
                kv_position_offset);
            row_begin[tid] = range.begin;
            row_end[tid] = range.end;
        } else {
            row_begin[tid] = 0u;
            row_end[tid] = 0u;
        }
    }
    for (unsigned index = tid;
         index < kHeadsPerCta * kQueryTile;
         index += kThreads) {
        running_max[index] = kNegativeInfinity;
        running_denom[index] = 0.0f;
        row_alpha[index] = 1.0f;
        row_beta[index] = 0.0f;
    }
    __syncthreads();

    if (tid == 0u) {
        unsigned union_begin = kv_seq_len;
        unsigned union_end = 0u;
        for (unsigned row = 0u; row < valid_rows; ++row) {
            if (row_begin[row] < row_end[row]) {
                if (row_begin[row] < union_begin) union_begin = row_begin[row];
                if (row_end[row] > union_end) union_end = row_end[row];
            }
        }
        if (union_end == 0u) union_begin = 0u;
        physical[0] = (union_begin / kKeyTile) * kKeyTile;
        physical[1] = ceil_div(union_end, kKeyTile) * kKeyTile;
    }
    __syncthreads();
    const unsigned first_key_tile = physical[0];
    const unsigned final_key_tile = physical[1];

    // Each physical warp owns two virtual Flash-v1 score warps. Keeping these
    // A fragments live across the key loop is the central resource experiment.
    wmma::fragment<wmma::matrix_a, 16, 16, 16, half, wmma::row_major>
        query_low[kVirtualWarpFragments];
    wmma::fragment<wmma::matrix_a, 16, 16, 16, half, wmma::row_major>
        query_high[kVirtualWarpFragments];

    // Stage head 0, then head 1, through one shared allocation. Only the four
    // warps that own the staged head consume it before the next overwrite.
#pragma unroll
    for (unsigned staged_head = 0u; staged_head < kHeadsPerCta; ++staged_head) {
        for (unsigned index = tid; index < kQueryTile * HeadDim; index += kThreads) {
            const unsigned row = index / HeadDim;
            const unsigned column = index - row * HeadDim;
            q_shared[index] = row < valid_rows
                ? __float2half_rn(q[(tile_query_begin + row) * kHeads * HeadDim +
                      (first_head + staged_head) * HeadDim + column])
                : __float2half_rn(0.0f);
        }
        __syncthreads();
        if (head_in_cta == staged_head) {
#pragma unroll
            for (unsigned fragment_index = 0u;
                 fragment_index < kVirtualWarpFragments;
                 ++fragment_index) {
                const unsigned low_dimension =
                    warp_in_head * 16u + fragment_index * kWarps * 16u;
                const unsigned high_dimension =
                    (warp_in_head + kWarpsPerHead) * 16u +
                    fragment_index * kWarps * 16u;
                wmma::load_matrix_sync(
                    query_low[fragment_index], q_shared + low_dimension, HeadDim);
                wmma::load_matrix_sync(
                    query_high[fragment_index], q_shared + high_dimension, HeadDim);
            }
        }
        __syncthreads();
    }

    wmma::fragment<wmma::accumulator, 16, 16, 16, float>
        output_fragments[kOutputFragments];
#pragma unroll
    for (unsigned fragment_index = 0u;
         fragment_index < kOutputFragments;
         ++fragment_index) {
        wmma::fill_fragment(output_fragments[fragment_index], 0.0f);
    }

    for (unsigned key_tile_begin = first_key_tile;
         key_tile_begin < final_key_tile;
         key_tile_begin += kKeyTile) {
        if (tid < kKeyTile) {
            const unsigned logical_token = key_tile_begin + tid;
            physical[tid] = logical_token < kv_seq_len
                ? physical_token(logical_token, block_table, block_count,
                    page_size_tokens, physical_token_capacity)
                : kInvalidToken;
            if (logical_token < kv_seq_len && physical[tid] == kInvalidToken) {
                atomicCAS(shared_error, kStatusOk, kStatusMappedPageOutOfBounds);
            }
        }
        __syncthreads();
        if (*shared_error != kStatusOk) return;

        for (unsigned index = tid; index < kKeyTile * HeadDim; index += kThreads) {
            const unsigned key_row = index / HeadDim;
            const unsigned column = index - key_row * HeadDim;
            const unsigned token = physical[key_row];
            kv_shared[index] = token != kInvalidToken
                ? reinterpret_cast<const half*>(
                      k + static_cast<size_t>(token) * key_row_bytes)[column]
                : __float2half_rn(0.0f);
        }
        __syncthreads();

        wmma::fragment<wmma::accumulator, 16, 16, 16, float> score_low;
        wmma::fragment<wmma::accumulator, 16, 16, 16, float> score_high;
        wmma::fill_fragment(score_low, 0.0f);
        wmma::fill_fragment(score_high, 0.0f);
#pragma unroll
        for (unsigned fragment_index = 0u;
             fragment_index < kVirtualWarpFragments;
             ++fragment_index) {
            const unsigned low_dimension =
                warp_in_head * 16u + fragment_index * kWarps * 16u;
            const unsigned high_dimension =
                (warp_in_head + kWarpsPerHead) * 16u +
                fragment_index * kWarps * 16u;
            wmma::fragment<wmma::matrix_b, 16, 16, 16, half, wmma::col_major>
                key_low;
            wmma::fragment<wmma::matrix_b, 16, 16, 16, half, wmma::col_major>
                key_high;
            wmma::load_matrix_sync(key_low, kv_shared + low_dimension, HeadDim);
            wmma::load_matrix_sync(key_high, kv_shared + high_dimension, HeadDim);
            wmma::mma_sync(score_low, query_low[fragment_index], key_low, score_low);
            wmma::mma_sync(score_high, query_high[fragment_index], key_high, score_high);
        }

        // Reduce virtual warps 0..3, then 4..7. The two phases reuse the same
        // eight physical-warp tiles and preserve Flash-v1's exact F32 order.
        wmma::store_matrix_sync(
            warp_scratch + warp * kQueryTile * kKeyTile,
            score_low,
            kKeyTile,
            wmma::mem_row_major);
        __syncthreads();
        for (unsigned index = tid;
             index < kHeadsPerCta * kQueryTile * kKeyTile;
             index += kThreads) {
            const unsigned score_head = index / (kQueryTile * kKeyTile);
            const unsigned element = index % (kQueryTile * kKeyTile);
            const unsigned first_warp = score_head * kWarpsPerHead;
            float score = 0.0f;
#pragma unroll
            for (unsigned source = 0u; source < kWarpsPerHead; ++source) {
                score += warp_scratch[
                    (first_warp + source) * kQueryTile * kKeyTile + element];
            }
            scores[index] = score;
        }
        __syncthreads();
        wmma::store_matrix_sync(
            warp_scratch + warp * kQueryTile * kKeyTile,
            score_high,
            kKeyTile,
            wmma::mem_row_major);
        __syncthreads();
        for (unsigned index = tid;
             index < kHeadsPerCta * kQueryTile * kKeyTile;
             index += kThreads) {
            const unsigned score_head = index / (kQueryTile * kKeyTile);
            const unsigned element = index % (kQueryTile * kKeyTile);
            const unsigned first_warp = score_head * kWarpsPerHead;
            float score = scores[index];
#pragma unroll
            for (unsigned source = 0u; source < kWarpsPerHead; ++source) {
                score += warp_scratch[
                    (first_warp + source) * kQueryTile * kKeyTile + element];
            }
            scores[index] = score;
        }
        __syncthreads();

        if (tid < kHeadsPerCta * kQueryTile) {
            const unsigned softmax_head = tid / kQueryTile;
            const unsigned row = tid % kQueryTile;
            const unsigned row_state = softmax_head * kQueryTile + row;
            const float scale = rsqrtf(static_cast<float>(HeadDim));
            float tile_max = kNegativeInfinity;
            bool tile_has_value = false;
#pragma unroll
            for (unsigned column = 0u; column < kKeyTile; ++column) {
                const unsigned logical_token = key_tile_begin + column;
                const bool visible = row < valid_rows && logical_token < kv_seq_len &&
                    logical_token >= row_begin[row] && logical_token < row_end[row] &&
                    physical[column] != kInvalidToken;
                if (visible) {
                    tile_max = fmaxf(tile_max,
                        scores[row_state * kKeyTile + column] * scale);
                    tile_has_value = true;
                }
            }
            float tile_denom = 0.0f;
#pragma unroll
            for (unsigned column = 0u; column < kKeyTile; ++column) {
                const unsigned logical_token = key_tile_begin + column;
                const bool visible = tile_has_value && row < valid_rows &&
                    logical_token < kv_seq_len && logical_token >= row_begin[row] &&
                    logical_token < row_end[row] && physical[column] != kInvalidToken;
                const float probability = visible
                    ? expf(scores[row_state * kKeyTile + column] * scale - tile_max)
                    : 0.0f;
                probabilities[row_state * kKeyTile + column] =
                    __float2half_rn(probability);
                tile_denom += probability;
            }
            if (tile_has_value) {
                const float old_max = running_max[row_state];
                const float old_denom = running_denom[row_state];
                const float next_max = fmaxf(old_max, tile_max);
                const float alpha = old_denom > 0.0f
                    ? expf(old_max - next_max)
                    : 0.0f;
                const float beta = expf(tile_max - next_max);
                running_max[row_state] = next_max;
                running_denom[row_state] = old_denom * alpha + tile_denom * beta;
                row_alpha[row_state] = alpha;
                row_beta[row_state] = beta;
            } else {
                row_alpha[row_state] = 1.0f;
                row_beta[row_state] = 0.0f;
            }
        }
        __syncthreads();

        for (unsigned index = tid; index < kKeyTile * HeadDim; index += kThreads) {
            const unsigned key_row = index / HeadDim;
            const unsigned column = index - key_row * HeadDim;
            const unsigned token = physical[key_row];
            kv_shared[index] = token != kInvalidToken
                ? reinterpret_cast<const half*>(
                      v + static_cast<size_t>(token) * value_row_bytes)[column]
                : __float2half_rn(0.0f);
        }
        __syncthreads();

        wmma::fragment<wmma::matrix_a, 16, 16, 16, half, wmma::row_major>
            probability_fragment;
        wmma::load_matrix_sync(
            probability_fragment,
            probabilities + head_in_cta * kQueryTile * kKeyTile,
            kKeyTile);
        float* const scratch = warp_scratch + warp * kQueryTile * kKeyTile;
        const unsigned row_state_base = head_in_cta * kQueryTile;
#pragma unroll
        for (unsigned fragment_index = 0u;
             fragment_index < kOutputFragments;
             ++fragment_index) {
            const unsigned output_tile =
                warp_in_head + fragment_index * kWarpsPerHead;
            wmma::fragment<wmma::matrix_b, 16, 16, 16, half, wmma::row_major>
                value_fragment;
            wmma::fragment<wmma::accumulator, 16, 16, 16, float>
                tile_output_fragment;
            wmma::load_matrix_sync(
                value_fragment, kv_shared + output_tile * 16u, HeadDim);
            wmma::fill_fragment(tile_output_fragment, 0.0f);
            wmma::mma_sync(
                tile_output_fragment,
                probability_fragment,
                value_fragment,
                tile_output_fragment);

            wmma::store_matrix_sync(
                scratch,
                output_fragments[fragment_index],
                16,
                wmma::mem_row_major);
            __syncwarp();
            for (unsigned index = lane; index < kQueryTile * 16u; index += 32u) {
                scratch[index] *= row_alpha[row_state_base + index / 16u];
            }
            __syncwarp();
            wmma::load_matrix_sync(
                output_fragments[fragment_index],
                scratch,
                16,
                wmma::mem_row_major);

            wmma::store_matrix_sync(
                scratch,
                tile_output_fragment,
                16,
                wmma::mem_row_major);
            __syncwarp();
            for (unsigned index = lane; index < kQueryTile * 16u; index += 32u) {
                scratch[index] *= row_beta[row_state_base + index / 16u];
            }
            __syncwarp();
            wmma::load_matrix_sync(
                tile_output_fragment,
                scratch,
                16,
                wmma::mem_row_major);
#pragma unroll
            for (unsigned element = 0u;
                 element < output_fragments[fragment_index].num_elements;
                 ++element) {
                output_fragments[fragment_index].x[element] +=
                    tile_output_fragment.x[element];
            }
        }
        __syncthreads();
    }

#pragma unroll
    for (unsigned fragment_index = 0u;
         fragment_index < kOutputFragments;
         ++fragment_index) {
        const unsigned output_tile =
            warp_in_head + fragment_index * kWarpsPerHead;
        float* const scratch = warp_scratch + warp * kQueryTile * kKeyTile;
        wmma::store_matrix_sync(
            scratch,
            output_fragments[fragment_index],
            16,
            wmma::mem_row_major);
        __syncwarp();
        for (unsigned index = lane; index < kQueryTile * 16u; index += 32u) {
            const unsigned row = index / 16u;
            const unsigned column = index % 16u;
            if (row < valid_rows) {
                const float denominator =
                    running_denom[head_in_cta * kQueryTile + row];
                const size_t output_index =
                    static_cast<size_t>(tile_query_begin + row) * kHeads * HeadDim +
                    (first_head + head_in_cta) * HeadDim +
                    output_tile * 16u + column;
                dst[output_index] = denominator > 0.0f
                    ? scratch[index] / denominator
                    : 0.0f;
            }
        }
        __syncwarp();
    }
}

} // namespace antfly_flash_prefill_v3_gqa2

#define ANTFLY_V3_ARGUMENTS \
    float* dst, \
    const float* q, \
    const unsigned char* k, \
    const unsigned char* v, \
    const unsigned* block_table, \
    const unsigned char* attn_or_mask, \
    const float* bias, \
    unsigned batch, \
    unsigned q_seq_len, \
    unsigned kv_seq_len, \
    unsigned num_heads, \
    unsigned num_kv_heads, \
    unsigned head_dim, \
    unsigned query_position_offset, \
    unsigned kv_position_offset, \
    unsigned sliding_window, \
    unsigned total_sequence_len, \
    unsigned mask_len, \
    unsigned bias_mode, \
    unsigned key_row_bytes, \
    unsigned base_key_row_bytes, \
    unsigned value_row_bytes, \
    unsigned block_count, \
    unsigned page_size_tokens, \
    unsigned format, \
    unsigned value_format, \
    unsigned physical_token_capacity, \
    const unsigned* decode_scalars

#define ANTFLY_V3_FORWARD \
    dst, q, k, v, block_table, batch, q_seq_len, kv_seq_len, num_heads, \
    num_kv_heads, head_dim, query_position_offset, kv_position_offset, \
    sliding_window, total_sequence_len, mask_len, bias_mode, key_row_bytes, \
    base_key_row_bytes, value_row_bytes, block_count, page_size_tokens, format, \
    value_format, physical_token_capacity, decode_scalars

extern "C" __global__ ANTFLY_FLASH_V3_LAUNCH_BOUNDS void
antfly_gqa_attention_prefill_flash_f16_sm89_hd256_swa512_f32_prototype(
    ANTFLY_V3_ARGUMENTS
) {
    (void)attn_or_mask;
    (void)bias;
    extern __shared__ __align__(16) unsigned char storage[];
    antfly_flash_prefill_v3_gqa2::body<256, 512>(ANTFLY_V3_FORWARD, storage);
}

extern "C" __global__ ANTFLY_FLASH_V3_LAUNCH_BOUNDS void
antfly_gqa_attention_prefill_flash_f16_sm89_hd512_global_f32_prototype(
    ANTFLY_V3_ARGUMENTS
) {
    (void)attn_or_mask;
    (void)bias;
    extern __shared__ __align__(16) unsigned char storage[];
    antfly_flash_prefill_v3_gqa2::body<512, 0>(ANTFLY_V3_FORWARD, storage);
}

#undef ANTFLY_V3_FORWARD
#undef ANTFLY_V3_ARGUMENTS
#undef ANTFLY_FLASH_V3_LAUNCH_BOUNDS
