...
extern "C" __global__ void termite_linear_q6_k_panel16_argmax_rows_stage1_tile16(
    float* partial_values,
    unsigned int* partial_indices,
    const float* input,
    const unsigned char* weight_panel16,
    const int* suppress_token_ids,
    unsigned int rows,
    unsigned int in_dim,
    unsigned int out_dim,
    ...
    const unsigned int cols = 16u;
    const unsigned int panel_bytes = 4416u;
    const unsigned int panel_scales_offset = 64u;
    const unsigned int panel_values_offset = 320u;
    unsigned int col_tiles = (out_dim + cols - 1u) / cols;
    unsigned int global_tile = blockIdx.x;
    unsigned int row = col_tiles == 0u ? 0u : global_tile / col_tiles;
    unsigned int tile = col_tiles == 0u ? 0u : global_tile - row * col_tiles;
    ...
    if (row < rows) {
        unsigned int sub = tid / 16u;
        unsigned int lane_value = tid & 15u;
        for (unsigned int block = 0; block < row_blocks; ++block) {
            float x = input[row * in_dim + block * 256u + tid];
            const unsigned char* panel = weight_panel16 + (tile * row_blocks + block) * panel_bytes;
            const float* panel_d = (const float*)panel;
            const signed char* panel_scales = (const signed char*)(panel + panel_scales_offset);
            const signed char* panel_values = (const signed char*)(panel + panel_values_offset);
            #pragma unroll
            for (unsigned int c = 0; c < 16u; ++c) {
                unsigned int col = col_tile + c;
                if (col < out_dim) {
                    const signed char* q6_values = panel_values + (sub * cols + c) * 16u;
                    float scale = panel_d[c] * (float)panel_scales[sub * cols + c];
                    acc[c] += x * scale * (float)q6_values[lane_value];
                }
            }
        }
    }
...
extern "C" __global__ void termite_linear_q6_k_panel32_argmax_rows_stage1_tile32(
    float* partial_values,
    unsigned int* partial_indices,
    const float* input,
    const unsigned char* weight_panel32,
    const int* suppress_token_ids,
    unsigned int rows,
    unsigned int in_dim,
    unsigned int out_dim,
    ...
    const unsigned int panel_bytes = 8832u;
    const unsigned int panel_scales_offset = 128u;
    const unsigned int panel_values_offset = 640u;
    unsigned int col_tiles = (out_dim + cols - 1u) / cols;
    unsigned int global_tile = blockIdx.x;
    unsigned int row = col_tiles == 0u ? 0u : global_tile / col_tiles;
    unsigned int tile = col_tiles == 0u ? 0u : global_tile - row * col_tiles;
    ...
    unsigned int warp = tid >> 5;
    unsigned int row_blocks = in_dim / 256u;
    __shared__ float warp_partial[32][8];
    float acc[32];
    #pragma unroll
    for (unsigned int c = 0; c < 32u; ++c) acc[c] = 0.0f;
    ...
    if (row < rows) {
        ...
        for (unsigned int block = 0; block < row_blocks; ++block) {
            float x = input[row * in_dim + block * 256u + tid];
            const unsigned char* panel = weight_panel32 + (tile * row_blocks + block) * panel_bytes;
            const float* panel_d = (const float*)panel;
            const signed char* panel_scales = (const signed char*)(panel + panel_scales_offset);
            const signed char* panel_values = (const signed char*)(panel + panel_values_offset);
            #pragma unroll
            for (unsigned int c = 0; c < 32u; ++c) {
                unsigned int col = col_tile + c;
                if (col < out_dim) {
                    const signed char* q6_values = panel_values + (sub * cols + c) * 16u;
                    float scale = panel_d[c] * (float)panel_scales[sub * cols + c];
...
extern "C" __global__ void termite_linear_q6_k_q8_1_argmax_rows_stage1_tile16_e4b(
    float* partial_values,
    unsigned int* partial_indices,
    const unsigned char* input_q8_1,
    const unsigned char* weight,
    ...
    unsigned int lane = tid & 31u;
    unsigned int warp = tid >> 5;
    __shared__ float warp_partial[16][5];
    float acc[16];
    #pragma unroll
    for (unsigned int c = 0; c < 16u; ++c) acc[c] = 0.0f;
...
extern "C" __global__ void termite_linear_q6_k_q8_1_argmax_rows_stage1_tile8_w4(
    ...
    if (row < rows) {
        for (unsigned int task = tid; task < task_count; task += blockDim.x) {
            unsigned int block = task / 16u;
            unsigned int sub = task - block * 16u;
            unsigned int q8_sub_block = sub >> 1u;
            unsigned int q8_lane_base = (sub & 1u) * 16u;
            const unsigned char* q8_bp = input_q8_1 + (row * q8_row_blocks + block * 8u + q8_sub_block) * 36u;
            unsigned short q8_d_h = (unsigned short)q8_bp[0] | ((unsigned short)q8_bp[1] << 8);
            float q8_d = termite_half_to_float(q8_d_h);
            const signed char* q8_values = (const signed char*)(q8_bp + 4u);
            unsigned int value_base = sub * 16u;
            #pragma unroll
            for (unsigned int c = 0; c < 8u; ++c) {
                unsigned int col = col_tile + c;
                if (col < out_dim) {
                    ...
                    for (unsigned int g = 0u; g < 4u; ++g) {
                        int q6_packed = termite_pack_q6k_i8x4(bp, value_base + g * 4u);
                        int q8_packed = termite_load_i8x4_aligned(q8_values + q8_lane_base + g * 4u);
                        sumi = __dp4a(q6_packed, q8_packed, sumi);
                    }
                    acc[c] += (q8_d * termite_q6k_sub_scale_f32(bp, sub)) * (float)sumi;
                }
            }
    ...
    if (tid == 0u) {
        ...
        for (unsigned int c = 0; c < 8u; ++c) {
            ...
            float value = 0.0f;
            #pragma unroll
            for (unsigned int w = 0u; w < 4u; ++w) value += warp_partial[c][w];
            bool suppressed = false;
            for (unsigned int j = 0; j < suppress_count; ++j) {
                int token_id = suppress_token_ids[j];
                if (token_id >= 0 && (unsigned int)token_id == col) {
...
extern "C" __global__ void termite_linear_q6_k_panel8_q8_1_argmax_rows_stage1_tile8(
    float* partial_values,
    unsigned int* partial_indices,
    const unsigned char* input_q8_1,
    const unsigned char* weight_panel8,
    const int* suppress_token_ids,
    unsigned int rows,
    unsigned int in_dim,
    unsigned int out_dim,
    ...
    const unsigned int cols = 8u;
    const unsigned int panel_bytes = 2208u;
    const unsigned int panel_scales_offset = 32u;
    const unsigned int panel_values_offset = 160u;
    unsigned int col_tiles = (out_dim + cols - 1u) / cols;
    unsigned int global_tile = blockIdx.x;
    unsigned int row = col_tiles == 0u ? 0u : global_tile / col_tiles;
    unsigned int tile = col_tiles == 0u ? 0u : global_tile - row * col_tiles;
    ...
    if (row < rows && tid < task_count) {
        ...
        float q8_d = termite_half_to_float(q8_d_h);
        const signed char* q8_values = (const signed char*)(q8_bp + 4u);
        const unsigned char* panel = weight_panel8 + (tile * row_blocks + block) * panel_bytes;
        const float* panel_d = (const float*)panel;
        const signed char* panel_scales = (const signed char*)(panel + panel_scales_offset);
        const signed char* panel_values = (const signed char*)(panel + panel_values_offset);
        #pragma unroll
        for (unsigned int c = 0; c < 8u; ++c) {
            ...
            if (col < out_dim) {
                ...
                #pragma unroll
                for (unsigned int g = 0u; g < 4u; ++g) {
                    int q6_packed = termite_pack_i8x4(q6_values + g * 4u);
                    int q8_packed = termite_load_i8x4_aligned(q8_values + q8_lane_base + g * 4u);
                    sumi = __dp4a(q6_packed, q8_packed, sumi);
                }
                float q6_scale = panel_d[c] * (float)panel_scales[sub * cols + c];
                acc[c] = (q8_d * q6_scale) * (float)sumi;
            }
        }
    }
    #pragma unroll
...
extern "C" __global__ void termite_linear_q6_k_panel32_q8_1_argmax_rows_stage1_tile32(
    float* partial_values,
    unsigned int* partial_indices,
    const unsigned char* input_q8_1,
    const unsigned char* weight_panel32,
...
extern "C" __global__ void termite_linear_q6_k_q8_1_argmax_rows_stage1_tile16(
    float* partial_values,
    unsigned int* partial_indices,
    const unsigned char* input_q8_1,
    const unsigned char* weight,
...
extern "C" __global__ void termite_linear_q6_k_q8_1_argmax_rows_stage1_tile32(
    float* partial_values,
    unsigned int* partial_indices,
    const unsigned char* input_q8_1,
    const unsigned char* weight,
...
extern "C" __global__ void termite_argmax_reduce_rows_pairs_f32_w32(
    unsigned int* dst,
    const float* partial_values,
    const unsigned int* partial_indices,
    unsigned int rows,
    ...
    __shared__ float warp_best_values[32];
    __shared__ unsigned int warp_best_indices[32];
    unsigned int row = blockIdx.x;
    unsigned int tid = threadIdx.x;
    unsigned int lane = tid & 31u;
    unsigned int warp = tid >> 5;
