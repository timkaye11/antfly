...
pub const TrainerConfig = struct {
    ...
    /// Require a fully device-resident CUDA graph step. Runtime-input uploads
    /// are declared, graph execution may read back only the scalar loss, and
    /// gradients plus optimizer state must remain on the GPU.
...
pub const StepProfile = struct {
    ...
    train_step_ns: u64 = 0,
    compile_ns: u64 = 0,
    /// True when this step created the shape-specialized compiled session.
    /// CUDA permits immutable constant uploads only in this cold setup phase.
    compiled_session_built: bool = false,
    /// True when this step first materialized backend-owned immutable state
    /// for an existing compiled session. This is distinct from graph build
    /// accounting and permits exactly one deferred CUDA setup upload.
    ...
    /// One-time immutable graph state uploaded while a new compiled session
    /// is prepared. Strict CUDA permits H2D here only on a cold compile and
    /// still rejects any readback or training-input upload in this phase.
...
/// Cumulative, serialization-safe evidence for every successful graph step
/// executed by one trainer. Preference recipes perform scoring and backward
/// through several internal calls, so retaining this centrally prevents DPO
/// and GRPO reports from accidentally describing only their final update.
pub const TrainingExecutionEvidence = struct {
    ...
    fn record(self: *TrainingExecutionEvidence, profile: StepProfile, mode: ExecutionMode) void {
        ...
        self.graph_executor_command_dispatches += profile.graph_executor_command_dispatches;
...
pub fn validateStrictCudaStepEvidence(
    ...
    if (profile.graph_executor_runtime_input_transfers != 0 or
        profile.graph_executor_device_resident_transfers != 0)
    {
        return error.StrictCudaRuntimeInputTransfer;
    }
    if (profile.runtime_input_uploads != profile.declared_runtime_input_uploads or
        profile.runtime_input_upload_bytes != profile.declared_runtime_input_upload_bytes or
        profile.runtime_input_h2d_bytes != profile.declared_runtime_input_h2d_bytes or
    ...
    if (profile.graph_executor_runtime_region_fallbacks != 0 or
        profile.graph_executor_host_output_runtime_region != 0)
    {
        return error.StrictCudaRuntimeRegion;
    }
    ...
    // A compiled CUDA step returns one f32 loss scalar. Any larger readback,
    // extra conversion, or allocating download indicates hidden host work.
...
pub const RealAutodiffTrainer = struct {
    ...
    fn graphHasSelectedTiedHeadForward(graph: *const Graph) bool {
        for (0..graph.nodeCount()) |node_index| {
            if (graph.node(@intCast(node_index)).op == .fused_selected_tied_head_logits) return true;
        }
        return false;
    }
    ...
    pub fn ensureCompiledEvalSessionBuilt(self: *RealAutodiffTrainer) !bool {
        ...
        if (self.compiled_eval_session == null) {
            ...
            const match_training_forward = training.graphHasFusedGqaForwardAlternate(&initial_gs.graph) or
                graphHasSelectedTiedHeadForward(&initial_gs.graph);
    ...
    pub fn detachAccumulatedDeviceGradients(self: *RealAutodiffTrainer) !DetachedDeviceGradients {
        ...
        try self.compute_backend.trainingSynchronize();
        var snapshot_frame_active = if (self.compute_backend.kind() == .metal)
    ...
    fn runStep(self: *RealAutodiffTrainer, input: TrainerInput, mode: ExecutionMode) !StepResult {
        ...
        const strict_device = strict_metal or strict_cuda;
        if (strict_metal and self.compute_backend.kind() != .metal) return error.StrictMetalBackendRequired;
        if (strict_cuda and self.compute_backend.kind() != .cuda) return error.StrictCudaBackendRequired;
        if (strict_metal and strict_cuda) return error.ConflictingStrictDeviceBackends;
        ...
        const retain_device_gradients = mode == .train and (use_device_optimizer or strict_device);
        const use_cached_runtime_inputs = (use_device_optimizer or strict_device) and
        ...
        if (use_compiled and strict_cuda) {
            const session = switch (mode) {
                .train => &self.compiled_session.?,
                .eval => &self.compiled_eval_session.?,
            };
            compiled_session_backend_prepared = !session.backendPrepared(self.compute_backend);
            try session.prepareBackend(self.compute_backend);
        }
        ...
        if (strict_cuda) {
            compiledDiag(
                "strict CUDA evidence setup_h2d={} setup_d2h={} graph_uploads={} graph_upload_bytes={} graph_h2d={} graph_d2h={} runtime_uploads={}/{} runtime_h2d={}/{} planned={} partitions={} host_outputs={} d2h_max={} to_f32={} downloads={}",
                .{
                    profile.compiled_session_setup_h2d_bytes,
                    profile.compiled_session_setup_d2h_bytes,
                    profile.graph_execution_input_uploads,
                    profile.graph_execution_input_upload_bytes,
                    profile.graph_execution_h2d_bytes,
                    profile.graph_execution_d2h_bytes,
                    profile.runtime_input_uploads,
                    profile.declared_runtime_input_uploads,
                    profile.runtime_input_h2d_bytes,
                    profile.declared_runtime_input_h2d_bytes,
                    profile.graph_executor_planned_dispatches,
                    profile.graph_executor_partitions,
                    profile.graph_executor_true_host_outputs,
                    profile.cuda_largest_d2h_transfer_bytes,
                    profile.cuda_to_float32_calls,
                    profile.cuda_download_alloc_calls,
                },
            );
...
test "gemma4 strict CUDA evidence permits only scalar loss readback" {
    var profile = StepProfile{
        .graph_executor_partitions = 1,
    ...
    profile.compiled_session_setup_h2d_bytes = 128;
    try testing.expectError(
        error.StrictCudaInvalidSessionSetupTransfer,
        validateStrictCudaStepEvidence(profile, 0, 2, 2, .train),
    );
    profile.compiled_session_backend_prepared = true;
    try validateStrictCudaStepEvidence(profile, 0, 2, 2, .train);
    profile.compiled_session_backend_prepared = false;
    profile.compiled_session_built = true;
    try validateStrictCudaStepEvidence(profile, 0, 2, 2, .train);

    profile.compiled_session_setup_d2h_bytes = 4;
    try testing.expectError(
        error.StrictCudaInvalidSessionSetupTransfer,
        validateStrictCudaStepEvidence(profile, 0, 2, 2, .train),
    );
    profile.compiled_session_setup_d2h_bytes = 0;
    profile.compiled_session_setup_input_uploads = 1;
    try testing.expectError(
        error.StrictCudaInvalidSessionSetupTransfer,
        validateStrictCudaStepEvidence(profile, 0, 2, 2, .train),
    );
    profile.compiled_session_setup_input_uploads = 0;
