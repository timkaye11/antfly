...
const head_files = @import("gliner2_files.zig");
...
pub const adapter_checkpoint_file_name = head_files.adapter_checkpoint_file_name;

pub const adapter_config_file_name = head_files.adapter_config_file_name;

pub const task_head_checkpoint_file_name = head_files.task_head_checkpoint_file_name;
...
pub const MaterializationInventory = struct {
    config: bool,
    encoder_config: bool,
    tokenizer: bool,
...
pub const MaterializationManifest = struct {
...
pub fn inspectLoRABundle(
    ...
    const boundary_head_path = try std.fs.path.join(allocator, &.{ adapter_model_input, head_files.boundary_head_file_name });
    defer allocator.free(boundary_head_path);
    const boundary_task_head_path = try std.fs.path.join(allocator, &.{ adapter_model_input, head_files.boundary_task_head_file_name });
    defer allocator.free(boundary_task_head_path);
    const cleanup_head_path = try std.fs.path.join(allocator, &.{ adapter_model_input, head_files.cleanup_head_file_name });
...
pub fn materializeMergedModel(
    ...
    try copySupportingArtifactIfPresent(allocator, adapter_model_dir, staging_dir, head_files.boundary_head_file_name);
    try copySupportingArtifactIfPresent(allocator, adapter_model_dir, staging_dir, head_files.boundary_task_head_file_name);
    try copySupportingArtifactIfPresent(allocator, adapter_model_dir, staging_dir, head_files.cleanup_head_file_name);
    ...
    const copied_boundary_head = blk: {
        const path = try std.fs.path.join(allocator, &.{ staging_dir, head_files.boundary_head_file_name });
        defer allocator.free(path);
        break :blk isRegularFilePath(path);
    };
    const copied_boundary_task_head = blk: {
        const path = try std.fs.path.join(allocator, &.{ staging_dir, head_files.boundary_task_head_file_name });
        defer allocator.free(path);
        break :blk isRegularFilePath(path);
    };
    const copied_cleanup_head = blk: {
        const path = try std.fs.path.join(allocator, &.{ staging_dir, head_files.cleanup_head_file_name });
        defer allocator.free(path);
        break :blk isRegularFilePath(path);
    };
...
pub fn materializationStagingPath(allocator: std.mem.Allocator, out_dir: []const u8) ![]const u8 {
...
fn syncMaterializationFiles(allocator: std.mem.Allocator, staging_dir: []const u8) !void {
    const file_names = [_][]const u8{
        ...
        head_files.boundary_head_file_name,
        head_files.boundary_task_head_file_name,
        head_files.cleanup_head_file_name,
    };
...
pub fn writeSyncedJsonFile(allocator: std.mem.Allocator, path: []const u8, value: anytype) !void {
