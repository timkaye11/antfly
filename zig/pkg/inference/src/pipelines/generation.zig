...
test "bare channel close projects public content when no final header exists" {
    ...
    try std.testing.expectEqualSlices(
        i64,
        &.{ 20, 21, 22 },
        finalResponseTokenSlice(&.{ 7, 8, 9, 101, 20, 21, 22, 106, 106 }, true, 101, &header, 102, 106),
    );
    // Only the first bare close opens public content; a later boundary ends it.
    try std.testing.expectEqualSlices(
        i64,
        &.{20},
        finalResponseTokenSlice(&.{ 7, 101, 20, 101, 21 }, true, 101, &header, 102, 106),
    );
    // An explicit final header anywhere still wins over the bare-close rule.
    try std.testing.expectEqualSlices(
        i64,
        &.{30},
        finalResponseTokenSlice(&.{ 7, 101, 20, 102, 103, 104, 101, 30, 106 }, true, 101, &header, 102, 106),
    );
    ...
    try std.testing.expectEqualSlices(
        i64,
        &.{40},
        finalResponseTokenSlice(&.{ 102, 103, 105, 101, 7, 8, 101, 40, 106 }, true, 101, &header, 102, 106),
    );
    // A bare close with nothing after it fails closed.
    try std.testing.expectEqualSlices(
        i64,
        &.{},
        finalResponseTokenSlice(&.{ 7, 101 }, true, 101, &header, 102, 106),
    );
    // An unterminated explicit header fails closed.
    try std.testing.expectEqualSlices(
        i64,
        &.{},
        finalResponseTokenSlice(&.{ 102, 103, 7, 8 }, true, 101, &header, 102, 106),
    );
}
