...
def write_outputs(
    *,
    ...
    admitted_occurrence: int = 0,
    model_key: str = "gemma-4-E2B-it",
) -> dict[str, Any]:
    if model_key not in MODEL_KEYS:
        raise MaterializationError(f"unsupported Gemma4 parity model key: {model_key}")
    model_size = "e2b" if model_key == "gemma-4-E2B-it" else "e4b"
    ...
    case_payload: dict[str, Any] = {
        ...
        "name": (
            f"gemma4-{model_size}-ultrafeedback-test-prefs-stratified-seq512"
            if admitted_occurrence == 0
            else f"gemma4-{model_size}-ultrafeedback-test-prefs-stratified-seq512-"
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument(
        ...
        help="locked Gemma4 adapter/model inventory used by the MLX case",
    )
    result.add_argument(
        "--bucket-occurrence",
        type=int,
