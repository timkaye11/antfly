...
def benchmark_workload_sha256(
    ...
    """
    if (
        len(input_rows) != len(label_rows)
        or len(input_rows) != len(attention_mask_rows)
        or not input_rows
    ):
        raise ContractError(
            "benchmark workload requires equal, non-empty input, label, and attention-mask row sets"
        )
        ...
        for field, values in (
            ...
            for value_index, value in enumerate(values):
                if isinstance(value, bool) or not isinstance(value, int):
                    raise ContractError(
                        f"benchmark workload {field}[{row_index}][{value_index}] must be an integer"
                    )
                if field == "input_ids" and value < 0:
                    raise ContractError(
                        f"benchmark workload input_ids[{row_index}][{value_index}] must be non-negative"
                    )
                if field == "labels" and value < 0 and value != -100:
                    raise ContractError(
                        f"benchmark workload labels[{row_index}][{value_index}] permits only -100 as a negative value"
                    )
                if field == "attention_mask" and value not in (0, 1):
                    raise ContractError(
                        f"benchmark workload attention_mask[{row_index}][{value_index}] must be zero or one"
                    )
                try:
...
def canonical_target_inventory_sha256(canonical_modules: Sequence[str]) -> str:
    ...
    if not modules or len(modules) != len(set(modules)):
        raise ContractError(
            "target inventory must be non-empty and contain no duplicates"
        )
...
def canonical_initial_adapter_sha256(tensors: Sequence[Mapping[str, Any]]) -> str:
    ...
    for index, raw in enumerate(tensors):
        ...
        require_exact_keys(
            tensor,
            ("module", "role", "shape", "values"),
            where=f"initial adapter tensor {index}",
        )
        module = string(tensor["module"], f"initial adapter tensor {index}.module")
        if not module.startswith("model."):
            raise ContractError(
                f"initial adapter tensor {index}.module is not canonical"
            )
        role = tensor["role"]
        ...
        if not isinstance(raw_shape, list) or not raw_shape:
            raise ContractError(
                f"initial adapter tensor {index}.shape must be non-empty"
            )
        shape = tuple(
            integer(dim, f"initial adapter tensor {index}.shape[]", 1)
        ...
        if (
            ...
            or len(raw_values) != expected_values
        ):
            raise ContractError(
                f"initial adapter tensor {index}.values does not match shape"
            )
        encoded_values = bytearray()
        for value_index, value in enumerate(raw_values):
            number = finite_signed(
                value, f"initial adapter tensor {index}.values[{value_index}]"
            )
            try:
                encoded = struct.pack("<f", number)
            except (OverflowError, struct.error) as exc:
                raise ContractError(
                    f"initial adapter tensor {index}.values[{value_index}] is outside F32"
                ) from exc
            if not math.isfinite(struct.unpack("<f", encoded)[0]):
                raise ContractError(
                    f"initial adapter tensor {index}.values[{value_index}] is non-finite in F32"
                )
    ...
    if not rows or len(identities) != len(set(identities)):
        raise ContractError(
            "initial adapter must contain one unique LoRA A/B tensor identity"
        )
...
def expected_semantic_contract(
    lock: Mapping[str, Any], case: Mapping[str, Any]
) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "optimizer": optimizer,
        "determinism": dict(
            mapping(frozen["determinism"], "benchmark_contract.determinism")
        ),
    ...
    binding = {
        ...
        "initial_adapter": dict(
            mapping(case["initial_adapter"], "case.initial_adapter")
        ),
        "target_inventory": dict(
            mapping(case["target_inventory"], "case.target_inventory")
        ),
        "contract": payload,
    }
...
def campaign_implementation_identity(sample: Sample) -> dict[str, Any]:
    ...
    inventory = mapping(
        mlx["native_artifact_inventory"], "implementation.mlx.native_artifact_inventory"
    )
    attestation = mapping(
        mlx["build_attestation"], "implementation.mlx.build_attestation"
    )
    ...
    *,
    ...
    for index, raw_tensor in enumerate(raw_tensors):
        ...
        if tensor["dtype"] != expected_dtype:
            raise ContractError(
                f"{where}.tensors[{index}].dtype must be {expected_dtype}"
            )
        raw_shape = tensor["shape"]
        if not isinstance(raw_shape, list):
            raise ContractError(f"{where}.tensors[{index}].shape must be an array")
        shape = [
            integer(dim, f"{where}.tensors[{index}].shape[]", 1) for dim in raw_shape
        ]
...
def validate_precision_evidence_payload(
    ...
    *,
    ...
    if evidence["framework"] != "mlx-lm" or sample.get("framework") != "mlx-lm":
        raise ContractError(
            f"{where}: precision evidence is valid only for MLX-LM samples"
        )
    ...
    expected_binding = {
        ...
        "command_sha256": mapping(sample.get("implementation"), "implementation").get(
            "command_sha256"
        ),
        "semantic_contract_sha256": mapping(
            sample.get("semantic_contract"), "semantic_contract"
        ).get("sha256"),
        "sample_payload_sha256": canonical_precision_sample_binding_sha256(sample),
    }
    if dict(binding) != expected_binding:
        raise ContractError(
            f"{where}: sample binding differs from the benchmark sample"
        )
    ...
    if runner != sample_producer:
        raise ContractError(
            f"{where}: precision producer source differs from the benchmark sample"
        )
    ...
    if (
        native_runtime["precision_policy_sha256"]
        != lock["mlx_reference"]["native_runtime"]["precision_policy_sha256"]
    ):
        raise ContractError(f"{where}: native precision policy differs from the lock")

    precision = mapping(
        lock["benchmark_contract"]["precision"], "benchmark_contract.precision"
    )
    verified = mapping(evidence["verified"], f"{where}.verified")
    if dict(verified) != dict(
        mapping(precision["verified"], "benchmark_contract.precision.verified")
    ):
    ...
    if not_asserted != precision["not_asserted"]:
        raise ContractError(
            f"{where}: not_asserted precision fields differ from the lock"
        )
    if not_asserted != ["activation_dtype", "matmul_accumulator_dtype"]:
        raise ContractError(
            f"{where}: activation and matmul accumulator must remain not_asserted"
        )
    if evidence["comparison_policy"] != precision["comparison_policy"]:
        raise ContractError(
            f"{where}: diagnostic-only comparison policy differs from the lock"
        )
    if (
        evidence["comparison_policy"]
        != "diagnostic-only-until-all-comparison-critical-dtypes-are-runtime-proven"
    ):
    ...
    gradient = mapping(
        observations["gradient_storage"], f"{where}.observations.gradient_storage"
    )
    require_exact_keys(
        gradient,
    ...
    if (
        gradient["evidence_kind"] != "compiled-gradient-tree-inventory"
        or gradient["dtype"] != "float32"
    ):
        raise ContractError(f"{where}: gradient storage evidence kind/dtype is invalid")
    if integer(
        gradient["tensor_count"],
        f"{where}.observations.gradient_storage.tensor_count",
        1,
    ) != len(lora_tensors):
        raise ContractError(
            f"{where}: gradient tensor count differs from LoRA trainables"
        )
    raw_stages = gradient["stages"]
    if not isinstance(raw_stages, list) or len(raw_stages) != 3:
        raise ContractError(
            f"{where}: gradient evidence must contain raw, accumulated, and clipped stages"
        )
    for index, expected_stage in enumerate(("raw", "accumulated", "clipped")):
        stage = mapping(
            raw_stages[index], f"{where}.observations.gradient_storage.stages[{index}]"
        )
        require_exact_keys(
            stage,
            ("stage", "inventory_sha256", "tensors"),
            where=f"{where}.gradient stage",
        )
        if stage["stage"] != expected_stage:
            raise ContractError(
                f"{where}: gradient evidence stages are incomplete or out of order"
            )
        ...
        if stage_tensors != lora_tensors:
            raise ContractError(
                f"{where}: {expected_stage} gradient inventory differs from LoRA trainables"
            )
        if stage["inventory_sha256"] != canonical_tensor_inventory_sha256(
            stage_tensors
        ):
            raise ContractError(
                f"{where}: {expected_stage} gradient inventory digest mismatch"
            )

    optimizer = mapping(
        observations["optimizer_moment_storage"],
        f"{where}.observations.optimizer_moment_storage",
    )
    require_exact_keys(
        optimizer,
        (
    ...
    if (
        optimizer["evidence_kind"] != "materialized-post-cold-adamw-moment-inventory"
        or optimizer["dtype"] != "float32"
    ):
        raise ContractError(f"{where}: optimizer-moment evidence kind/dtype is invalid")
    if integer(
        optimizer["parameter_count"], f"{where}.optimizer.parameter_count", 1
    ) != len(lora_tensors):
        raise ContractError(
            f"{where}: optimizer parameter count differs from LoRA trainables"
        )
    ...
    for index, raw_moment in enumerate(raw_moments):
        ...
        require_exact_keys(
            moment,
            ("name", "parameter_name", "role", "dtype", "shape"),
        ...
        parameter_name = string(
            moment["parameter_name"],
            f"{where}.optimizer.moments[{index}].parameter_name",
        )
        ...
        shape = [
            integer(dim, f"{where}.optimizer.moments[{index}].shape[]", 1)
            for dim in raw_shape
        ]
        moments.append(
            {
    ...
    if [moment["name"] for moment in moments] != sorted(
    ...
    if moments != expected_moments:
        raise ContractError(
            f"{where}: optimizer m/v inventory differs from exact LoRA trainables"
        )
    if integer(
        optimizer["moment_tensor_count"], f"{where}.optimizer.moment_tensor_count", 2
    ) != len(moments):
        raise ContractError(
            f"{where}: optimizer moment count differs from exact inventory"
        )
    ...
    require_exact_keys(
        loss,
        ("evidence_kind", "loss_tensor_dtype", "reduction_input_dtype"),
    ...
    if derived_verified != dict(verified):
        raise ContractError(
            f"{where}: one or more verified fields lack matching runtime evidence"
        )
    return evidence
...
def validate_precision_evidence_reference(
    ...
    *,
    ...
    require_exact_keys(
        reference,
        ("schema_version", "relative_path", "artifact_sha256"),
    ...
    relative_path = string(
        reference["relative_path"], "precision_evidence.relative_path"
    )
    expected_name = (
    ...
    if (
        ...
        raise ContractError(
            "precision_evidence.relative_path must be one sibling file name"
        )
    candidate = (
        evidence_path.expanduser().absolute()
        if evidence_path is not None
        else sample_path.expanduser().absolute().with_name(relative_path)
    )
    if candidate.name != relative_path:
        raise ContractError(
            "precision_evidence.relative_path differs from the supplied artifact"
        )
    if candidate.is_symlink() or not candidate.is_file():
        raise ContractError(
            "precision_evidence artifact must be an existing regular non-symlink file"
        )
    if candidate.parent.resolve(
        strict=True
    ) != sample_path.expanduser().absolute().parent.resolve(strict=True):
        raise ContractError(
            "precision_evidence artifact must be a sibling of the sample"
        )
    ...
    return validate_precision_evidence_payload(
        evidence, sample, lock, where=str(candidate)
    )
...
def validate_zig_execution_evidence(
    ...
    *,
    ...
    if not isinstance(steps, list) or len(steps) != expected_count:
        raise ContractError(
            f"metrics.execution_evidence must contain exactly {expected_count} optimizer steps"
        )
    ...
    for index, raw_step in enumerate(steps):
        ...
        if step["index"] != index:
            raise ContractError(
                "metrics.execution_evidence optimizer-step indexes are not contiguous"
            )
        expected_phase = (
            "cold"
            if index == 0
        ...
        if step["phase"] != expected_phase:
            raise ContractError(
                f"metrics.execution_evidence optimizer step {index} has the wrong phase"
            )

        phase = mapping(step["phase_evidence"], f"execution phase {index}")
        require_exact_keys(
            phase, ZIG_PHASE_EVIDENCE_FIELDS, where=f"execution phase {index}"
        )
        for field in ZIG_PHASE_EVIDENCE_FIELDS:
            integer(phase[field], f"execution phase {index}.{field}")
        if (phase["compile_ns"] > 0) != (index == 0):
            raise ContractError(
                f"execution phase {index} compile evidence violates the cold-only contract"
            )

        command = mapping(
            step["command_plan_evidence"], f"execution command plan {index}"
        )
        require_exact_keys(
            command,
            ZIG_COMMAND_PLAN_EVIDENCE_FIELDS,
        ...
        if command["graph_executor_partitions"] < grad_accum:
            raise ContractError(
                f"execution command plan {index} omitted graph partitions"
            )
        dispatches = command["graph_executor_command_dispatches"]
        if dispatches == 0:
            raise ContractError(
                f"execution command plan {index} omitted graph dispatches"
            )
        if command["graph_executor_planned_dispatches"] > dispatches:
            raise ContractError(
                f"execution command plan {index} planned dispatches exceed graph dispatches"
            )
        if sum(command[field] for field in ZIG_COMMAND_ATTRIBUTION_FIELDS) > dispatches:
            raise ContractError(
                f"execution command plan {index} command attribution exceeds graph dispatches"
            )
        ...
        if (
            ...
            or command["metal_linear_cce_forward_state_misses"] != 0
        ):
            raise ContractError(
                f"execution command plan {index} linear CCE route evidence is inconsistent"
            )
        if (command["metal_linear_cce_peak_scratch_bytes"] != 0) != (cce_forward != 0):
            raise ContractError(
                f"execution command plan {index} linear CCE scratch evidence is inconsistent"
            )
        hits = command["graph_executor_plan_cache_hits"]
        misses = command["graph_executor_plan_cache_misses"]
        if hits + misses != grad_accum:
            raise ContractError(
                f"execution command plan {index} has the wrong cache lookup count"
            )
        expected_misses = 1 if index == 0 else 0
        if misses != expected_misses:
            raise ContractError(
                f"execution command plan {index} has an unexpected cache miss count"
            )
        plan_was_built = misses != 0
        if (phase["graph_executor_plan_build_ns"] != 0) != plan_was_built or (
            phase["graph_executor_buffer_plan_build_ns"] != 0
        ) != plan_was_built:
            raise ContractError(
                f"execution command plan {index} differs from plan-build phase evidence"
            )
...
def validate_sample(
    ...
    *,
    ...
    sha256_identity(
        implementation.get("command_sha256"), "implementation.command_sha256"
    )
    ...
    if payload["framework"] == "antfly-zig-metal":
        ...
        revision = string(
            antfly["source_revision"], "implementation.antfly.source_revision"
        )
        if re.fullmatch(r"[0-9a-f]{40}", revision) is None:
            raise ContractError(
                "implementation.antfly.source_revision must be a full commit"
            )
        ...
        if producer_source["source_revision"] != revision:
            raise ContractError(
                "Antfly wrapper source revision differs from the benchmark executable source"
            )
        sha256_identity(
            antfly["executable_sha256"], "implementation.antfly.executable_sha256"
        )
    else:
        require_exact_keys(
            implementation,
        ...
        inventory = mapping(
            mlx["native_artifact_inventory"],
            "implementation.mlx.native_artifact_inventory",
        )
        require_exact_keys(
            inventory,
        ...
        if inventory["schema_version"] != MLX_NATIVE_ARTIFACT_INVENTORY_SCHEMA_VERSION:
            raise ContractError(
                f"{path}: unsupported MLX native artifact inventory schema"
            )
        loaded_core_path = Path(
            string(
                inventory["loaded_core_path"],
                "implementation.mlx.native_artifact_inventory.loaded_core_path",
            )
        )
        if not loaded_core_path.is_absolute():
            raise ContractError(
                "implementation.mlx native loaded core path must be absolute"
            )
        ...
        if inventory["sha256"] != inventory_sha256:
            raise ContractError(
                f"{path}: MLX native artifact inventory digest mismatch"
            )
        python_extension = next(
            (
                artifact
                for artifact in artifacts
                if artifact.get("role") == "python-extension"
            ),
            None,
        )
        if python_extension is None:
            raise ContractError(
                f"{path}: MLX native artifact inventory omitted the Python extension"
            )
        if loaded_core_path.name != Path(python_extension["relative_path"]).name:
            raise ContractError(
                f"{path}: loaded mlx.core path differs from the bound native extension"
            )
        attestation = mapping(
            mlx["build_attestation"], "implementation.mlx.build_attestation"
        )
        ...
        if (
            ...
            != native_runtime["build_attestation_schema_version"]
        ):
            raise ContractError(
                f"{path}: unsupported MLX native build attestation schema"
            )
        if not Path(
            string(attestation["path"], "implementation.mlx.build_attestation.path")
        ).is_absolute():
            raise ContractError(
                "implementation.mlx build attestation path must be absolute"
            )
        sha256_identity(
            attestation["sha256"], "implementation.mlx.build_attestation.sha256"
        )
        sha256_identity(
            attestation["build_command_sha256"],
            "implementation.mlx.build_attestation.build_command_sha256",
        )
        if (
            attestation["source_revision"] != mlx["source_revision"]
            or attestation["source_clean"] is not True
        ):
            raise ContractError(
                f"{path}: MLX native build source attestation differs from pinned clean source"
            )
        if attestation["native_artifact_inventory_sha256"] != inventory_sha256:
            raise ContractError(
                f"{path}: MLX build attestation does not bind the native artifact inventory"
            )
        if (
            attestation["precision_policy_sha256"]
            != native_runtime["precision_policy_sha256"]
        ):
            raise ContractError(
                f"{path}: MLX build attestation precision policy differs from the lock"
            )

        mlx_lm = mapping(implementation["mlx_lm"], "implementation.mlx_lm")
        require_exact_keys(
            mlx_lm,
            ("version", "source_revision", "source_clean"),
            where="implementation.mlx_lm",
        )
    ...
    if case["grad_accum"] not in gate["gradient_accumulation"]:
        raise ContractError(
            f"{path}: gradient accumulation differs from the release matrix"
        )
    ...
    if (
        initial_adapter["schema_version"]
        != "antfly_gemma4_initial_adapter_semantics/v1"
    ):
        raise ContractError(f"{path}: unsupported initial adapter semantic schema")
    sha256_identity(
        initial_adapter["semantic_sha256"], "case.initial_adapter.semantic_sha256"
    )
    ...
    modules = [
        string(module, "case.target_inventory.canonical_modules[]")
        for module in raw_modules
    ]
    module_count = integer(
        target_inventory["module_count"], "case.target_inventory.module_count", 1
    )
    if module_count != len(modules):
        raise ContractError(
            f"{path}: target inventory count differs from canonical module list"
        )
    if target_inventory["sha256"] != canonical_target_inventory_sha256(modules):
        raise ContractError(
            f"{path}: target inventory digest does not bind canonical module list"
        )
    validate_target_inventory(lock, model_key, case["target_preset"], modules)
    if initial_adapter["tensor_count"] != 2 * module_count:
        raise ContractError(
            f"{path}: initial adapter must contain exactly LoRA A and B for every target"
        )
    ...
    if semantic_contract_bytes != expected_contract_bytes:
        raise ContractError(
            f"{path}: semantic contract differs from the locked case contract"
        )
    if payload["framework"] == "mlx-lm":
        validate_precision_evidence_reference(
            ...
            payload,
            lock,
            evidence_path=precision_evidence_path,
        )
    ...
    require_exact_keys(
        ...
        (
            "fresh_process",
            "cold_optimizer_steps",
            "cold_step_mutates_optimizer_state",
            "first_steady_steps",
            "warmup_steps",
            "measured_steps",
    ...
    finite(
        metrics["cold_compile_and_step_seconds"],
        "metrics.cold_compile_and_step_seconds",
        positive=True,
    )
    finite(
        metrics["first_steady_step_seconds"],
        "metrics.first_steady_step_seconds",
        positive=True,
    )
    ...
    if not isinstance(steps, list) or len(steps) != reference["measured_steps"]:
        raise ContractError(
            f"{path}: expected exactly {reference['measured_steps']} measured steps"
        )
    ...
    integer(
        memory["process_peak_phys_footprint_bytes"],
        "metrics.memory.process_peak_phys_footprint_bytes",
        1,
    )
    if (
        memory["sampler_interval_ms"]
        != lock["benchmark_contract"]["memory"]["sampler_interval_ms"]
    ):
        raise ContractError(
            f"{path}: process-memory sampler interval differs from the lock"
        )
    ...
    if payload["framework"] == "mlx-lm":
        ...
        or allocator_source != "antfly-metal-allocator-unavailable"
    ):
        raise ContractError(
            f"{path}: Antfly allocator peak must be explicitly unavailable"
        )
    ...
    for field in ("swapins_bytes", "swapouts_bytes", "pageins_bytes", "pageouts_bytes"):
        ...
        if delta > maximum:
            raise ContractError(
                f"{path}: measured-phase {field} exceeds the locked zero-I/O gate"
            )
    ...
    if not -100 <= pressure_delta <= 100:
        raise ContractError(
            "metrics.memory.system_deltas.pressure_available_percent_delta must be finite in [-100,100]"
        )
    if pressure_delta < memory_contract["minimum_pressure_available_percent_delta"]:
        raise ContractError(
            f"{path}: measured-phase memory pressure drop exceeds the locked threshold"
        )
    ...
    expected_input_tokens = (
        case["sequence_length"] * case["microbatch"] * case["grad_accum"]
    )
...
def summarize_framework(samples: list[Sample]) -> dict[str, Any]:
    per_process_medians = [
        median(sample.payload["metrics"]["step_seconds"]) for sample in samples
    ]
    all_steps = [
        duration
        for sample in samples
        for duration in sample.payload["metrics"]["step_seconds"]
    ]
    input_tokens = {sample.payload["metrics"]["input_tokens"] for sample in samples}
    supervised_tokens = {
        sample.payload["metrics"]["supervised_tokens"] for sample in samples
    }
    ...
    return {
        ...
        "median_process_peak_phys_footprint_bytes": median(
            sample.payload["metrics"]["memory"]["process_peak_phys_footprint_bytes"]
            for sample in samples
        ),
        ...
        "median_load_seconds": median(
            sample.payload["metrics"]["load_seconds"] for sample in samples
        ),
        "median_cold_compile_and_step_seconds": median(
            sample.payload["metrics"]["cold_compile_and_step_seconds"]
            for sample in samples
        ),
...
def expected_full_matrix(
    lock: Mapping[str, Any], samples: list[Sample]
) -> set[tuple[Any, ...]]:
...
def compare_campaign(
    ...
    *,
    ...
    if require_full_matrix and campaign_manifest_path is None:
        raise ContractError(
            "release comparison requires an orchestrator-published campaign manifest"
        )
    ...
    hardware_values = {
        json.dumps(sample.payload["hardware"], sort_keys=True) for sample in samples
    }
    if len(hardware_values) != 1:
        raise ContractError(
            "all benchmark samples must have byte-equivalent hardware identity"
        )
    run_ids = [sample.payload["run_id"] for sample in samples]
    process_ids = [
        (sample.payload["process"]["pid"], sample.payload["process"]["started_unix_ns"])
        for sample in samples
    ]
    ...
    if len(process_ids) != len(set(process_ids)):
        raise ContractError(
            "process identity was reused across supposedly fresh samples"
        )
    command_digests = [
        sample.payload["implementation"]["command_sha256"] for sample in samples
    ]
    if len(command_digests) != len(set(command_digests)):
        raise ContractError(
            "every fresh-process sample must have a unique command invocation digest"
        )
    ...
    if sorted(sequence_indices) != list(range(len(sequence_indices))):
        raise ContractError(
            "campaign sequence_index values must be contiguous from zero"
        )

    implementation_identities: dict[str, str] = {}
    for framework in FRAMEWORKS:
        framework_samples = [
            sample for sample in samples if sample.payload["framework"] == framework
        ]
        identities = {
    ...
    for sample in samples:
        grouped.setdefault(sample.case_key, {}).setdefault(
            sample.payload["framework"], []
        ).append(sample)
        matrix_case = sample.case_key[:MATRIX_KEY_FIELDS]
        identities_by_matrix_case.setdefault(matrix_case, set()).add(
            sample.case_key[MATRIX_KEY_FIELDS]
        )
    mixed_identities = [
    ...
    if mixed_identities:
        raise ContractError(
            "matrix cells mix semantic case identities: "
            + ", ".join(sorted(mixed_identities))
        )
    if require_full_matrix:
        ...
        if not expected or actual_matrix != expected:
            missing = sorted(
                display_matrix_case(key) for key in expected - actual_matrix
            )
            extra = sorted(display_matrix_case(key) for key in actual_matrix - expected)
            raise ContractError(
                f"campaign does not contain the full release matrix (missing={missing}, extra={extra})"
            )
    ...
    for key in sorted(grouped):
        ...
        semantic_contracts = {
            json.dumps(
                sample.payload["semantic_contract"],
                sort_keys=True,
                separators=(",", ":"),
            )
        ...
        if len(semantic_contracts) != 1:
            raise ContractError(
                f"{display_case(key)}: paired group semantic contracts are not byte-equivalent"
            )
        for framework, framework_samples in by_framework.items():
            repetitions = {
                sample.payload["repetition"]: sample for sample in framework_samples
            }
            if len(repetitions) != len(framework_samples):
                failures.append(
                    f"{display_case(key)}/{framework}: duplicate repetition"
                )
            repetitions_by_framework[framework] = repetitions
            if len(framework_samples) < minimum_processes:
                failures.append(
                    f"{display_case(key)}/{framework}: fewer than {minimum_processes} fresh processes"
                )
        execution_contracts = {
        ...
        if len(execution_contracts) != 1:
            raise ContractError(
                f"{display_case(key)}: execution contract changed across paired repetitions"
            )
        ...
        if repetitions != list(range(len(repetitions))):
            failures.append(
                f"{display_case(key)}: repetitions must be contiguous from zero"
            )
        ...
        for repetition in repetitions:
            ...
            if (
                abs(left.payload["sequence_index"] - right.payload["sequence_index"])
                != 1
            ):
                failures.append(
                    f"{display_case(key)}/repetition={repetition}: paired runs are not adjacent"
                )
            first_frameworks.append(
                "antfly-zig-metal"
                if left.payload["sequence_index"] < right.payload["sequence_index"]
                else "mlx-lm"
            )
            left_counts = (
        ...
        if any(
            first_frameworks[index] == first_frameworks[index - 1]
            for index in range(1, len(first_frameworks))
        ):
            failures.append(
                f"{display_case(key)}: framework execution order did not alternate"
            )

        antfly = summarize_framework(by_framework["antfly-zig-metal"])
        mlx = summarize_framework(by_framework["mlx-lm"])
        throughput_ratio = (
            antfly["supervised_tokens_per_second"] / mlx["supervised_tokens_per_second"]
        )
        memory_ratio = (
        ...
        if throughput_ratio < gate["minimum_cell_throughput_ratio"]:
            failures.append(
                f"{display_case(key)}: throughput ratio {throughput_ratio:.4f} is below the cell floor"
            )
        if memory_ratio > gate["maximum_peak_memory_ratio"]:
            failures.append(
                f"{display_case(key)}: peak-memory ratio {memory_ratio:.4f} exceeds the ceiling"
            )
        cell_rows.append(
            {
                "case": {
                    ...
                    "microbatch": key[8],
                    "prepared": dict(
                        by_framework["antfly-zig-metal"][0].payload["case"]["prepared"]
                    ),
                    "initial_adapter": dict(
                        by_framework["antfly-zig-metal"][0].payload["case"][
                            "initial_adapter"
                        ]
                    ),
                    "target_inventory": dict(
                ...
                "semantic_contract": dict(
                    by_framework["antfly-zig-metal"][0].payload["semantic_contract"]
                ),
                "antfly": antfly,
                "mlx_lm": mlx,
    ...
    geomean = (
        math.exp(
            sum(math.log(value) for value in throughput_ratios) / len(throughput_ratios)
        )
    ...
    if geomean < gate["minimum_geomean_throughput_ratio"]:
        failures.append(
            f"geometric-mean throughput ratio {geomean:.4f} is below the release floor"
        )
    regression_fraction = (
        regressed_cells / len(throughput_ratios) if throughput_ratios else 1.0
    )
        ...
        "campaign_manifest": (
            {
                ...
                "sha256": prefixed_sha256(
                    campaign_manifest_path.expanduser().resolve()
                ),
...
def parser() -> argparse.ArgumentParser:
    ...
    fingerprint = commands.add_parser(
        "fingerprint", help="print a host identity template"
    )
    fingerprint.add_argument(
        "--metal-device",
        required=True,
        help="exact Metal device label reported by the runner",
    )
    ...
    compare.add_argument(
        "--allow-partial",
        action="store_true",
        help="diagnostic only: do not require all 24 release cells",
    )
    return root
    ...
    try:
        ...
        if args.command == "fingerprint":
            if platform.system() != "Darwin" or platform.machine() != "arm64":
                raise ContractError(
                    "same-Mac benchmark fingerprint requires Darwin arm64"
                )
            ...
            if missing:
                raise ContractError(
                    f"could not resolve required host fields: {missing}"
                )
        ...
        if args.command == "validate":
            ...
            payload = {
                ...
                "case": display_case(sample.case_key),
            }
            emit(payload, args.output)
            return 0
