...
RUNNER_RELATIVE_PATH = (
    "zig/pkg/inference/scripts/gemma4/export_gemma4_lora_zig_oracle.py"
)
STRICT_METAL_ENV = {
...
def _source_identity(source_root: Path) -> tuple[Path, str]:
    ...
    resolved_top = (
        Path(top.stdout.strip()).resolve(strict=True)
    ...
    if relative != RUNNER_RELATIVE_PATH:
        raise ContractError(
            "Zig oracle wrapper path differs from the admitted source path"
        )
    head = subprocess.run(
    ...
    if dirty.returncode != 0 or dirty.stdout:
        raise ContractError(
            "Antfly Zig oracle source checkout must be clean, including untracked files"
        )
...
def _executable_version(executable: Path) -> str:
    ...
    if (
        ...
        or len(matches) != 1
        or not matches[0]
        or len(matches[0]) > 256
    ):
        detail = result.stderr.strip() or f"exit status {result.returncode}"
        raise ContractError(
            f"Antfly inference version output was missing or ambiguous: {detail}"
        )
    return matches[0]
...
def _run_child(
    ...
    try:
        ...
        raise ContractError(
            f"Antfly Zig oracle timed out after {timeout} seconds"
        ) from exc
...
def _verify_packaging_dependencies(lock: Mapping[str, Any]) -> dict[str, str]:
    expected_packages = _mapping(
        _mapping(lock["python_oracle"], "python_oracle")["packages"],
        "python_oracle.packages",
    )
    expected = {
        name: str(expected_packages[name]) for name in ZIG_ORACLE_PACKAGING_PACKAGES
    }
    actual: dict[str, str] = {}
    for name, version in expected.items():
        ...
        if actual[name] != version:
            raise ContractError(
                f"Zig oracle packager requires {name}={version}, found {actual[name]}"
            )
    return actual


def _host_identity(
    backend: str, executable_sha256: str, metal_device: str | None
) -> dict[str, Any]:
...
def _validate_run_manifest(root: Path, expected_files: Sequence[str]) -> None:
    ...
    for path in root.iterdir():
        if path.is_symlink() or not path.is_file():
            raise ContractError(
                f"Zig capture contains a non-regular entry: {path.name}"
            )
    ...
    _exact_keys(
        manifest,
        ("schema_version", "status", "artifact_family_version", "artifacts"),
    ...
    if (
        ...
        raise ContractError(
            "Zig capture run manifest is not complete or uses the wrong schema"
        )
    entries: dict[str, Mapping[str, Any]] = {}
    for index, raw in enumerate(
        _list(manifest["artifacts"], "capture run manifest.artifacts")
    ):
        entry = _mapping(raw, f"capture run manifest.artifacts[{index}]")
        _exact_keys(
            entry,
            ("name", "sha256", "size_bytes"),
            f"capture run manifest.artifacts[{index}]",
        )
    ...
    if set(entries) != expected:
        raise ContractError(
            "Zig capture run manifest does not close over the expected file set"
        )
...
def _artifact_file(
    root: Path, descriptor: Mapping[str, Any], expected_name: str, where: str
) -> Path:
    ...
    if (
        _integer(descriptor["size_bytes"], f"{where}.size_bytes", minimum=1)
        != path.stat().st_size
    ):
...
def _safetensor_keys(path: Path) -> set[str]:
    try:
        ...
        raise ContractError(
            "safetensors is required to validate a Zig oracle capture"
        ) from exc
...
def _decode_u64_chunks(values: Any, where: str) -> list[int]:
    ...
    if array.dtype != np.float32 or array.ndim != 1 or array.size % 4 != 0:
        raise ContractError(
            f"{where} must be a flat F32 array with four chunks per integer"
        )
    result: list[int] = []
    for start in range(0, array.size, 4):
        ...
        for chunk_index, raw in enumerate(array[start : start + 4]):
            chunk = float(raw)
            if (
                not math.isfinite(chunk)
                or chunk != math.floor(chunk)
                or not 0 <= chunk <= 65535
            ):
...
def _validate_checkpoint_contract(
    path: Path, targets: Sequence[Mapping[str, Any]], steps: int, seed: int
) -> None:
    from safetensors import safe_open
    ...
    for target in targets:
        ...
        expected_keys.update(
            f"{prefix}::{slot}"
            for prefix in ("weight", "adam_m", "adam_v", "adam_step", "grad_accum")
        )
    with safe_open(str(path), framework="np", device="cpu") as source:
        if set(source.keys()) != expected_keys:
            raise ContractError(
                "trainer checkpoint tensor inventory differs from the closed oracle contract"
            )
        counters = _decode_u64_chunks(
            source.get_tensor("__trainer_counters"), "__trainer_counters"
        )
        if counters != [steps, steps]:
            raise ContractError(
                f"trainer checkpoint counters differ from requested steps: {counters}"
            )
        state = _decode_u64_chunks(
            source.get_tensor("__trainer_state_v2"), "__trainer_state_v2"
        )
        if len(state) != 18 or state[:7] != [2, steps, steps, steps, 0, 1, seed]:
            raise ContractError(
                "trainer checkpoint state does not bind the requested completed trajectory"
            )
        if state[7] != steps or state[8] != 0 or state[9] != steps or state[11] != 0:
            raise ContractError(
                "trainer checkpoint dataset cursor is not at the requested epoch boundary"
            )
        for target in targets:
            slot = str(target["trainer_slot_name"])
            shape = tuple(
                _integer(dim, "capture target shape", minimum=1)
                for dim in _list(target["shape"], "capture target shape")
            )
            ...
            for prefix in ("weight", "adam_m", "adam_v", "grad_accum"):
                ...
                if (
                    ...
                    or tuple(tensor.shape) != checkpoint_shape
                ):
                    raise ContractError(
                        f"checkpoint {prefix} tensor metadata differs for {slot}"
                    )
                if not bool((tensor == tensor).all()) or not bool(
                    abs(tensor).max(initial=0) < float("inf")
                ):
                    raise ContractError(
                        f"checkpoint {prefix} tensor is non-finite for {slot}"
                    )
            step_tensor = source.get_tensor(f"adam_step::{slot}")
            if (
                step_tensor.dtype.name != "float32"
                or tuple(step_tensor.shape) != (1,)
                or float(step_tensor[0]) != steps
            ):
                raise ContractError(f"checkpoint Adam step differs for {slot}")
            if bool(source.get_tensor(f"grad_accum::{slot}").any()):
                raise ContractError(
                    f"checkpoint retains a nonzero accumulated gradient for {slot}"
                )
...
def validate_zig_capture(
    ...
    if payload["schema_version"] != CAPTURE_SCHEMA_VERSION:
        raise ContractError(
            f"unsupported Zig capture schema: {payload['schema_version']!r}"
        )
    if payload["request_sha256"] != prefixed_sha256(request_path):
        raise ContractError(
            "Zig capture request SHA-256 differs from the admitted request"
        )
    ...
    loss_history = [
        _finite(value, "Zig capture loss")
        for value in _list(result["loss_history"], "Zig capture.loss_history")
    ]
    if len(loss_history) != steps:
        raise ContractError(
            "Zig capture must contain one finite loss per requested step"
        )
    raw_gradient_norm = _finite(
        result["raw_gradient_norm"], "Zig capture.raw_gradient_norm"
    )
    ...
    supervised_positions = [
        index for index in range(1, len(labels)) if labels[index] != -100
    ]
    if result["supervised_tokens"] != len(supervised_positions):
        raise ContractError(
            "Zig capture supervised-token count differs from the prepared row"
        )

    probes = _list(result["logit_probes"], "Zig capture.logit_probes")
    if len(probes) != len(supervised_positions):
        raise ContractError(
            "Zig capture must contain one logit probe per supervised causal position"
        )
    for probe_index, (raw_probe, label_position) in enumerate(
        zip(probes, supervised_positions, strict=True)
    ):
        ...
        _exact_keys(
            ...
            f"Zig capture.logit_probes[{probe_index}]",
        )
        predictor = label_position - 1
        target = labels[label_position]
        if (
            probe["predictor_position"] != predictor
            or probe["target_token_id"] != target
        ):
            raise ContractError(
                "Zig capture logit probe is not aligned to the prepared causal target"
            )
        token_ids = [
            _integer(value, "Zig capture probe token id")
            for value in _list(probe["token_ids"], "Zig capture probe token_ids")
        ]
        if token_ids != _stable_probe_token_ids(target, vocab_size, predictor, seed):
            raise ContractError(
                "Zig capture logit probe does not use the locked deterministic token projection"
            )
        values = [
            _finite(value, "Zig capture probe value")
            for value in _list(probe["values"], "Zig capture probe values")
        ]
        if len(values) != len(token_ids):
            raise ContractError(
                "Zig capture logit probe values and token IDs differ in length"
            )
    ...
    if execution["optimizer_steps"] != steps or execution["micro_batch_steps"] != steps:
        raise ContractError(
            "Zig capture execution counters differ from the requested trajectory"
        )
    backend = _mapping(payload["implementation"], "Zig capture.implementation")[
        "backend"
    ]
    if backend == "metal":
        if (
            ...
            raise ContractError(
                "Zig capture does not attest a fallback-free Metal trajectory"
            )
    elif backend == "native":
        if execution["metal_optimizer_steps"] != 0:
            raise ContractError(
                "native Zig capture unexpectedly reports Metal optimizer steps"
            )
    else:
        raise ContractError(f"unsupported Zig capture backend: {backend!r}")

    targets = [
        _mapping(value, f"Zig capture.targets[{index}]")
        for index, value in enumerate(_list(result["targets"], "Zig capture.targets"))
    ]
    target_fields = (
        "source_name",
        "trainer_slot_name",
        ...
        "checkpoint_weight_storage_key",
        "checkpoint_m_storage_key",
        "checkpoint_v_storage_key",
    )
    expected_count = (
        _integer(
            _mapping(payload["bindings"], "Zig capture.bindings")["target_count"],
            "Zig capture target_count",
            minimum=1,
        )
        * 2
    )
    if len(targets) != expected_count:
        raise ContractError(
            "Zig capture target count differs from the admitted adapter"
        )
    source_tensors = _mapping(source_adapter["tensors"], "source adapter tensors")
    candidate_tensors = _mapping(
        candidate_adapter["tensors"], "candidate adapter tensors"
    )
    if set(source_tensors) != set(candidate_tensors):
        raise ContractError(
            "candidate adapter changed the initial canonical tensor inventory"
        )
    ...
    for index, target in enumerate(targets):
        ...
        source_name = _string(
            target["source_name"], f"Zig capture.targets[{index}].source_name"
        )
        identity = canonicalize_adapter_tensor_name(source_name)
        if identity in identities or identity not in source_tensors:
            raise ContractError(
                f"Zig capture target identity is duplicate or absent from the adapter: {identity}"
            )
        identities.add(identity)
        slot = _string(
            target["trainer_slot_name"],
            f"Zig capture.targets[{index}].trainer_slot_name",
        )
        if slot in slots:
            raise ContractError(f"Zig capture reuses trainer slot {slot}")
        slots.add(slot)
        shape = tuple(
            _integer(dim, "Zig capture target shape", minimum=1)
            for dim in _list(target["shape"], "Zig capture target shape")
        )
        if (
            len(shape) != 2
            or shape != tuple(source_tensors[identity]["shape"])
            or shape != tuple(candidate_tensors[identity]["shape"])
        ):
            raise ContractError(
                f"Zig capture target shape differs from adapter metadata for {identity}"
            )
        if (
            source_tensors[identity]["dtype"] != "float32"
            or candidate_tensors[identity]["dtype"] != "float32"
        ):
            raise ContractError(
                f"Zig oracle adapters must use float32 LoRA tensors: {identity}"
            )
        expected_storage = {
        ...
        for field, expected_key in expected_storage.items():
            if target[field] != expected_key:
                raise ContractError(
                    f"Zig capture target storage key is not canonical: {field}"
                )
        gradient_keys.add(str(target["gradient_storage_key"]))
    if identities != set(source_tensors):
        raise ContractError(
            "Zig capture does not cover the full adapter tensor inventory"
        )

    artifacts = _mapping(payload["artifacts"], "Zig capture.artifacts")
    _exact_keys(
        artifacts,
        ("raw_gradients", "trainer_checkpoint", "candidate_adapter_model"),
        "Zig capture.artifacts",
    )
    raw_path = _artifact_file(
        root,
        _mapping(artifacts["raw_gradients"], "raw_gradients artifact"),
        "raw_gradients.safetensors",
        "raw_gradients artifact",
    )
    checkpoint_path = _artifact_file(
        root,
        _mapping(artifacts["trainer_checkpoint"], "trainer_checkpoint artifact"),
        "trainer_checkpoint.safetensors",
        "trainer_checkpoint artifact",
    )
    candidate_descriptor = _mapping(
        artifacts["candidate_adapter_model"], "candidate_adapter_model artifact"
    )
    _exact_keys(
        candidate_descriptor,
        ("sha256", "size_bytes"),
        "candidate_adapter_model artifact",
    )
    candidate_checkpoint = _regular_file(
    ...
    if (
        _integer(
            candidate_descriptor["size_bytes"], "candidate adapter size", minimum=1
        )
        != candidate_checkpoint.stat().st_size
    ):
    ...
    if candidate_adapter["adapter_model_sha256"] != candidate_descriptor["sha256"]:
        raise ContractError(
            "candidate adapter inspection and Zig capture digest disagree"
        )
    if _safetensor_keys(raw_path) != gradient_keys:
        raise ContractError(
            "raw-gradient Safetensors inventory differs from the capture target manifest"
        )
...
def _tensor_f32(source: Any, key: str, shape: tuple[int, ...], where: str) -> Any:
    ...
    if tensor.dtype != np.float32 or tuple(tensor.shape) != shape:
        raise ContractError(
            f"{where} tensor {key!r} must be float32 with shape {shape}"
        )
...
def _build_trace_tensor_store(
    ...
    with (
        safe_open(
            str(source_checkpoint), framework="np", device="cpu"
        ) as initial_source,
        safe_open(
            str(candidate_checkpoint), framework="np", device="cpu"
        ) as updated_source,
        safe_open(
            str(capture["raw_gradients_path"]), framework="np", device="cpu"
        ) as gradient_source,
        safe_open(
            str(capture["checkpoint_path"]), framework="np", device="cpu"
        ) as checkpoint_source,
    ):
        for tensor_index, identity in enumerate(sorted(target_by_identity)):
            ...
            if not np.array_equal(updated, checkpoint_weight):
                raise ContractError(
                    f"candidate adapter and trainer checkpoint weights differ for {identity}"
                )
            ...
            if expectation == "active" and not gradient_nonzero:
                raise ContractError(
                    f"active Zig target has an all-zero gradient: {identity}"
                )
            if expectation != "active" and gradient_nonzero:
                raise ContractError(
                    f"zero-initialization Zig gradient expectation failed: {identity}"
                )
            gradient_squares.append(
                float(np.sum(gradient.astype(np.float64) ** 2, dtype=np.float64))
            )
    ...
    if not math.isclose(
        grad_norm, float(capture["raw_gradient_norm"]), rel_tol=1e-9, abs_tol=1e-12
    ):
        raise ContractError(
...
def _publish_staging(staging: Path, output: Path) -> None:
    ...
    try:
        ...
        raise ContractError(
            f"refusing to replace existing Zig oracle output: {target}"
        ) from exc
...
def _validate_optimizer_contract(
    ...
    if supplied != expected:
        raise ContractError(
            f"optimizer arguments differ from training_contract (expected={expected}, actual={supplied})"
        )
    return trajectory
...
def export(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for immutable in (
        ...
        if _paths_overlap(output, immutable):
            raise ContractError(
                f"Zig oracle output overlaps immutable input: {immutable}"
            )
    ...
    prepared_summary, prepared = load_prepared_example(
        prepared_path, args.example_index
    )
    verified_source = verify_prepared_source_dataset(
        prepared_summary, args.source_dataset
    )
    source_dataset_path = Path(verified_source["path"])
    if _paths_overlap(output, source_dataset_path):
        raise ContractError(
            "Zig oracle output overlaps the immutable prepared source dataset"
        )

    source_adapter = inspect_adapter_artifact(
        adapter_dir, target_preset=args.target_preset
    )
    if source_adapter["key_layout"] != ANTFLY_ADAPTER_KEY_FORMAT:
        raise ContractError(
            "the Zig oracle requires a provenance-bound Antfly internal-key adapter"
        )
    ...
    for field in ("base_model_sha256", "tokenizer_sha256", "chat_template_sha256"):
        if provenance.get(field) != prepared_summary.get(field):
            raise ContractError(
                f"source adapter and prepared artifact disagree on {field}"
            )
    ...
    source_dataset_snapshot = _tree_snapshot(
        source_dataset_path, "prepared source dataset"
    )
    ...
    request = {
        ...
        "training": {
            ...
            "supervised_token_normalization": trajectory[
    ...
    with tempfile.TemporaryDirectory(
        prefix=f".{output.name}.zig-oracle-", dir=output.parent
    ) as temporary_name:
        ...
        returncode = _run_child(
            command, environment, stdout_path, stderr_path, args.timeout_seconds
        )
        if returncode != 0 or not candidate_dir.is_dir() or not capture_dir.is_dir():
            detail = _tail(stderr_path).strip()
            suffix = f"\nchild stderr tail:\n{detail}" if detail else ""
            raise ContractError(
                f"Antfly Zig oracle child failed with exit status {returncode}{suffix}"
            )
        ...
        if source_revision_after != source_revision:
            raise ContractError(
                "Antfly source revision changed during the Zig oracle run"
            )
        ...
        if (
            executable_snapshot_after != executable_snapshot
            or prefixed_sha256(executable) != executable_sha256
        ):
        ...
        if (
            prefixed_sha256(prepared_path)
            != request["bindings"]["train_prepared_sha256"]
        ):
            raise ContractError(
                "prepared artifact digest changed during the Zig oracle run"
            )
        if (
            prefixed_sha256(adapter_dir / "adapter_model.safetensors")
            != request["bindings"]["initial_adapter_sha256"]
        ):
            raise ContractError(
                "source adapter digest changed during the Zig oracle run"
            )
        verify_prepared_source_dataset(prepared_summary, args.source_dataset)

        candidate_adapter = inspect_adapter_artifact(
            candidate_dir, target_preset=preset
        )
        if (
        ...
        candidate_provenance = _mapping(
            candidate_adapter.get("provenance"), "candidate adapter provenance"
        )
        for field in ("base_model_sha256", "tokenizer_sha256", "chat_template_sha256"):
            if candidate_provenance.get(field) != prepared_summary.get(field):
                raise ContractError(
                    f"candidate adapter and prepared artifact disagree on {field}"
                )
        ...
        trace = {
            ...
            "metrics": {
                ...
                "supervised_tokens": len(
                    [value for value in prepared["labels"][1:] if value != -100]
                ),
            },
        ...
        write_json(
            staging / "packaging_environment.json",
            {
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument(
        "--antfly", type=Path, required=True, help="release-built Antfly executable"
    )
    result.add_argument(
        "--source-root",
        type=Path,
        required=True,
        help="clean source checkout used to build the executable",
    )
    result.add_argument(
    ...
    result.add_argument(
        "--source-dataset",
        type=Path,
        help="override the recorded v6 source path while preserving its digest",
    )
    ...
    result.add_argument(
        "--metal-device", help="auditable Metal device label; required only for Metal"
    )
