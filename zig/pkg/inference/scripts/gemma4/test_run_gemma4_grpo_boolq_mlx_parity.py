...
class BoolQGrpoMlxParityTests(unittest.TestCase):
    ...
    def _materialization(self, root: Path) -> Path:
        ...
        payload = {
            ...
            "dataset": {
                ...
                "revision": "a" * 40,
                "selection_policy": {
                    "ordering": parity.boolq_materializer.V2_SELECTION_ORDERING,
    ...
    def test_materialization_binds_jsonl_digests_and_disjoint_ids(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            drifted["dataset"]["revision"] = "b" * 40
            path.write_text(json.dumps(drifted), encoding="utf-8")
            with self.assertRaisesRegex(
                parity.BoolQParityContractError, "semantic SHA-256 drifted"
            ):
    ...
    def test_materialization_keeps_legacy_v1_compatibility(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            path = self._materialization(root)
