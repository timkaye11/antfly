...
class IncrementalKvParityGateTests(unittest.TestCase):
    ...
    def test_v8_requires_the_qualified_training_order(self) -> None:
        report = {
            ...
            "training_order": dict(gate.GRPO_TRAINING_ORDER),
        }
        gate.require_training_order_contract(report, "candidate")
        report["training_order"]["algorithm"] = "fixed-order"
        with self.assertRaisesRegex(gate.ParityError, "training-order contract"):
            gate.require_training_order_contract(report, "candidate")

        # Historical schemas predate this field and remain usable for archived
        # eager/incremental comparisons.
        gate.require_training_order_contract(
            {"schema_version": "antfly_inference_finetune_grpo_report/v7"},
            "baseline",
        )
