...
def _regular_json(path: Path, label: str) -> tuple[Path, Mapping[str, Any]]:
    ...
    try:
        ...
        raise PrefixAdapterParityError(f'{label} is not valid JSON: {resolved}') from exc
...
def validate_mlx_run(
    ...
    if Path(str(status.get('adapter_output_path', ''))).expanduser().resolve() != adapter_path:
    ...
    if (
        ...
        raise PrefixAdapterParityError('MLX runner identity is not stable across the guarded run')
    ...
    if (
        ...
        raise PrefixAdapterParityError('MLX report is not the requested trace-replay boundary')
    return {
...
def compare_adapter_files(
    ...
    for name in seed:
        ...
        if identity in seed_by_identity:
            raise PrefixAdapterParityError(f'duplicate seed adapter identity: {identity}')
    ...
    for checkpoint_name, values in checkpoint.items():
        ...
        if seed_name in metal_by_seed_name:
            raise PrefixAdapterParityError(f'duplicate Metal checkpoint identity: {identity}')
        ...
        if flat.size != seed_values.size:
            raise PrefixAdapterParityError(f'Metal checkpoint size differs for {seed_name}')
    ...
    for name in sorted(seed):
        ...
        tensor_dot = float(np.sum(
            metal_delta.astype(np.float64) * mlx_delta.astype(np.float64)
        ))
        ...
        per_tensor.append({
            ...
            'delta_cosine_similarity': (
                ...
                if metal_norm > 0.0 and mlx_norm > 0.0 else None
            ),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if (
        ...
        raise PrefixAdapterParityError('Metal capture is not the requested group boundary')
    checkpoint_artifact = summary.get('artifacts', {}).get('checkpoint', {})
    if checkpoint_artifact.get('sha256') != sha256_file(checkpoint_path):
        raise PrefixAdapterParityError('Metal checkpoint digest differs from capture summary')
    ...
    comparison = compare_adapter_files(
        args.seed_adapter, checkpoint_path, args.mlx_adapter,
    )
    ...
    payload = {
        ...
        'artifacts': {
            'seed_adapter': {'path': str(_adapter_path(args.seed_adapter)), 'sha256': sha256_file(_adapter_path(args.seed_adapter))},
            'metal_checkpoint': {'path': str(checkpoint_path), 'sha256': sha256_file(checkpoint_path)},
            'metal_capture_summary': {'path': str(summary_path), 'sha256': sha256_file(summary_path)},
            'mlx_adapter': {'path': str(_adapter_path(args.mlx_adapter)), 'sha256': sha256_file(_adapter_path(args.mlx_adapter))},
            'mlx_run_status': {'path': mlx_run['status_path'], 'sha256': mlx_run['status_sha256']},
            'mlx_run_report': {'path': mlx_run['report_path'], 'sha256': mlx_run['report_sha256']},
        },
    }
