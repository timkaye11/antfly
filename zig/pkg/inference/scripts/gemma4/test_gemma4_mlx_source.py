...
class SourceArchiveTests(unittest.TestCase):
    def setUp(self):
        ...
        self.write_archive()
        self.pin = patch.dict(source.MLX_LM_ARCHIVE_SHA256,
            {self.revision: hashlib.sha256(self.archive.read_bytes()).hexdigest()}, clear=True)
        self.pin.start()
    ...
    def write_archive(self, name='model.py'):
        ...
        with tarfile.open(self.archive, 'w:gz') as out:
            entry = tarfile.TarInfo(prefix); entry.type = tarfile.DIRTYPE; out.addfile(entry)
            entry = tarfile.TarInfo(prefix + '/' + name); entry.size = len(self.payload)
    ...
    def test_exact_source_is_bound_to_pinned_archive(self):
        ...
        self.assertEqual(hashlib.sha256(self.archive.read_bytes()).hexdigest(), result['archive_sha256'])
    ...
    def test_changed_source_and_generated_bytecode_are_rejected(self):
        ...
        with self.assertRaisesRegex(source.SourceArchiveError, 'file changed'):
            self.attest()
        (self.root / 'model.py').write_bytes(self.payload)
        cache = self.root / '__pycache__'; cache.mkdir(); (cache / 'model.pyc').write_bytes(b'stale')
    ...
    def test_source_symlink_is_rejected(self):
        original = self.root / 'model.py'; original.rename(self.base / 'elsewhere.py')
    ...
    def test_traversal_is_rejected_even_with_matching_archive_digest(self):
        ...
        source.MLX_LM_ARCHIVE_SHA256[self.revision] = hashlib.sha256(self.archive.read_bytes()).hexdigest()
