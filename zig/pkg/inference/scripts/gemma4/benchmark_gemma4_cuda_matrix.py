...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=pathlib.Path,
        default=pathlib.Path("/tmp/antfly-gemma4-cuda-matrix"),
    )
    ...
    parser.add_argument(
        "--require-graph-replay", action=argparse.BooleanOptionalAction, default=True
    )
    parser.add_argument(
        "--require-generated-attention",
...
def summarize_entry(output_tokens: int, summary: dict, pair_exit_code: int = 0) -> dict:
    ...
    return {
        ...
        "generated_q6_lm_head_argmax_ok": bool(
            summary.get("ok_generated_q6_lm_head_argmax", False)
        ),
    }
...
def evaluate(entries: list[dict], args: argparse.Namespace) -> dict:
        ...
        slopes.append(
            {
                ...
                "additional_ms_per_token": right["antfly_ms_per_token"]
                - left["antfly_ms_per_token"],
            }
        )
    ...
    checks = {
        ...
        "graph_replay": (not args.require_graph_replay)
        or all(entry["graph_replay_ok"] for entry in entries),
        "generated_attention": (not args.require_generated_attention)
        or all(entry["generated_attention_ok"] for entry in entries),
        "generated_q6_lm_head_argmax": (not args.require_generated_q6_lm_head_argmax)
        or all(entry["generated_q6_lm_head_argmax_ok"] for entry in entries),
    }
    return {
        "config": {
        ...
        "median_antfly_tok_s": statistics.median(
            entry["antfly_tok_s"] for entry in entries
        ),
        "checks": checks,
...
def run_pair_case(
    pair_script: pathlib.Path, run_dir: pathlib.Path, env: dict[str, str]
) -> tuple[int, dict]:
        ...
        env.update(
            {
                ...
                "REQUIRE_GENERATED_ATTENTION": "1"
                if args.require_generated_attention
                else "0",
                "ANTFLY_GENERATED_ATTENTION_DECODE": "1"
                if args.require_generated_attention
                else "0",
                "REQUIRE_GENERATED_Q6_LM_HEAD_ARGMAX": "1"
                if args.require_generated_q6_lm_head_argmax
                else "0",
...
def main() -> None:
    ...
    if len(set(args.lengths)) != len(args.lengths) or any(
        length < 2 for length in args.lengths
    ):
    ...
    target = next(
        entry for entry in entries if entry["output_tokens"] == args.target_length
    )
    print(
