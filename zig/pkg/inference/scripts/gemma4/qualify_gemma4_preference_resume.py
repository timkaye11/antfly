...
durable checkpoint, and resumed into a fresh immutable artifact root. The
default interruption target is an epoch boundary; `--interrupt-after-examples`
instead targets a durable mid-epoch checkpoint written by the eager
`checkpoint.every_examples` cadence (DPO pairs or GRPO prompt groups), which
is the only way to interrupt a one-epoch trajectory. A PASS requires a
content-addressed aggregate sidecar, byte-identical final adapter tensors,
exact training/discrete metrics and traces, and a bounded absolute tolerance
only for terminal Metal GRPO KL floats. The harness is standard-library-only.
"""
...
STATE_SCHEMA_VERSION_V1 = "antfly_gemma4_preference_checkpoint_state/v1"
STATE_SCHEMA_VERSION_V2 = "antfly_gemma4_preference_checkpoint_state/v2"
...
def _expected_sha256(value: Any, where: str) -> str | None:
    if value is None:
        return None
...
def inspect_training_checkpoint(path: Path) -> dict[str, Any]:
    ...
    if (
        fields[2] > fields[1]
        or fields[3] != fields[1]
        or fields[5] == 0
        or fields[4] >= fields[5]
        or fields[4] > fields[1]
        or fields[16] > 64
        or (fields[16] < 64 and fields[17] >> fields[16] != 0)
    ):
...
def inspect_preference_checkpoint(path: Path, task: str) -> dict[str, Any]:
    ...
    if schema_version not in STATE_SCHEMA_VERSIONS or state.get("task") != task:
        raise ContractError("checkpoint sidecar schema or task mismatch")
    examples_into_epoch = _integer(
        state.get("examples_into_epoch", 0),
        "checkpoint sidecar.examples_into_epoch",
    )
    if schema_version == STATE_SCHEMA_VERSION_V1 and examples_into_epoch != 0:
        raise ContractError("v1 checkpoint sidecar cannot carry a mid-epoch cursor")
    ...
    return {
        **trainer,
        ...
        "examples_into_epoch": examples_into_epoch,
...
def _retain_interrupted_boundary(
    ...
    sources = {
        "training_checkpoint": (
            _regular_file(checkpoint_path, "interrupted training checkpoint"),
            checkpoint_state.get("sha256"),
        ),
        "preference_sidecar": (
            _regular_file(
                Path(str(checkpoint_state.get("state_path"))),
                "interrupted preference sidecar",
            ),
            checkpoint_state.get("state_sha256"),
        ),
    }
    evidence: dict[str, dict[str, Any]] = {}
    for name, (source, expected_digest) in sources.items():
        if not isinstance(expected_digest, str) or _sha256(source) != expected_digest:
            raise ContractError(f"interrupted {name} changed before retention")
        destination = retained_root / source.name
        try:
            os.link(source, destination, follow_symlinks=False)
        except OSError as exc:
            raise ContractError(
                f"cannot retain interrupted {name} with an immutable hard link: {exc}"
            ) from exc
        retained = _regular_file(destination, f"retained interrupted {name}")
        if _sha256(retained) != expected_digest:
            raise ContractError(f"retained interrupted {name} digest mismatch")
        evidence[name] = {
            "path": str(retained),
...
def _verify_retained_interrupted_boundary(
    evidence: Mapping[str, Any],
) -> None:
    for name in ("training_checkpoint", "preference_sidecar"):
        artifact = _mapping(evidence.get(name), f"retained interrupted {name}")
        path_value = artifact.get("path")
        if not isinstance(path_value, str):
            raise ContractError(f"retained interrupted {name}: missing path")
        path = _regular_file(Path(path_value), f"retained interrupted {name}")
        if path.stat().st_size != _integer(
            artifact.get("size_bytes"),
            f"retained interrupted {name}.size_bytes",
            1,
        ):
            raise ContractError(f"retained interrupted {name} size changed")
        if _sha256(path) != artifact.get("sha256"):
            raise ContractError(f"retained interrupted {name} digest changed")
...
def _run_and_interrupt(
    ...
    with stdout_path.open("wb") as stdout, stderr_path.open("wb") as stderr:
        ...
        try:
            while time.monotonic() - started < timeout_seconds:
                ...
                if checkpoint_path.exists():
                    ...
                    if candidate is not None:
                        ...
                        examples = _integer(
                            candidate["examples_into_epoch"],
                            "checkpoint examples_into_epoch",
                        )
                        # Durable checkpoints advance in (epoch, cursor)
                        # lexicographic order: mid-epoch cursors within an
                        # epoch, then the boundary save with cursor zero for
                        # the next epoch index.
                        if (epoch, examples) > (expected_epoch, expected_examples):
                            raise ContractError(
                                "checkpoint advanced past interruption boundary: "
                                f"({epoch}, {examples}) > ({expected_epoch}, {expected_examples})"
                            )
                        if (epoch, examples) == (expected_epoch, expected_examples):
                            state = candidate
                            break
            ...
            if state is None:
                raise ContractError(
                    f"checkpoint (epoch {expected_epoch}, examples {expected_examples}) "
                    f"was not observed within {timeout_seconds:g}s"
                )
    ...
    if final_state["examples_into_epoch"] != expected_examples:
        raise ContractError("checkpoint cursor does not match the interruption target")
    return (
        {
            "pid": process.pid,
...
def _recipe_variant(
    ...
    recipe["checkpoint"] = {
        ...
        **(
            {"every_examples": checkpoint_every_examples}
            if checkpoint_every_examples
            else {}
        ),
...
def _require_final_report_checkpoint_consistency(
    report: Mapping[str, Any],
    checkpoint: Mapping[str, Any],
    task: str,
) -> dict[str, int]:
    binding: dict[str, int] = {}
    for field in ("optimizer_steps", "micro_batch_steps"):
        reported = _integer(report.get(field), f"final report.{field}")
        durable = _integer(checkpoint.get(field), f"final checkpoint.{field}")
        if reported != durable:
            raise ContractError(
                f"final report {field} does not match the durable checkpoint"
            )
        binding[field] = durable
    horizon_field = "examples" if task == "dpo" else "groups"
    reported_horizon = _integer(
        report.get(horizon_field),
        f"final report.{horizon_field}",
        1,
    )
    durable_horizon = _integer(
        checkpoint.get("examples_seen"),
        "final checkpoint.examples_seen",
        1,
    )
    if reported_horizon != durable_horizon:
        raise ContractError(
...
def qualify(args: argparse.Namespace) -> Mapping[str, Any]:
    ...
    interrupt_after_examples = _integer(
        getattr(args, "interrupt_after_examples", 0),
        "--interrupt-after-examples",
    )
    expected_final_adapter_sha256 = _expected_sha256(
        getattr(args, "expected_final_adapter_sha256", None),
        "--expected-final-adapter-sha256",
    )
    if interrupt_after_examples:
        # Mid-epoch mode: interrupt during epoch `interrupt_after_epoch` once
    ...
    if interrupt_after_examples and compiled_sampling:
        raise ContractError(
            "mid-epoch interruption is qualified for eager sampling only"
        )
    if interrupt_after_examples and args.incremental_kv:
        raise ContractError(
            "mid-epoch interruption is not qualified with incremental KV"
        )
    ...
    if direct_gguf_training and interrupt_after_examples:
        raise ContractError(
            "mid-epoch interruption is not qualified for direct GGUF training"
        )
    ...
    configured_max_examples = dataset_config.get("max_examples")
    if configured_max_examples is not None:
        configured_max_examples = _integer(
            configured_max_examples,
            "recipe.dataset.max_examples",
            1,
        )
        if interrupt_after_examples >= configured_max_examples:
            raise ContractError(
                "--interrupt-after-examples must be smaller than "
                "recipe.dataset.max_examples"
            )
    ...
    for name, root, checkpoint, resume in (
        ...
        _write_json(
            ...
            _recipe_variant(
                ...
                checkpoint,
                args.epochs,
                # Mid-epoch mode keeps only the mandatory final-epoch boundary
                # save so the polled interruption target is unambiguous.
                args.epochs if interrupt_after_examples else args.interrupt_after_epoch,
                interrupt_after_examples,
                resume,
            ),
        )
        recipes[name] = path
    ...
    if uninterrupted_state["examples_into_epoch"] != 0:
        raise ContractError("uninterrupted final checkpoint is not an epoch boundary")
    ...
    retained_interrupted_boundary = _retain_interrupted_boundary(
        interrupted_checkpoint,
        interrupted_state,
        output_root / "interrupted-boundary",
    )
    ...
    _verify_retained_interrupted_boundary(retained_interrupted_boundary)
    ...
    if resumed_state["examples_into_epoch"] != 0:
        raise ContractError("resumed final checkpoint is not an epoch boundary")
    ...
    parity["final_checkpoint_counter_binding"] = {
        ...
        "resumed": _require_final_report_checkpoint_consistency(
            semantic_report,
            resumed_state,
            task,
        ),
    ...
    trained_adapter_sha256 = _require_changed_adapter(
        input_evidence["adapter"]["checkpoint_sha256"],
        parity.get("adapter_model_sha256"),
    )
    if (
        expected_final_adapter_sha256 is not None
        and trained_adapter_sha256 != expected_final_adapter_sha256
    ):
        raise ContractError(
            "trained adapter does not match --expected-final-adapter-sha256"
        )
    ...
    report = {
        ...
        "contract": {
            ...
            "interrupt_after_examples": interrupt_after_examples,
        ...
        "retained_interrupted_boundary": retained_interrupted_boundary,
        "parity": parity,
    }
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "interrupt at the durable mid-epoch checkpoint written after this "
            "many examples (DPO pairs or GRPO groups) of --interrupt-after-epoch; "
            "0 keeps the epoch-boundary contract"
        ),
    )
    parser.add_argument(
        "--expected-final-adapter-sha256",
        help=(
            "optionally require the uninterrupted and resumed final adapter "
            "to match this pinned SHA-256 digest"
        ),
    )
