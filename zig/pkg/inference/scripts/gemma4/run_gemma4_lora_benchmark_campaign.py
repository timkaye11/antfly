...
def canonical_argv_sha256(argv: Sequence[str]) -> str:
    ...
    for index, argument in enumerate(argv):
        if not isinstance(argument, str) or not argument or "\x00" in argument:
            raise ContractError(
                f"runner argv[{index}] must be a non-empty string without NUL"
            )
...
def _reserved_option_present(argv: Sequence[str], option: str) -> bool:
    return any(
        argument == option or argument.startswith(option + "=") for argument in argv
    )
...
def _executable_path(path: Path, where: str) -> Path:
    ...
    try:
        ...
        raise ContractError(
            f"{where}: could not resolve executable {path}: {exc}"
        ) from exc
...
def _resolve_command(raw: Any, framework: str, where: str) -> list[str]:
    ...
    if any(_reserved_option_present(argv, option) for option in RESERVED_OPTIONS):
        raise ContractError(
            f"{where}: campaign identity/output options are reserved to the orchestrator"
        )
...
def load_plan(path: Path) -> tuple[dict[str, Any], str]:
    ...
    require_exact_keys(
        plan, ("schema_version", "repetitions", "cells"), where="campaign plan"
    )
    ...
    for index, raw_cell in enumerate(raw_cells):
        ...
        require_exact_keys(
            cell, ("cell_id", "commands"), where=f"campaign plan.cells[{index}]"
        )
        ...
        require_exact_keys(
            commands, FRAMEWORKS, where=f"campaign plan.cells[{index}].commands"
        )
        cells.append(
            {
                "cell_id": cell_id,
                "commands": {
                    framework: _resolve_command(
                        commands[framework],
                        framework,
                        f"campaign plan.cells[{index}].commands.{framework}",
                    )
...
def _sample_identity(path: Path, expected: Mapping[str, Any]) -> None:
    ...
    for field in (
        ...
        if field not in payload:
            raise ContractError(
                f"runner sample: missing orchestrator-owned field {field}"
            )
    for field in ("campaign_id", "run_id", "repetition", "sequence_index", "framework"):
        if payload[field] != expected[field]:
            raise ContractError(
                f"runner sample: {field} differs from the orchestrated run"
            )
...
def _atomic_publish(path: Path, payload: Mapping[str, Any]) -> None:
    ...
    try:
        ...
        try:
            ...
            raise ContractError(
                f"refusing to replace existing campaign ledger: {target}"
            ) from exc
        published = True
...
def execute_campaign(
    ...
    *,
    ...
    if manifest_path.exists() or manifest_path.is_symlink():
        raise ContractError(
            f"refusing to replace existing campaign ledger: {manifest_path}"
        )
    ...
    for cell in plan["cells"]:
        for repetition in range(plan["repetitions"]):
            ...
            for framework in order:
                run_id = _identifier(
                    f"{campaign_id}.run-{sequence_index:04d}-{token(8)}",
                    "generated run_id",
                )
                if run_id in generated_run_ids:
                    raise ContractError(
                        "campaign run identifier generator produced a collision"
                    )
                ...
                if runs and started_unix_ns < runs[-1]["completed_unix_ns"]:
                    raise ContractError(
                        "campaign clock moved backwards across serial runs"
                    )
                ...
                if completed_unix_ns <= started_unix_ns:
                    raise ContractError(
                        "campaign run completion must be after its start"
                    )
                if isinstance(returncode, bool) or not isinstance(returncode, int):
                    raise ContractError(
                        "campaign runner returned a non-integer exit status"
                    )
                if returncode != 0:
                    raise ContractError(
    ...
    if prefixed_sha256(_regular_non_symlink(plan_path, "campaign plan")) != plan_digest:
        raise ContractError(
            "campaign plan changed during execution; COMPLETE was not published"
        )
    for run in runs:
        artifact = run["sample"]
        sample_path = _evidence_file(
            output_root, artifact["relative_path"], "final campaign sample"
        )
        if (
            sample_path.stat().st_size != artifact["size_bytes"]
            or prefixed_sha256(sample_path) != artifact["sha256"]
        ):
...
def verify_complete_campaign_manifest(
    ...
    created = _integer(
        manifest["created_unix_ns"], "campaign manifest.created_unix_ns", 1
    )
    completed = _integer(
        manifest["completed_unix_ns"], "campaign manifest.completed_unix_ns", 1
    )
    ...
    require_exact_keys(
        plan,
        ("sha256", "cell_count", "repetitions_per_cell"),
        where="campaign manifest.plan",
    )
    ...
    repetitions = _integer(
        plan["repetitions_per_cell"], "campaign manifest.plan.repetitions_per_cell", 1
    )

    orchestrator = _mapping(manifest["orchestrator"], "campaign manifest.orchestrator")
    require_exact_keys(
        orchestrator,
        ("schema_version", "relative_path", "source_sha256"),
    ...
    if orchestrator["relative_path"] != ORCHESTRATOR_RELATIVE_PATH:
        raise ContractError(
            "campaign manifest: orchestrator path differs from the admitted source"
        )
    ...
    if orchestrator_source_sha256 != prefixed_sha256(SCRIPT_PATH):
        raise ContractError(
            "campaign manifest: orchestrator source hash differs from the validating source"
        )
    ...
    if run_count != len(raw_runs) or run_count != expected_run_count:
        raise ContractError(
            "campaign manifest run count is not closed over every cell/repetition/framework"
        )
    ...
    for sample_path, payload in samples:
        ...
        if resolved in supplied:
            raise ContractError(
                "comparison supplied the same sample artifact more than once"
            )
    ...
    for index, raw_run in enumerate(raw_runs):
        ...
        cell_id = _identifier(
            run["cell_id"], f"campaign manifest.runs[{index}].cell_id"
        )
        ...
        if framework not in FRAMEWORKS:
            raise ContractError(
                f"campaign manifest.runs[{index}]: unsupported framework"
            )
        repetition = _integer(
            run["repetition"], f"campaign manifest.runs[{index}].repetition"
        )
        sequence_index = _integer(
            run["sequence_index"], f"campaign manifest.runs[{index}].sequence_index"
        )
        if sequence_index != index:
            raise ContractError(
                "campaign manifest sequence_index values must match immutable launch order"
            )
        ...
        if not isinstance(argv, list) or len(argv) < 12:
            raise ContractError(
                f"campaign manifest.runs[{index}].argv must contain the runner and owned suffix"
            )
        argv_digest = canonical_argv_sha256(argv)
        if run["argv_sha256"] != argv_digest:
            raise ContractError(
                f"campaign manifest.runs[{index}]: argv digest mismatch"
            )
        if not Path(argv[0]).is_absolute() or not Path(argv[1]).is_absolute():
            raise ContractError(
                f"campaign manifest.runs[{index}]: executable and runner paths must be absolute"
            )
        if Path(argv[1]).name != RUNNER_NAMES[framework]:
            raise ContractError(
                f"campaign manifest.runs[{index}]: framework is relabeled against runner argv"
            )
        expected_flags = {
        ...
        for option, expected_value in expected_flags.items():
            if (
                _option_value(argv, option, f"campaign manifest.runs[{index}].argv")
                != expected_value
            ):
                raise ContractError(
                    f"campaign manifest.runs[{index}]: {option} differs from ledger identity"
                )
        expected_suffix = [
            "--campaign-id",
            campaign_id,
            ...
            str(sequence_index),
            "--output",
            _option_value(argv, "--output", f"campaign manifest.runs[{index}].argv"),
        ]
        if argv[-len(expected_suffix) :] != expected_suffix:
            raise ContractError(
                "campaign manifest runner argv does not end in the orchestrator-owned identity suffix"
            )
        prefix_key = (cell_id, framework)
        previous_prefix = command_prefixes.setdefault(
            prefix_key, argv[: -len(expected_suffix)]
        )
        if argv[: -len(expected_suffix)] != previous_prefix:
            raise ContractError(
                f"campaign manifest cell {cell_id!r} changed runner argv across repetitions"
            )
        started = _integer(
            run["started_unix_ns"],
            f"campaign manifest.runs[{index}].started_unix_ns",
            1,
        )
        finished = _integer(
            run["completed_unix_ns"],
            f"campaign manifest.runs[{index}].completed_unix_ns",
            1,
        )
        if finished <= started:
            raise ContractError(
                "campaign manifest contains a non-positive run interval"
            )
        if index == 0 and started < created:
            raise ContractError(
                "campaign manifest first run precedes campaign creation"
            )
        ...
        require_exact_keys(
            ...
            where=f"campaign manifest.runs[{index}].sample",
        )
        relative_path = _normalized_relative_path(
            artifact["relative_path"],
            f"campaign manifest.runs[{index}].sample.relative_path",
        )
        sample_path = _evidence_file(
            root, relative_path, f"campaign manifest.runs[{index}].sample"
        )
        if sample_path in ledger_paths:
            raise ContractError(
                "campaign manifest lists one sample artifact more than once"
            )
        ledger_paths.add(sample_path)
        sample_digest = _sha256(
            artifact["sha256"], f"campaign manifest.runs[{index}].sample.sha256"
        )
        sample_size = _integer(
            artifact["size_bytes"],
            f"campaign manifest.runs[{index}].sample.size_bytes",
            1,
        )
        if (
            sample_path.stat().st_size != sample_size
            or prefixed_sha256(sample_path) != sample_digest
        ):
            raise ContractError(
                "campaign manifest sample artifact was replaced or modified"
            )
        if _option_value(
            argv, "--output", f"campaign manifest.runs[{index}].argv"
        ) != str(sample_path):
            raise ContractError(
                "campaign manifest runner output path differs from the recorded sample"
            )
        payload = supplied.get(sample_path)
        if payload is None:
            raise ContractError(
                "campaign manifest sample membership differs from comparison inputs"
            )
        ...
        if encoded_payload != encoded_disk_payload:
            raise ContractError(
                "comparison sample payload differs from its recorded artifact"
            )
        payload = disk_payload
        implementation = _mapping(
            payload.get("implementation"), "comparison sample.implementation"
        )
        producer_source = _mapping(
            implementation.get("producer_source"),
            "comparison sample.implementation.producer_source",
        )
        producer_files = producer_source.get("files")
        if not isinstance(producer_files, list):
            raise ContractError(
                "comparison sample producer source files must be an array"
            )
        orchestrator_sources = [
            item
            for item in producer_files
            if isinstance(item, Mapping)
            and item.get("relative_path") == ORCHESTRATOR_RELATIVE_PATH
        ]
        if (
            len(orchestrator_sources) != 1
            or orchestrator_sources[0].get("source_sha256")
            != orchestrator_source_sha256
        ):
            raise ContractError(
                "campaign manifest orchestrator differs from the sample producer source closure"
            )
        expected_identity = {
            "campaign_id": campaign_id,
            "run_id": run_id,
        ...
        for field, expected_value in expected_identity.items():
            if payload.get(field) != expected_value:
                raise ContractError(
                    f"campaign manifest sample {field} differs from launch evidence"
                )
        process = _mapping(payload.get("process"), "comparison sample.process")
        process_started = _integer(
            process.get("started_unix_ns"),
            "comparison sample.process.started_unix_ns",
            1,
        )
        if not started <= process_started <= finished:
            raise ContractError(
                "campaign manifest sample process start is outside its launch interval"
            )
        normalized_runs.append(run)

    if len(cell_order) != cell_count:
        raise ContractError(
            "campaign manifest cell count differs from its launch groups"
        )
    ...
    for cell_id in cell_order:
        ...
        for repetition in range(repetitions):
            ...
            expected_order = list(
                FRAMEWORKS if repetition % 2 == 0 else tuple(reversed(FRAMEWORKS))
            )
            ...
            if pair[1]["sequence_index"] != pair[0]["sequence_index"] + 1:
                raise ContractError(
                    f"campaign manifest cell {cell_id!r} repetition {repetition} is not adjacent"
                )
            pair_payloads = [
                supplied[
                    _evidence_file(
                        root, run["sample"]["relative_path"], "campaign pair sample"
                    )
                ]
                for run in pair
            ]
            ...
            if len(case_encodings) != 1:
                raise ContractError(
                    f"campaign manifest cell {cell_id!r} paired different benchmark cases"
                )
    return manifest
