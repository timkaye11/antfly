...
from gemma4_oracle_contract import (
    ...
    verify_requirements_match_lock_subset,
    write_json,
)
...
class Gemma4OracleContractTest(unittest.TestCase):
    ...
    def test_lock_requirements_and_source_revisions_are_exact(self) -> None:
        ...
        self.assertEqual(
            {
                name: self.lock["python_oracle"]["packages"][name]
                for name in ZIG_ORACLE_PACKAGING_PACKAGES
            },
            verify_requirements_match_lock_subset(
                self.lock,
                "python_oracle",
                ZIG_ORACLE_PACKAGING_PACKAGES,
                script_dir / "requirements-gemma4-zig-oracle.txt",
            ),
        )
    ...
    def test_benchmark_producer_attestation_closes_clean_committed_source_tree(self) -> None:
        ...
        drift_path = "zig/pkg/inference/scripts/gemma4/gemma4_oracle_contract.py"
