...
class ResumeQualificationTest(unittest.TestCase):
    ...
    def test_end_to_end_exact_resume_contract(self) -> None:
        ...
        self.assertTrue(
            (self.root / "qualification" / "qualification_report.json").is_file()
        )
    ...
    def test_direct_gguf_model_artifact_is_snapshotted_and_accepted(self) -> None:
        ...
        self.assertEqual(
            report["inputs"]["model"]["snapshot_sha256"],
            qualifier._snapshot_digest(qualifier._tree_snapshot(gguf)),
        )
        self.assertTrue(report["contract"]["experimental_direct_gguf_qlora"])
        self.assertEqual(
            report["contract"]["strict_metal_environment"][
                "ANTFLY_EXPERIMENTAL_GEMMA4_GGUF_QLORA"
            ],
            "1",
        )
        self.assertFalse(
            (self.root / "qualification" / "uninterrupted" / "tokenizer.json").exists()
        )
    ...
    def test_direct_gguf_requires_explicit_experimental_admission(self) -> None:
        ...
        with self.assertRaisesRegex(
            qualifier.ContractError, "--experimental-gguf-qlora"
        ):
            qualifier.qualify(args)
    ...
    def test_checkpoint_parser_rejects_wrong_schema(self) -> None:
        ...
        path.write_bytes(
            struct.pack("<Q", len(header) + len(padding)) + header + padding + data
        )
