...
class Gemma4DpoMlxBenchmarkTest(unittest.TestCase):
    ...
    def test_pair_sequence_length_is_shared_bounded_and_matches_antfly_rounding(self) -> None:
        ...
        expected = min(((required + 15) // 16) * 16, case.sequence_length)
        self.assertEqual(
            case.sequence_length,
            benchmark.pair_sequence_length(example, case.sequence_length, None),
        )
        self.assertEqual(
            expected,
            benchmark.pair_sequence_length(example, case.sequence_length, 16),
        )
        self.assertEqual(
            96,
            benchmark.pair_sequence_length(example, case.sequence_length, 16, 96),
        )
        with self.assertRaisesRegex(
            benchmark.DpoBenchmarkContractError, "minimum requires a quantum"
        ):
            benchmark.pair_sequence_length(example, case.sequence_length, None, 16)
        with self.assertRaisesRegex(
            benchmark.DpoBenchmarkContractError, "quantum must be positive"
        ):
            benchmark.pair_sequence_length(example, case.sequence_length, 0)

    def test_sequence_length_policy_reports_actual_padding_reduction(self) -> None:
        case = benchmark.load_case(benchmark.DEFAULT_CASE_PATH)
        pair_lengths = [
            benchmark.pair_sequence_length(example, case.sequence_length, 16)
            for example in case.examples
        ]
        policy = benchmark.summarize_sequence_length_policy(
            case, pair_lengths, 16, None
        )
        self.assertEqual("pair-safe-length-buckets", policy["mode"])
        self.assertEqual(pair_lengths, policy["pair_sequence_lengths"])
        self.assertLess(
            policy["scheduled_branch_rows"], policy["fixed_shape_branch_rows"]
        )
        self.assertGreater(policy["padding_reduction_fraction"], 0.0)
    ...
    def test_case_rejects_unknown_model_key(self) -> None:
        payload = self.load_payload()
        payload["model_key"] = "gemma-4-unknown"
        with tempfile.TemporaryDirectory() as temp:
            with self.assertRaisesRegex(
                benchmark.DpoBenchmarkContractError, "outside the Gemma4 DPO matrix"
            ):
                benchmark.load_case(self.write_case(payload, Path(temp)))
    ...
    def test_input_and_output_leaf_symlinks_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            target = root / "target.safetensors"
            target.write_bytes(b"adapter")
            input_link = root / "input-link.safetensors"
            input_link.symlink_to(target)
            with self.assertRaisesRegex(
                benchmark.DpoBenchmarkContractError,
                "must not be a symlink",
            ):
                benchmark.require_regular_nonsymlink(input_link, "candidate")

            missing_target = root / "must-not-be-created.json"
            ...
            with self.assertRaisesRegex(
                ...
                benchmark.write_json_exclusive(output_link, {"run": 1})
