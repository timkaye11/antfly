...
def extract_sampling_source() -> str:
    source_path = Path(__file__).resolve().parents[2] / 'src/finetune/gemma4_real_autodiff.zig'
    source = source_path.read_text()
    sections = [
        source[source.index('pub const GrpoSamplingOptions = struct {'):source.index('/// Derive the independent PRNG stream used')],
        source[source.index('fn selectTopRankedTokens('):source.index('fn logProbAtToken(')],
    ]
    return '\n'.join(sections)
...
'''
    ...
    command = [str(args.zig), 'run', str(zig_path), '-O', 'ReleaseFast', '--global-cache-dir', '/tmp/antfly-zig-global-cache']
    ...
    fixture = {
        ...
        'policies': [{}, {'temperature': .8, 'top_k': 4}, {'temperature': 1.2, 'top_p': .7}, {'temperature': .6, 'top_p': .95, 'top_k': 7}, {'top_k': 1}],
        'row_fields': ['run_seed', 'domain', 'epoch', 'prompt', 'completion', 'group_seed', 'completion_seed', 'step', 'draw_bits', 'policy_tokens...'],
        'rows': rows,
    }
    ...
    with args.output.open('x') as handle:
        ...
        handle.write('\n  ]\n}\n')
    print(json.dumps({'rows': len(rows), 'token_selections': len(rows) * 5, 'output': str(args.output), 'sha256': hashlib.sha256(args.output.read_bytes()).hexdigest()}))
