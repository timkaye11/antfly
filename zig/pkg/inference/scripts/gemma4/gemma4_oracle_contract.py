...
BENCHMARK_PRODUCER_RELATIVE_PATHS = (
    ...
    "zig/pkg/inference/scripts/gemma4/gemma4_files.py",
...
def benchmark_precision_policy_sha256(precision: Mapping[str, Any]) -> str:
    ...
    return (
        "sha256:"
        + hashlib.sha256(BENCHMARK_PRECISION_POLICY_DOMAIN + encoded).hexdigest()
    )
...
def canonical_mlx_native_artifact_inventory_sha256(
    ...
    for index, raw in enumerate(artifacts):
        ...
        role = _require_string(
            artifact["role"], f"MLX native artifact inventory[{index}].role"
        )
        relative_text = _require_string(
            artifact["relative_path"],
            f"MLX native artifact inventory[{index}].relative_path",
        )
        ...
        if (
            ...
            raise ContractError(
                "MLX native artifact inventory paths must be normalized relative paths"
            )
        size = _require_int(
            artifact["size_bytes"],
            f"MLX native artifact inventory[{index}].size_bytes",
            minimum=1,
        )
        digest = _require_string(
            artifact["sha256"], f"MLX native artifact inventory[{index}].sha256"
        )
    ...
    if (
        ...
        raise ContractError(
            "MLX native artifact inventory has an unexpected runtime library name"
        )
    extension_name = names["python-extension"]
    if not extension_name.startswith("core.") or not extension_name.endswith(".so"):
        raise ContractError(
            "MLX native artifact inventory does not bind the loaded mlx.core extension"
        )
    encoded = json.dumps(
        normalized,
        sort_keys=True,
    ...
    return (
        "sha256:"
        + hashlib.sha256(MLX_NATIVE_ARTIFACT_INVENTORY_DOMAIN + encoded).hexdigest()
    )
...
def canonical_benchmark_producer_source_sha256(
    *,
    ...
    return (
        "sha256:"
        + hashlib.sha256(BENCHMARK_PRODUCER_SOURCE_DOMAIN + encoded).hexdigest()
    )
...
def validate_benchmark_producer_source(
    ...
    *,
    ...
    if expected_entrypoint is not None and relative_path != expected_entrypoint:
        raise ContractError(
            f"{where}: entrypoint differs from the admitted benchmark runner"
        )
    ...
    entrypoint_sha256 = _require_string(
        source["source_sha256"], f"{where}.source_sha256"
    )
    manifest_sha256 = _require_string(
        source["manifest_sha256"], f"{where}.manifest_sha256"
    )
    for field, value in (
    ...
    for index, raw_file in enumerate(raw_files):
        ...
        require_exact_keys(
            item, ("relative_path", "source_sha256"), where=f"{where}.files[{index}]"
        )
        path = _require_string(
            item["relative_path"], f"{where}.files[{index}].relative_path"
        )
        digest = _require_string(
            item["source_sha256"], f"{where}.files[{index}].source_sha256"
        )
        relative = Path(path)
        if (
            relative.is_absolute()
            ...
            raise ContractError(
                f"{where}: source paths must be normalized relative paths"
            )
        if re.fullmatch(r"sha256:[0-9a-f]{64}", digest) is None:
    ...
    if [item["relative_path"] for item in files] != expected_paths:
        raise ContractError(
            f"{where}: producer source inventory is incomplete, duplicated, or unsorted"
        )
    ...
    if len(entrypoints) != 1 or entrypoints[0]["source_sha256"] != entrypoint_sha256:
        raise ContractError(
            f"{where}: entrypoint digest differs from the closed source inventory"
        )
    ...
    *,
    ...
    def git(
        ...
        try:
            ...
            raise ContractError(
                f"could not attest benchmark producer source: {str(detail).strip()}"
            ) from exc
    ...
    try:
        ...
        raise ContractError(
            "benchmark producer entrypoint is outside its Git checkout"
        ) from exc
    if relative_entrypoint != expected_entrypoint:
        raise ContractError(
            "benchmark producer entrypoint differs from its admitted repository path"
        )
    ...
    if _HEX40.fullmatch(revision) is None or _HEX40.fullmatch(source_tree) is None:
        raise ContractError(
            "benchmark producer revision/tree is not a full Git object id"
        )
    status = git(root, "status", "--porcelain=v1", "--untracked-files=all").stdout
    if status:
        raise ContractError(
            "benchmark producer checkout must be completely clean, including untracked files"
        )
    ...
    for relative_path in BENCHMARK_PRODUCER_RELATIVE_PATHS:
        ...
        if path.is_symlink() or not path.is_file():
            raise ContractError(
                f"benchmark producer source must be a regular tracked file: {relative_path}"
            )
        ...
        if committed != actual:
            raise ContractError(
                f"benchmark producer source differs from commit {revision}: {relative_path}"
            )
        files.append(
            {
                "relative_path": relative_path,
    ...
    return validate_benchmark_producer_source(
...
def build_evidence_ledger(root: Path) -> dict[str, Any]:
    ...
    if publication_candidate.is_symlink():
        raise ContractError(
            f"oracle publication root cannot be a symlink: {publication_candidate}"
        )
    publication_root = publication_candidate.resolve()
    if not publication_root.is_dir():
        raise ContractError(
            f"oracle publication root is not a directory: {publication_root}"
        )
    complete = publication_root / "COMPLETE.json"
    if complete.exists():
        raise ContractError(
            "COMPLETE.json must be written only after the evidence ledger is built"
        )
    files: list[dict[str, Any]] = []
    for path in sorted(
        publication_root.rglob("*"),
        key=lambda item: item.relative_to(publication_root).as_posix(),
    ):
        if path.is_symlink():
        ...
        if not path.is_file():
            raise ContractError(
                f"oracle publication contains a non-regular file: {path}"
            )
...
def validate_evidence_ledger(trace_path: Path) -> tuple[str, dict[str, dict[str, Any]]]:
    ...
    require_exact_keys(
        marker, ("schema_version", "status", "files"), where="COMPLETE.json"
    )
    if (
        marker["schema_version"] != PUBLICATION_SCHEMA_VERSION
    ...
    for index, raw_entry in enumerate(
        _require_list(marker["files"], "COMPLETE.json.files")
    ):
        entry = _require_mapping(raw_entry, f"COMPLETE.json.files[{index}]")
        require_exact_keys(
            entry,
            ("path", "sha256", "size_bytes"),
            where=f"COMPLETE.json.files[{index}]",
        )
        relative_text = _require_string(
            entry["path"], f"COMPLETE.json.files[{index}].path"
        )
        relative = Path(relative_text)
        if (
            ...
            raise ContractError(
                f"COMPLETE.json.files[{index}].path must be a normalized safe relative path"
            )
        if relative_text == "COMPLETE.json" or relative_text in declared:
            raise ContractError(
                f"COMPLETE.json contains a duplicate or self-referential path: {relative_text}"
            )
        digest = _require_string(
            entry["sha256"], f"COMPLETE.json.files[{index}].sha256"
        )
        if re.fullmatch(r"sha256:[0-9a-f]{64}", digest) is None:
            raise ContractError(f"COMPLETE.json.files[{index}].sha256 is malformed")
        size_bytes = _require_int(
            entry["size_bytes"], f"COMPLETE.json.files[{index}].size_bytes", minimum=0
        )
        path = root / relative
        if path.is_symlink() or not path.is_file():
            raise ContractError(
                f"COMPLETE.json evidence file is missing or not regular: {relative_text}"
            )
        if path.stat().st_size != size_bytes or prefixed_sha256(path) != digest:
            raise ContractError(
                f"COMPLETE.json evidence digest/size mismatch: {relative_text}"
            )
...
def validate_lock(lock: Any) -> dict[str, Any]:
    ...
    if lock["schema_version"] != LOCK_SCHEMA_VERSION:
        raise ContractError(
            f"unsupported oracle lock schema: {lock['schema_version']!r}"
        )
    ...
    for model_key, raw_model in models.items():
        ...
        require_exact_keys(
            model,
            ("repo_id", "revision", "architecture", "files"),
            where=f"models.{model_key}",
        )
        if model["repo_id"] != expected_repositories[model_key]:
            raise ContractError(
                f"models.{model_key}.repo_id does not match the model key"
            )
        if not _HEX40.fullmatch(
            _require_string(model["revision"], f"models.{model_key}.revision")
        ):
        ...
        if set(files) != required_files:
            raise ContractError(
                f"models.{model_key}.files must pin exactly {sorted(required_files)}"
            )
        for name, raw_spec in files.items():
            ...
            if set(spec) not in ({"size", "sha256"}, {"size", "git_blob_sha1"}):
                raise ContractError(
                    f"models.{model_key}.files.{name}: require one exact digest"
                )
            _require_int(
                spec["size"], f"models.{model_key}.files.{name}.size", minimum=1
            )
            digest_key = "sha256" if "sha256" in spec else "git_blob_sha1"
            matcher = _HEX64 if digest_key == "sha256" else _HEX40
            if not matcher.fullmatch(
                _require_string(
                    spec[digest_key], f"models.{model_key}.files.{name}.{digest_key}"
                )
            ):
                raise ContractError(
                    f"models.{model_key}.files.{name}.{digest_key}: malformed digest"
                )
    ...
    if (
        re.fullmatch(
            r"[0-9]+\.[0-9]+",
            _require_string(python_oracle["python"], "python_oracle.python"),
        )
        is None
    ):
    ...
    if execution != expected_execution:
        raise ContractError(
            "python_oracle.execution differs from the deterministic BF16 lane"
        )
    ...
    if (
        re.fullmatch(
            r"[0-9]+\.[0-9]+",
            _require_string(mlx_reference["python"], "mlx_reference.python"),
        )
        is None
    ):
        raise ContractError("mlx_reference.python must pin a major.minor runtime")
    if (
        mlx_reference["required_platform"] != "Darwin"
        or mlx_reference["required_machine"] != "arm64"
    ):
        raise ContractError("MLX-LM reference hardware must be Darwin arm64")
    _require_int(
        mlx_reference["minimum_fresh_processes"],
        "mlx_reference.minimum_fresh_processes",
        minimum=5,
    )
    _require_int(mlx_reference["warmup_steps"], "mlx_reference.warmup_steps", minimum=1)
    _require_int(
        mlx_reference["measured_steps"], "mlx_reference.measured_steps", minimum=5
    )
    mlx_native_runtime = _require_mapping(
        mlx_reference["native_runtime"], "mlx_reference.native_runtime"
    )
    require_exact_keys(
        mlx_native_runtime,
        (
    ...
    if (
        mlx_native_runtime["build_attestation_schema_version"]
        != "antfly_mlx_native_build_attestation/v1"
    ):
        raise ContractError(
            "mlx_reference native build attestation schema is unsupported"
        )
    if (
        tuple(
            _require_list(
                mlx_native_runtime["artifact_roles"],
                "mlx_reference.native_runtime.artifact_roles",
            )
        )
        != MLX_NATIVE_ARTIFACT_ROLES
    ):
        raise ContractError(
            "mlx_reference native artifact roles are not the exact sorted runtime surface"
        )
    if mlx_native_runtime["minimum_macos_sdk_version"] != "26.2":
        raise ContractError(
            "mlx_reference native runtime must require the JACCL-capable macOS SDK"
        )
    if (
        re.fullmatch(
            r"sha256:[0-9a-f]{64}",
            _require_string(
                mlx_native_runtime["precision_policy_sha256"],
    ...
    for env_name in ("python_oracle", "mlx_reference"):
        ...
        if set(packages) != expected_packages:
            raise ContractError(
                f"{env_name}.packages must be exactly {sorted(expected_packages)}"
            )
        for package, version in packages.items():
            if not _VERSION.fullmatch(
                _require_string(version, f"{env_name}.packages.{package}")
            ):
                raise ContractError(
                    f"{env_name}.packages.{package}: version is not exact"
                )
        revisions = _require_mapping(
            env.get("source_revisions"), f"{env_name}.source_revisions"
        )
        expected_sources = {
        ...
        if set(revisions) != expected_sources:
            raise ContractError(
                f"{env_name}.source_revisions must be exactly {sorted(expected_sources)}"
            )
        for source, revision in revisions.items():
            if not _HEX40.fullmatch(
                _require_string(revision, f"{env_name}.source_revisions.{source}")
            ):
                raise ContractError(
                    f"{env_name}.source_revisions.{source}: expected full commit"
                )
    ...
    for name, raw_targets in presets.items():
        targets = [
            _require_string(item, f"target_presets.{name}[]")
            for item in _require_list(raw_targets, f"target_presets.{name}")
        ]
        if not targets or len(set(targets)) != len(targets):
            raise ContractError(
                f"target_presets.{name}: targets must be non-empty and unique"
            )
    ...
    for model_key, raw_model_inventory in target_inventory.items():
        model_inventory = _require_mapping(
            raw_model_inventory, f"target_inventory.{model_key}"
        )
        if set(model_inventory) != set(presets):
            raise ContractError(
                f"target_inventory.{model_key} must cover exactly the target presets"
            )
        for preset, raw_counts in model_inventory.items():
            counts = _require_mapping(
                raw_counts, f"target_inventory.{model_key}.{preset}"
            )
            ...
            for suffix, count in counts.items():
                _require_int(
                    count, f"target_inventory.{model_key}.{preset}.{suffix}", minimum=1
                )
    ...
    steps = [
        _require_int(item, "training_contract.steps[]", minimum=1)
        for item in _require_list(training_contract["steps"], "training_contract.steps")
    ]
    if steps != [1, 2, 8]:
        raise ContractError("training_contract.steps must be exactly [1,2,8]")
    if (
        _finite_float(
            training_contract["learning_rate"], "training_contract.learning_rate"
        )
        <= 0
    ):
        raise ContractError("training_contract.learning_rate must be positive")
    betas = [
        _finite_float(item, "training_contract.betas[]")
        for item in _require_list(training_contract["betas"], "training_contract.betas")
    ]
    ...
    benchmark_contract = _require_mapping(
        lock["benchmark_contract"], "benchmark_contract"
    )
    require_exact_keys(
        benchmark_contract,
        (
    ...
    if (
        benchmark_contract["schema_version"]
        != "antfly_gemma4_lora_benchmark_semantics/v3"
    ):
    ...
    for section, expected in expected_benchmark_contract.items():
        actual = _require_mapping(
            benchmark_contract[section], f"benchmark_contract.{section}"
        )
        if actual != expected:
            raise ContractError(
                f"benchmark_contract.{section} differs from the frozen same-Mac contract"
            )
    if mlx_native_runtime[
        "precision_policy_sha256"
    ] != benchmark_precision_policy_sha256(
        _require_mapping(
            benchmark_contract["precision"], "benchmark_contract.precision"
        )
    ):
        raise ContractError(
            "mlx_reference native runtime precision policy digest differs from benchmark_contract.precision"
        )

    tolerance_profiles = _require_mapping(
        lock["tolerance_profiles"], "tolerance_profiles"
    )
    tolerance_keys = {
        "loss_abs",
        "loss_rel",
    ...
    if set(tolerance_profiles) != {
        ...
        raise ContractError(
            "tolerance_profiles must contain exactly the four release lanes"
        )
    for name, raw_profile in tolerance_profiles.items():
        profile_value = _require_mapping(raw_profile, f"tolerance_profiles.{name}")
        require_exact_keys(
            profile_value, tolerance_keys, where=f"tolerance_profiles.{name}"
        )
        for key, value in profile_value.items():
            ...
            if key.endswith("cosine_min"):
                if not -1 <= number <= 1:
                    raise ContractError(
                        f"tolerance_profiles.{name}.{key}: expected [-1,1]"
                    )
            elif number < 0:
                raise ContractError(
                    f"tolerance_profiles.{name}.{key}: expected non-negative"
                )
    ...
    require_exact_keys(
        performance,
        (
            "minimum_geomean_throughput_ratio",
            "minimum_cell_throughput_ratio",
    ...
    for field in ("primary_sequence_lengths", "gradient_accumulation"):
        values = [
            _require_int(item, f"performance_gate.{field}[]", minimum=1)
            for item in _require_list(performance[field], f"performance_gate.{field}")
        ]
        if not values or len(values) != len(set(values)):
            raise ContractError(
                f"performance_gate.{field} must be non-empty and unique"
            )
...
def verify_requirements_match_lock(
    lock: Mapping[str, Any], environment: str, path: Path
) -> dict[str, str]:
    expected = dict(_require_mapping(lock[environment], environment)["packages"])
    actual = parse_requirements(path)
    if actual != expected:
        raise ContractError(
            f"{path.name} does not exactly match {environment}.packages"
        )
    return actual
...
def verify_requirements_match_lock_subset(
    ...
    if missing:
        raise ContractError(
            f"requirements subset is absent from {environment}.packages: {missing}"
        )
    expected = {
...
def verify_python_runtime(lock: Mapping[str, Any]) -> str:
    expected = _require_string(
        _require_mapping(lock["python_oracle"], "python_oracle")["python"],
        "python_oracle.python",
    )
...
def verify_packages(
    ...
    *,
    ...
    if mismatches:
        raise ContractError(
            f"{environment} dependency mismatch: " + ", ".join(mismatches)
        )
    return actual


def verify_source_checkout(
    path: Path, expected_revision: str, *, source_name: str
) -> dict[str, str]:
    ...
    if revision != expected_revision:
        raise ContractError(
            f"{source_name} revision {revision or '<unknown>'} != pinned {expected_revision}"
        )
...
def verify_import_source(imported: Any, checkout: Path, *, source_name: str) -> str:
    ...
    if not imported_from.is_relative_to(checkout):
        raise ContractError(
            f"{source_name} imported from unpinned source: {imported_from}"
        )
...
def verify_model_directory(
    ...
    for relative_path, raw_spec in sorted(
        _require_mapping(model["files"], f"models.{model_key}.files").items()
    ):
        ...
        if digest != spec[digest_kind]:
            raise ContractError(
                f"{path}: {digest_kind} {digest} != pinned {spec[digest_kind]}"
            )
        ...
        aggregate.update(
            digest_kind.encode("ascii") + b":" + digest.encode("ascii") + b"\0"
        )
    return {
        "model_key": model_key,
    ...
    for index, item in enumerate(_require_list(value, where)):
        ...
        if labels:
            if number < 0 and number != IGNORE_LABEL:
                raise ContractError(
                    f"{where}[{index}]: only {IGNORE_LABEL} may be negative"
                )
        elif number < 0:
...
def _fingerprint_prepared_examples(
    examples: Sequence[Mapping[str, Any]], domain: str
) -> str:
    ...
    for index, example in enumerate(examples):
        ...
        for field in ("prompt_input_ids", "response_input_ids", "input_ids"):
            ...
            if not isinstance(values, list) or any(
                ...
                raise ContractError(
                    f"examples[{index}].{field}: expected integer array"
                )
        ...
        for field in (
            ...
            _zig_hash_bytes(
                hasher,
                _optional_string(example.get(field), f"examples[{index}].{field}"),
            )
        media_hashes = example.get("media_content_sha256", [])
        if not isinstance(media_hashes, list) or any(
            not isinstance(value, str) for value in media_hashes
        ):
            raise ContractError(
                f"examples[{index}].media_content_sha256: expected string array"
            )
        ...
        for field in ("image_token_counts", "audio_token_counts"):
            ...
            if not isinstance(values, list) or any(
                ...
                raise ContractError(
                    f"examples[{index}].{field}: expected non-negative integer array"
                )
            _zig_hash_usizes(hasher, values)
        labels = example.get("labels", [])
        if not isinstance(labels, list) or any(
            isinstance(value, bool) or not isinstance(value, int) for value in labels
        ):
        ...
        for field in (
            ...
            _zig_hash_length(
                hasher,
                _require_int(
                    example.get(field, 0), f"examples[{index}].{field}", minimum=0
                ),
            )
        ...
        if not isinstance(teacher_ids, list) or any(
            isinstance(value, bool) or not isinstance(value, int)
            for value in teacher_ids
        ):
            raise ContractError(
                f"examples[{index}].teacher_top_k_token_ids: expected integer array"
            )
        _zig_hash_ints(hasher, teacher_ids)
        teacher_probs = example.get("teacher_top_k_probs", [])
        if not isinstance(teacher_probs, list):
            raise ContractError(
                f"examples[{index}].teacher_top_k_probs: expected number array"
            )
        _zig_hash_f32s(
            hasher,
            [
                _finite_float(value, f"examples[{index}].teacher_top_k_probs")
                for value in teacher_probs
            ],
        )
        _zig_hash_length(
            hasher,
            _require_int(
                example.get("teacher_top_k", 0),
                f"examples[{index}].teacher_top_k",
                minimum=0,
            ),
        )
        _zig_hash_f32s(
            ...
            [
                _finite_float(
                    example.get("teacher_temperature", 1.0),
                    f"examples[{index}].teacher_temperature",
                )
            ],
        )
        ...
        _zig_hash_length(
            hasher,
            _require_int(
                example.get("turns_dropped_from_left", 0),
                f"examples[{index}].turns_dropped_from_left",
                minimum=0,
            ),
        )
        _zig_hash_bytes(
            hasher,
            _optional_string(
                example.get("policy_version"), f"examples[{index}].policy_version"
            ),
        )
    return hasher.hexdigest()
...
def resolve_dataset_jsonl_files(path: Path, split: str) -> list[Path]:
    ...
    if manifest_path.is_file():
        ...
        for index, raw_entry in enumerate(files):
            ...
            relative = Path(
                _require_string(
                    ...
                    f"dataset manifest.files[{index}].relative_path",
                )
            )
        ...
        result = sorted(
            (
                ...
                and item.name.startswith(split + "-")
            ),
            key=str,
        )
    else:
    ...
    if not result or any(not item.is_file() for item in result):
        raise ContractError(
            f"source dataset has no complete JSONL files for split {split!r}"
        )
    return result
...
def verify_prepared_source_dataset(
    summary: Mapping[str, Any], path: Path | None = None
) -> dict[str, str]:
    split = _require_string(
        summary.get("source_split"), "prepared.summary.source_split"
    )
    source_path = (
        path
        if path is not None
            ...
            _require_string(
                summary.get("source_dataset_path"),
                "prepared.summary.source_dataset_path",
            )
        )
    )
    actual = fingerprint_dataset_source(source_path, split)
    expected = _require_string(
        summary.get("source_dataset_sha256"), "prepared.summary.source_dataset_sha256"
    )
    if actual != expected:
        raise ContractError(
            f"prepared source dataset SHA-256 {actual} != recorded {expected}"
        )
    return {
...
def load_prepared_example(
    ...
    if summary.get("schema_version") != PREPARED_SCHEMA_VERSION:
        raise ContractError(
            f"prepared.summary.schema_version must be {PREPARED_SCHEMA_VERSION}"
        )
    ...
    if _require_int(
        summary.get("examples_seen"), "prepared.summary.examples_seen", minimum=1
    ) != len(examples):
        raise ContractError("prepared examples_seen does not match examples length")
    if (
        summary.get("examples_with_images", 0) != 0
        or summary.get("examples_with_audio", 0) != 0
    ):
        raise ContractError(
            "Gemma4 correctness oracle admits text-only prepared artifacts"
        )
    ...
    _require_string(
        summary.get("source_dataset_path"), "prepared.summary.source_dataset_path"
    )
    source_split = _require_string(
        summary.get("source_split"), "prepared.summary.source_split"
    )
    source_revision = _require_string(
        summary.get("source_revision"), "prepared.summary.source_revision"
    )
    ...
    for index, raw_example in enumerate(examples):
        example = dict(
            _require_mapping(raw_example, f"prepared.summary.examples[{index}]")
        )
        input_ids = _validate_int_array(
            example.get("input_ids"), f"examples[{index}].input_ids"
        )
        labels = _validate_int_array(
            example.get("labels"), f"examples[{index}].labels", labels=True
        )
        if len(input_ids) != len(labels):
            raise ContractError(
                f"examples[{index}]: input_ids and labels lengths differ"
            )
        if _require_int(
            example.get("num_input_tokens"), f"examples[{index}].num_input_tokens"
        ) != len(input_ids):
        ...
        if supervised == 0 or not any(
            labels[position] != IGNORE_LABEL for position in range(1, len(labels))
        ):
            raise ContractError(f"examples[{index}]: no causal supervised tokens")
        if (
            _require_int(
                example.get("num_supervised_tokens"),
                f"examples[{index}].num_supervised_tokens",
            )
            != supervised
        ):
            raise ContractError(
                f"examples[{index}]: num_supervised_tokens is inconsistent"
            )
        for field in (
            "image_paths",
            "audio_paths",
            ...
            if example.get(field, []) != []:
                raise ContractError(
                    f"examples[{index}].{field}: multimodal data is not admitted"
                )
        if example.get("media_content_sha256", []) != []:
            raise ContractError(
                f"examples[{index}].media_content_sha256: text-only examples require an empty array"
            )
        source_id = _require_string(
            example.get("source_id"), f"examples[{index}].source_id"
        )
        source_group_id = _require_string(
            example.get("source_group_id"), f"examples[{index}].source_group_id"
        )
        source_name = _require_string(
            example.get("source_name"), f"examples[{index}].source_name"
        )
        record_digest = _require_string(
            example.get("source_record_sha256"),
            f"examples[{index}].source_record_sha256",
        )
        rendered_digest = _require_string(
            example.get("rendered_chat_sha256"),
            f"examples[{index}].rendered_chat_sha256",
        )
        if not _HEX64.fullmatch(record_digest) or not _HEX64.fullmatch(rendered_digest):
            raise ContractError(
                f"examples[{index}]: source/rendered digest is malformed"
            )
    ...
    max_seq_len = _require_int(
        summary.get("max_seq_len"), "prepared.summary.max_seq_len", minimum=1
    )
    if max_input > max_seq_len:
        raise ContractError("prepared example exceeds max_seq_len")
    if (
        summary.get("max_input_tokens") != max_input
        or summary.get("max_supervised_tokens") != max_supervised
    ):
    ...
    if actual_examples_fingerprint != fingerprint:
        raise ContractError(
            "prepared_examples_sha256 does not match the v6 example payload"
        )
...
def canonical_adapter_inventory(names: Iterable[str]) -> list[str]:
    inventory = [
        f"{module}:{role}"
        for module, role in (canonicalize_adapter_tensor_name(name) for name in names)
    ]
...
def validate_target_inventory(
    ...
    try:
        ...
        raise ContractError(
            f"no locked target inventory for {model_key}/{target_preset}"
        ) from exc
    expected = {
...
class TensorStore:
    ...
    def __init__(self, trace_path: Path, descriptor: Mapping[str, Any]):
        ...
        require_exact_keys(
            self.descriptor,
            ("format", "entries")
            if self.descriptor.get("format") == "inline-f64/v1"
            else ("format", "path", "sha256", "entries"),
            where="tensor_store",
        )
        ...
        raw_entries = _require_mapping(
            self.descriptor["entries"], "tensor_store.entries"
        )
        if not raw_entries:
        ...
        for logical_name, raw_entry in raw_entries.items():
            ...
            expected = (
                ...
                else ("shape", "dtype", "storage_key")
            )
            require_exact_keys(
                entry, expected, where=f"tensor_store.entries.{logical_name}"
            )
            shape = tuple(
                _require_int(dim, f"{logical_name}.shape", minimum=1)
                for dim in _require_list(entry["shape"], f"{logical_name}.shape")
            )
            if not shape:
                raise ContractError(
                    f"{logical_name}: scalar tensors are not part of the LoRA trace contract"
                )
            dtype = _require_string(entry["dtype"], f"{logical_name}.dtype")
            if self.format == "inline-f64/v1":
                if dtype != "float64":
                    raise ContractError(
                        f"{logical_name}: inline-f64/v1 requires dtype=float64"
                    )
                values = tuple(
                    _finite_float(item, f"{logical_name}.values")
                    for item in _require_list(entry["values"], f"{logical_name}.values")
                )
                if len(values) != _shape_product(shape):
                    raise ContractError(
                        f"{logical_name}: inline value count does not match shape"
                    )
                entries[logical_name] = TensorEntry(shape, dtype, None, values)
            else:
                if dtype != "float32":
                    raise ContractError(
                        f"{logical_name}: v1 release state tensors require dtype=float32"
                    )
                storage_key = _require_string(
                    entry["storage_key"], f"{logical_name}.storage_key"
                )
                if storage_key in storage_keys:
                    raise ContractError(
                        f"Safetensors storage key is reused: {storage_key}"
                    )
        ...
        if self.format == "safetensors/v1":
            relative_text = _require_string(
                self.descriptor["path"], "tensor_store.path"
            )
            relative = Path(relative_text)
            if (
                relative_text != relative.as_posix()
                ...
                raise ContractError(
                    "tensor_store.path must be a normalized safe relative path"
                )
            ...
            expected_sha = _require_string(
                self.descriptor["sha256"], "tensor_store.sha256"
            )
    ...
    def get(self, logical_name: str) -> tuple[tuple[int, ...], list[float]]:
        try:
            ...
            raise ContractError(
                f"tensor store is missing logical tensor {logical_name}"
            ) from exc
        ...
        try:
            ...
            raise ContractError(
                "safetensors is required to read an external trace tensor store"
            ) from exc
        ...
        if actual_shape != entry.shape:
            raise ContractError(
                f"{logical_name}: Safetensors shape {actual_shape} != manifest {entry.shape}"
            )
...
def validate_trace(
    ...
    *,
    ...
    require_exact_keys(
        producer, ("name", "version", "source_revision", "hardware"), where="producer"
    )
    if producer["name"] not in (
        "hf-peft",
        "antfly-zig-native",
        "antfly-zig-metal",
        "synthetic-test",
    ...
    if producer["name"] == "synthetic-test":
        if not allow_synthetic:
            raise ContractError(
                "synthetic-test traces are accepted only by explicit test code"
            )
    ...
    if producer["name"] == "hf-peft":
        ...
        _require_string(
            producer_hardware.get("cuda_device"), "producer.hardware.cuda_device"
        )
        _require_string(
            producer_hardware.get("cuda_runtime"), "producer.hardware.cuda_runtime"
        )
    elif producer["name"] in ("antfly-zig-native", "antfly-zig-metal"):
        if _HEX40.fullmatch(producer["source_revision"]) is None:
            raise ContractError(
                "Antfly trace source_revision must be a full clean Git commit"
            )
        expected_backend = (
            "native" if producer["name"] == "antfly-zig-native" else "metal"
        )
        ...
        if (
            producer_hardware.get("platform") != "Darwin"
            or producer_hardware.get("machine") != "arm64"
        ):
            raise ContractError(
                "Antfly Gemma4 release traces require Darwin arm64 hardware identity"
            )
        for field in ("os_version", "os_build", "chip"):
            _require_string(producer_hardware.get(field), f"producer.hardware.{field}")
        _require_int(
            producer_hardware.get("memory_bytes"),
            "producer.hardware.memory_bytes",
            minimum=1,
        )
        if producer["name"] == "antfly-zig-metal":
            _require_string(
                producer_hardware.get("metal_device"), "producer.hardware.metal_device"
            )

    model = _require_mapping(payload["model"], "model")
    require_exact_keys(
        model, ("key", "repo_id", "revision", "local_artifact_sha256"), where="model"
    )
    ...
    if (
        model["repo_id"] != locked_model["repo_id"]
        or model["revision"] != locked_model["revision"]
    ):
        raise ContractError("trace model identity does not match the lock")
    if not isinstance(model["local_artifact_sha256"], str) or not re.fullmatch(
        r"sha256:[0-9a-f]{64}", model["local_artifact_sha256"]
    ):
    ...
    if not isinstance(prepared["artifact_sha256"], str) or not re.fullmatch(
        r"sha256:[0-9a-f]{64}", prepared["artifact_sha256"]
    ):
    ...
    for field in (
        ...
        if not isinstance(prepared[field], str) or not _HEX64.fullmatch(
            prepared[field]
        ):
    ...
    supervised_positions = [
        index for index in range(1, len(labels)) if labels[index] != IGNORE_LABEL
    ]
    ...
    if alpha <= 0 or not math.isclose(
        scale, alpha / rank, rel_tol=1e-12, abs_tol=1e-12
    ):
    ...
    if len(betas) != 2 or not all(
        0 <= _finite_float(beta, "training.beta") < 1 for beta in betas
    ):
    ...
    if (
        ...
        or training["supervised_token_normalization"] != "mean"
    ):
        raise ContractError(
            "v1 numerical traces require one-step supervised-token mean semantics"
        )
    ...
    if (
        ...
        or [float(value) for value in training["betas"]]
        != [float(value) for value in trajectory["betas"]]
        ...
        or training["supervised_token_normalization"]
        != trajectory["supervised_token_normalization"]
    ):
    ...
    require_exact_keys(
        metrics,
        ("loss", "loss_history", "grad_norm", "supervised_tokens"),
        where="metrics",
    )
    _finite_float(metrics["loss"], "metrics.loss")
    history = [
        _finite_float(value, "metrics.loss_history[]")
        for value in _require_list(metrics["loss_history"], "metrics.loss_history")
    ]
    if len(history) != training["step"] or not math.isclose(
        history[-1], float(metrics["loss"]), rel_tol=0, abs_tol=0
    ):
        raise ContractError(
            "metrics.loss_history must contain one exact entry per step and end at loss"
        )
    ...
    for index, raw_probe in enumerate(
        _require_list(payload["logit_probes"], "logit_probes")
    ):
        ...
        require_exact_keys(
            ...
            where=f"logit_probes[{index}]",
        )
        predictor = _require_int(
            probe["predictor_position"],
            f"logit_probes[{index}].predictor_position",
            minimum=0,
        )
        ...
        if predictor + 1 >= len(labels) or labels[predictor + 1] == IGNORE_LABEL:
            raise ContractError(
                "logit probe does not correspond to a supervised causal target"
            )
        if probe["target_token_id"] != labels[predictor + 1]:
            raise ContractError("logit probe target does not match prepared labels")
        token_ids = _validate_int_array(
            probe["token_ids"], f"logit_probes[{index}].token_ids"
        )
        if len(set(token_ids)) != len(token_ids):
            raise ContractError("logit probe token ids must be unique")
        values = [
            _finite_float(value, f"logit_probes[{index}].values")
            for value in _require_list(probe["values"], f"logit_probes[{index}].values")
        ]
    ...
    if probe_positions != {position - 1 for position in supervised_positions}:
        raise ContractError(
            "trace must probe every supervised causal position exactly once"
        )

    tensor_store_descriptor = _require_mapping(payload["tensor_store"], "tensor_store")
    expected_store_format = (
        "inline-f64/v1" if producer["name"] == "synthetic-test" else "safetensors/v1"
    )
    ...
    for index, raw_target in enumerate(target_tensors):
        ...
        require_exact_keys(
            ...
            where=f"target_tensors[{index}]",
        )
        canonical = canonicalize_module_name(
            _require_string(
                target["canonical_name"], f"target_tensors[{index}].canonical_name"
            )
        )
        role = target["role"]
        if role not in ("lora_A", "lora_B"):
            raise ContractError(f"target_tensors[{index}].role is unsupported")
        source_identity = canonicalize_adapter_tensor_name(
            _require_string(
                target["source_name"], f"target_tensors[{index}].source_name"
            )
        )
        if source_identity != (canonical, role):
            raise ContractError(
                f"target_tensors[{index}] source/canonical names disagree"
            )
        ...
        shape = tuple(
            _require_int(dim, f"target_tensors[{index}].shape", minimum=1)
            for dim in _require_list(target["shape"], f"target_tensors[{index}].shape")
        )
        if (
            len(shape) != 2
            or (role == "lora_A" and shape[0] != rank)
            or (role == "lora_B" and shape[1] != rank)
        ):
            raise ContractError(
                f"target_tensors[{index}] shape is incompatible with rank {rank}"
            )
        if target["gradient_expectation"] not in (
            "active",
            "zero-by-zero-b-initialization",
        ):
            raise ContractError("unsupported gradient expectation")
        logical = _require_mapping(
            target["logical_tensors"], f"target_tensors[{index}].logical_tensors"
        )
        require_exact_keys(
            logical,
            ("initial", "gradient", "updated", "optimizer_m", "optimizer_v"),
            where=f"target_tensors[{index}].logical_tensors",
        )
        gradient: list[float] | None = None
        for state_name, logical_name in logical.items():
            logical_name = _require_string(
                logical_name, f"target_tensors[{index}].logical_tensors.{state_name}"
            )
            ...
            if stored_shape != shape:
                raise ContractError(
                    f"{logical_name}: shape does not match target manifest"
                )
            if any(not math.isfinite(value) for value in values):
        ...
        if target["gradient_expectation"] == "active" and gradient_norm == 0:
            raise ContractError(
                f"active target has an all-zero gradient: {canonical}:{role}"
            )
        if (
            target["gradient_expectation"] == "zero-by-zero-b-initialization"
            and gradient_norm != 0
        ):
            raise ContractError(
                f"expected-zero target has a nonzero gradient: {canonical}:{role}"
            )
    if any(roles != {"lora_A", "lora_B"} for roles in modules.values()):
        raise ContractError(
            "every target module must contain exactly one LoRA A/B pair"
        )
    recomputed_grad_norm = math.sqrt(global_gradient_sq)
    if not math.isclose(
        claimed_grad_norm, recomputed_grad_norm, rel_tol=1e-9, abs_tol=1e-12
    ):
        raise ContractError(
    ...
    semantics = _require_mapping(
        artifact["adapter_config_semantics"], "artifact.adapter_config_semantics"
    )
    require_exact_keys(
        semantics,
        (
        ...
        where="artifact.adapter_config_semantics",
    )
    ...
    configured_modules = sorted(
        canonicalize_module_name(item)
        for item in _require_list(
            semantics["target_modules"], "artifact.target_modules"
        )
    )
    if configured_modules != sorted(modules):
        raise ContractError(
            "adapter config target modules do not exactly match the tensor inventory"
        )
    ...
    if not isinstance(artifact["adapter_model_sha256"], str) or not re.fullmatch(
        r"sha256:[0-9a-f]{64}", artifact["adapter_model_sha256"]
    ):
        raise ContractError("artifact.adapter_model_sha256 is malformed")
    artifact_inventory = _require_list(
        artifact["tensor_inventory"], "artifact.tensor_inventory"
    )
    if artifact_inventory != sorted(inventory) or len(artifact_inventory) != len(
        set(artifact_inventory)
    ):
...
def vector_metrics(
    ...
    diff_sq = sum(
        (float(right) - float(left)) ** 2 for left, right in zip(reference, candidate)
    )
    ...
    max_abs = max(
        abs(float(right) - float(left)) for left, right in zip(reference, candidate)
    )
...
def scalar_within(
    reference: float, candidate: float, *, abs_tol: float, rel_tol: float
) -> tuple[bool, float]:
...
def compare_traces(
    ...
    if profile_name == "native-metal-bf16":
        ...
        ref_identity = tuple(
            ref_producer["hardware"].get(field) for field in identity_fields
        )
        cand_identity = tuple(
            cand_producer["hardware"].get(field) for field in identity_fields
        )
        if ref_identity != cand_identity:
            raise ContractError(
                "native-metal-bf16 requires byte-equivalent host hardware identity"
            )
        ...
        if (
            ...
            raise ContractError(
                "resume comparison requires the exact same binary and hardware identity"
            )

    failures: list[str] = []
    comparisons: dict[str, Any] = {}
    contract_equal = _comparison_contract(reference.payload) == _comparison_contract(
        candidate.payload
    )
    ...
    comparisons["grad_norm"] = {
        ...
        "absolute_delta": abs(
            candidate.recomputed_grad_norm - reference.recomputed_grad_norm
        ),
    ...
    if len(ref_metrics["loss_history"]) != len(cand_metrics["loss_history"]):
        ...
        for index, (left, right) in enumerate(
            zip(ref_metrics["loss_history"], cand_metrics["loss_history"])
        ):
            ok, bound = scalar_within(
                float(left),
                float(right),
                abs_tol=float(profile["loss_abs"]),
                rel_tol=float(profile["loss_rel"]),
    ...
    ref_probes = {
        row["predictor_position"]: row for row in reference.payload["logit_probes"]
    }
    cand_probes = {
        row["predictor_position"]: row for row in candidate.payload["logit_probes"]
    }
    ...
    for position in sorted(set(ref_probes) & set(cand_probes)):
        ...
        if (
            ...
            or left["target_token_id"] != right["target_token_id"]
        ):
            failures.append(
                f"logit probe token contract differs at position {position}"
            )
            continue
        metrics = vector_metrics(left["values"], right["values"])
        lse_ok, lse_bound = scalar_within(
            float(left["logsumexp"]),
            float(right["logsumexp"]),
            abs_tol=float(profile["loss_abs"]),
            rel_tol=float(profile["loss_rel"]),
        )
        ok = (
        ...
        probe_rows.append(
            {
                ...
                "logsumexp_absolute_delta": abs(
                    float(right["logsumexp"]) - float(left["logsumexp"])
                ),
    ...
    def targets(trace: ValidatedTrace) -> dict[tuple[str, str], Mapping[str, Any]]:
        return {
            ...
            for row in trace.payload["target_tensors"]
        }
    ...
    if set(ref_targets) != set(cand_targets):
        ...
        failures.append(
            f"target tensor inventory differs (missing={missing}, extra={extra})"
        )
    target_rows = []
    for identity in sorted(set(ref_targets) & set(cand_targets)):
        ...
        if (
            ...
            or left["gradient_expectation"] != right["gradient_expectation"]
        ):
        ...
        for state_name in (
            ...
            _, right_values = candidate.tensors.get(
                right["logical_tensors"][state_name]
            )
            ...
            if not ok:
                failures.append(
                    f"{identity[0]}:{identity[1]} {state_name} exceeded tolerance"
                )
    ...
    artifact_semantics_equal = (
        ref_artifact["adapter_config_semantics"]
        == cand_artifact["adapter_config_semantics"]
    ...
    comparisons["artifact_semantics"] = {
        ...
        "byte_hashes_equal": ref_artifact["adapter_model_sha256"]
        == cand_artifact["adapter_model_sha256"],
        "reference_key_layout": ref_artifact["key_layout"],
        "candidate_key_layout": cand_artifact["key_layout"],
        "canonical_name_normalization_applied": ref_artifact["key_layout"]
        != cand_artifact["key_layout"],
...
def _read_antfly_adapter_manifest(
    ...
    *,
    ...
    if schema_version == ANTFLY_ADAPTER_MANIFEST_SCHEMA_V3:
        require_exact_keys(
            manifest,
            fields + ("initialization_seed",),
            where="antfly_finetune_manifest",
        )
        _require_int(
            manifest["initialization_seed"], "manifest.initialization_seed", minimum=0
        )
    else:
    ...
    if (
        ...
        or manifest["artifact_family_version"] != "gemma4_lora/v1alpha1"
    ):
        raise ContractError(
            "Antfly adapter manifest is not a complete Gemma4 LoRA artifact"
        )
    _require_string(
        manifest["base_model_name_or_path"], "manifest.base_model_name_or_path"
    )
    for field in (
        "base_model_sha256",
        "tokenizer_sha256",
        ...
        if not isinstance(manifest[field], str) or not _HEX64.fullmatch(
            manifest[field]
        ):
            raise ContractError(f"manifest.{field} is missing or malformed")
    adapter_checkpoint = (
        adapter_dir.expanduser().resolve() / "adapter_model.safetensors"
    )
    ...
    if adapter_checkpoint.stat().st_size != checkpoint_size:
        raise ContractError(
            "Antfly adapter checkpoint size does not match its manifest"
        )
    if sha256_file(adapter_checkpoint) != manifest["adapter_checkpoint_sha256"]:
        raise ContractError(
            "Antfly adapter checkpoint SHA-256 does not match its manifest"
        )
    targets = [
        canonicalize_module_name(item)
        for item in _require_list(manifest["target_modules"], "manifest.target_modules")
    ]
...
def _read_antfly_peft_export_manifest(adapter_dir: Path) -> dict[str, Any] | None:
    ...
    fields = (
        "schema_version",
        "status",
        "source_artifact_family_version",
        "source_tensor_key_format",
        "destination_tensor_key_format",
        "source_adapter_model_sha256",
        "destination_adapter_model_sha256",
        "destination_adapter_model_size_bytes",
    ...
    if (
        manifest["status"] != "complete"
        or manifest["source_artifact_family_version"] != "gemma4_lora/v1alpha1"
    ):
    ...
    if manifest["destination_tensor_key_format"] != STOCK_PEFT_KEY_FORMAT:
        raise ContractError(
            "Antfly PEFT export destination tensor-key format is invalid"
        )
    _require_string(
        manifest["base_model_name_or_path"], "peft_export.base_model_name_or_path"
    )
    for field in (
        "source_adapter_model_sha256",
        "destination_adapter_model_sha256",
    ...
    if checkpoint.stat().st_size != checkpoint_size:
        raise ContractError(
            "Antfly PEFT export checkpoint size does not match its manifest"
        )
    if sha256_file(checkpoint) != manifest["destination_adapter_model_sha256"]:
        raise ContractError(
            "Antfly PEFT export checkpoint SHA-256 does not match its manifest"
        )
    if sha256_file(config) != manifest["adapter_config_sha256"]:
        raise ContractError(
            "Antfly PEFT export config SHA-256 does not match its manifest"
        )
    return manifest
...
def read_adapter_config(
    ...
    *,
    ...
    dropout = _finite_float(
        config.get("lora_dropout", 0.0), "adapter_config.lora_dropout"
    )
    if dropout != 0.0:
    ...
    targets = [
        canonicalize_module_name(item)
        for item in _require_list(
            config["target_modules"], "adapter_config.target_modules"
        )
    ]
    if not targets or len(targets) != len(set(targets)):
    ...
    if manifest is not None and export_manifest is not None:
        raise ContractError(
            "adapter cannot contain both internal and PEFT export manifests"
        )
    if manifest is not None:
        if target_preset is not None and target_preset != manifest["target_preset"]:
            raise ContractError(
                "explicit target preset conflicts with Antfly adapter manifest"
            )
        ...
        policy_source = (
            f"antfly-finetune-manifest/{manifest['schema_version'].rsplit('/', 1)[-1]}"
        )
        provenance = {
            ...
            "manifest_sha256": prefixed_sha256(
                adapter_dir.expanduser().resolve() / "antfly_finetune_manifest.json"
            ),
            "initialization_seed": manifest.get("initialization_seed"),
            "target_preset_attestation": manifest.get(
                "target_preset_attestation", "manifest"
            ),
        }
    elif export_manifest is not None:
        if (
            target_preset is not None
            and target_preset != export_manifest["target_preset"]
        ):
            raise ContractError(
                "explicit target preset conflicts with Antfly PEFT export manifest"
            )
        if (
            config.get("base_model_name_or_path")
            != export_manifest["base_model_name_or_path"]
        ):
        ...
        provenance = {
            ...
            "adapter_checkpoint_sha256": export_manifest[
                "destination_adapter_model_sha256"
            ],
            "adapter_checkpoint_size_bytes": export_manifest[
                "destination_adapter_model_size_bytes"
            ],
            "source_adapter_checkpoint_sha256": export_manifest[
                "source_adapter_model_sha256"
            ],
            "tensor_count": export_manifest["tensor_count"],
            "manifest_sha256": prefixed_sha256(
                adapter_dir.expanduser().resolve() / "antfly_peft_export.json"
            ),
        }
    else:
...
def stock_peft_module_name(name: str) -> str:
    ...
    if canonicalize_module_name(name) != canonicalize_module_name(result):
        raise ContractError(
            "stock PEFT module translation changed the canonical identity"
        )
    return result
...
def inspect_adapter_artifact(
    adapter_dir: Path, *, target_preset: str | None = None
) -> dict[str, Any]:
    root = adapter_dir.expanduser().resolve()
    ...
    try:
        ...
        raise ContractError(
            "safetensors is required to inspect an adapter artifact"
        ) from exc
    ...
    if has_internal_manifest and key_layout != ANTFLY_ADAPTER_KEY_FORMAT:
        raise ContractError(
            "Antfly manifest declares internal keys but checkpoint uses stock PEFT keys"
        )
    if not has_internal_manifest and key_layout != STOCK_PEFT_KEY_FORMAT:
        raise ContractError(
            "Antfly internal keys require an antfly_finetune_manifest.json sidecar"
        )
    if (
        config["policy_source"] == "antfly-peft-export/v1"
        and len(keys) != config["provenance"]["tensor_count"]
    ):
        raise ContractError(
            "Antfly PEFT export tensor count does not match its manifest"
        )
    configured_targets = semantics["target_modules"]
    if has_internal_manifest:
        if sorted(modules) != configured_targets:
            raise ContractError(
                "Antfly adapter config targets do not exactly match tensor modules"
            )
    else:
        unmatched_modules = [
            module
