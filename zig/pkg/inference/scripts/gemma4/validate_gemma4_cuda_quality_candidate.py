...
def collect_input_provenance(
    ...
    files = {
        ...
        "tuning_profile": stable_file_provenance(
            inference / "scripts/gemma4/gemma4_qat_cuda_tuning.sh"
        ),
        "candidate_catalog": stable_file_provenance(
            inference / "scripts/gemma4/validate_gemma4_cuda_candidate.py"
        ),
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--suite",
        type=pathlib.Path,
        default=inference / "scripts/gemma4/fixtures/gemma4_cuda_quality_suite_v1.json",
    )
    parser.add_argument(
        "--threshold-profile",
        default=None,
