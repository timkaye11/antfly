...
class SamplingTests(unittest.TestCase):
    def test_rollout_preserves_fixture_streams_across_tokens_and_prompt_order(self):
        fixture = json.loads(
            (Path(__file__).parent / "fixtures/grpo_sampling_zig_0_16.json").read_text()
        )
        ...
        for run, epoch, prompt in sorted(keys, reverse=True):
            rows = [
                ...
                if r[:4] == [run, TRAIN_DOMAIN, epoch, prompt]
            ]
            ...
            for policy_index, options in enumerate(fixture["policies"]):
                ...
                sequences, _ = categorical_rollout_group(
                    ...
                    prompt_index=prompt,
                    evaluation=False,
                    policy=SamplingPolicy(**options),
                    group_size=max(completions) + 1,
                    max_completion_tokens=steps,
                    eos_token_id=len(logits) - 1,
                )
                expected = [
                    [r[9 + policy_index] for r in rows if r[4] == i]
    ...
    def test_rollout_scores_tempered_policy_and_evaluation_zero_is_greedy(self):
        logits = [0.0, 2.0, -4.0]
        sequences, logps = categorical_rollout_group(
            lambda prefix: logits,
            [8],
            run_seed=17,
            epoch=0,
            prompt_index=9,
            evaluation=True,
            policy=SamplingPolicy(temperature=3.0, top_k=1),
            group_size=4,
            max_completion_tokens=3,
            eos_token_id=2,
        )
        expected = 2.0 / 3.0 - math.log(sum(math.exp(v / 3.0) for v in logits))
        ...
        for values in logps:
            for value in values:
                ...
                self.assertLess(
                    value, 0.0
                )  # Filtering to one token must not score it as log(1).
    ...
    def test_fixture_matches_production_source(self):
        ...
        self.assertEqual(
            hashlib.sha256(extract_sampling_source().encode()).hexdigest(),
            fixture["extracted_source_sha256"],
            "Production sampler changed; regenerate the executable Zig fixture",
        )
    ...
    def test_executable_zig_fixture(self):
        ...
        for row in fixture["rows"]:
            run, domain, epoch, prompt, completion, group, seed, step, bits, *tokens = (
                row
            )
            ...
            self.assertEqual(
                [p.select(fixture["logits"], draw) for p in policies], tokens
            )
    ...
    def test_rare_exponent_draws(self):
        ...
        with patch.object(rng, "next_u64", side_effect=[3, 0, 1 << 63]) as draw:
            value = rng.uniform()
            expected = struct.unpack("<d", struct.pack("<Q", ((1022 - 76) << 52) | 3))[
                0
            ]
    ...
    def test_uniform_cdf_preserves_vocabulary_order_and_strict_boundary(self):
        ...
        self.assertEqual(
            [
                policy.select([0, 0, 0, 0], x)
                for x in [0, 0.25, 0.5, 0.75, math.nextafter(1, 0)]
            ],
            [0, 1, 2, 3, 3],
        )
    ...
    def test_nucleus_retains_crossing_token_and_top_k_precedes_top_p(self):
        self.assertEqual(SamplingPolicy(top_p=0.5).select([0, 0, 0], 0.99), 1)
        self.assertEqual(SamplingPolicy(top_k=2, top_p=0.5).select([0, 0, 0], 0.99), 0)
        self.assertEqual(SamplingPolicy(top_k=100).select([0, 0], 0.99), 1)

    def test_ties_use_lowest_id_after_f32_rounding(self):
        self.assertEqual(ranked_tokens([1, 1 + 1e-9, 1], 3), [0, 1, 2])
        self.assertEqual(SamplingPolicy(top_k=1).select([2, 2, 1], 0.99), 0)

    def test_extreme_finite_logits(self):
        for draw in (0, 0.5, math.nextafter(1, 0)):
    ...
    def test_invalid_inputs_fail_closed(self):
        ...
        for draw in (-0.1, 1, math.inf, math.nan):
            with self.assertRaises(ValueError):
        ...
        for seed in (-1, MASK64 + 1, True, 0.5):
            with self.assertRaises(ValueError):
                ZigRandom(seed)
    ...
    def test_streams_use_original_prompt_identity(self):
        ...
        self.assertNotEqual(
            group_seed(42, TRAIN_DOMAIN, 0, 0), group_seed(42, EVAL_DOMAIN, 0, 0)
        )
    ...
    def test_evaluation_first_completion_is_always_greedy(self):
        ...
        with patch.object(
            sampler.random,
            "uniform",
            side_effect=AssertionError("greedy must not draw"),
        ):
    ...
    def test_eos_in_one_completion_cannot_perturb_siblings(self):
        ...
        self.assertEqual(
            [sibling.select([0] * 32) for _ in range(16)],
            [isolated.select([0] * 32) for _ in range(16)],
        )
