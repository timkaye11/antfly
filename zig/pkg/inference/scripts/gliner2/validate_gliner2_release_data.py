...
def production_accelerator_step_metrics(
    ...
    try:
        ...
        raise ValueError(
            f"release adapter has no retained {backend_label} training metrics: {metrics_path}: {exc}"
        ) from exc
    ...
    for line_no, raw in enumerate(lines, 1):
        ...
        try:
            ...
            raise ValueError(
                f"release adapter has invalid retained {backend_label} training metrics at line {line_no}: {exc}"
            ) from exc
        if not isinstance(row, dict):
            raise ValueError(
                f"release adapter training metric line {line_no} must be an object"
            )
    ...
    for index, row in enumerate(steps, 1):
        ...
        if row.get("step") != index or row.get("optimizer_backend") != backend:
            raise ValueError(
                f"release adapter step {index} was not executed by the {backend_label} optimizer"
            )
        if any(
        ...
        if any(
            ...
            raise ValueError(
                f"release adapter step {index} lacks compiled device-resident dispatch evidence"
            )
        ...
        if "graph_executor_fallback_reason" not in row or row.get(
            "graph_executor_fallback_reason"
        ) not in (None, ""):
            raise ValueError(
                f"release adapter step {index} reports a graph-executor fallback reason"
            )
        if backend == "cuda":
            cuda_integer_fields = (
                "cuda_h2d_bytes",
            ...
            if any(
                type(row.get(name)) is not int or row[name] < 0
                for name in cuda_integer_fields
            ):
                raise ValueError(
                    f"release adapter step {index} lacks complete CUDA runtime telemetry"
                )
            if (
                ...
                raise ValueError(
                    f"release adapter step {index} does not account for full-step CUDA input H2D"
                )
            if (
                ...
                raise ValueError(
                    f"release adapter step {index} has disallowed CUDA transfer or synchronization telemetry"
                )
            ...
            if any(
                ...
                raise ValueError(
                    f"release adapter step {index} lacks required CUDA fused-kernel coverage"
                )
    if backend == "cuda" and cuda_upload_synchronization_total > 1:
        raise ValueError(
            "release adapter has more than one cold-start CUDA upload synchronization"
        )
    return steps
...
def validate_release_artifact_provenance(
    ...
    if not train_data.is_file():
        raise ValueError(
            "release adapter provenance requires --train to be one JSONL file"
        )
    manifest = json_object(
        adapter_dir / "training_manifest.json", "Zig training manifest"
    )
    adapter_config = json_object(
        adapter_dir / "adapter_config.json", "PEFT adapter config"
    )
    ...
    if (
        manifest.get("objective") != FULL_TASK_OBJECTIVE
        or manifest.get("lora_only_trainables") is not True
    ):
        raise ValueError(
            "release adapter was not produced by LoRA-only full-task training"
        )
    if (
        str(manifest.get("backend", "")).lower() != backend
        or manifest.get("compiled_required") is not True
    ):
        raise ValueError(
            f"release adapter provenance requires {backend.upper()} training with compiled_required=true"
        )
    if (
        ...
        raise ValueError(
            "release adapter provenance requires FP32 training and optimizer state"
        )
    ...
    if (
        ...
        raise ValueError(
            "release adapter has invalid production training-policy provenance"
        )
    ...
    if (
        ...
        or cache_stats["graph_cache_resident_signatures"]
        > cache_stats["graph_cache_peak_resident_signatures"]
        or cache_stats["graph_cache_peak_resident_signatures"] > cache_capacity
        or cache_stats["graph_cache_resident_signatures"]
        > cache_stats["graph_cache_builds"]
        or cache_stats["graph_cache_evictions"] > cache_stats["graph_cache_builds"]
    ):
        raise ValueError(
            "release adapter has invalid production graph-shape/cache provenance"
        )
    if (
        ...
        raise ValueError(
            "release adapter provenance requires clean base-model initialization"
        )
    ...
    if resume_used:
        ...
        expected_resume_path = f"checkpoints/resume-source-{resume_sha256.removeprefix('sha256:')}.safetensors"
        if resume_path != expected_resume_path:
            raise ValueError(
                "release adapter has invalid retained resume-checkpoint path"
            )
        ...
        if (
            ...
            raise ValueError(
                "release adapter resume-checkpoint fingerprint does not match its manifest"
            )
    elif any(
        value is not None
        for value in (
    ...
    resolved_eval_data = (
        eval_data if eval_data is not None else Path(manifest_eval_data)
    )
    if not resolved_eval_data.is_file():
        raise ValueError(
            "release adapter provenance requires --eval to be one JSONL file"
        )
    ...
    if (
        ...
        raise ValueError(
            "release adapter has invalid held-out model-selection provenance"
        )
    ...
    if (
        ...
        or sha256_file(best_checkpoint) != best_checkpoint_sha256
    ):
        raise ValueError(
            "release adapter best-checkpoint fingerprint does not match its manifest"
        )
    max_examples = manifest.get("max_examples")
    if type(max_examples) is not int or max_examples != 0:
        raise ValueError(
            "release adapter provenance requires training over the complete dataset"
        )
    ...
    if (
        ...
        raise ValueError(
            "release adapter provenance requires uncapped training with max_steps=0"
        )
    ...
    if not all(
        ...
        raise ValueError(
            "release adapter manifest has invalid epoch or optimizer-step accounting"
        )
    actual_record_count = (
        train_record_count
        if train_record_count is not None
        else load_dataset(train_data).record_count
    )
    if example_count != actual_record_count:
        raise ValueError(
            "release adapter manifest example count does not match the training JSONL"
        )
    if examples_per_epoch != example_count:
        raise ValueError(
            "release adapter training does not consume every training record"
        )
    if (
    ...
    if (
        ...
        raise ValueError(
            "release adapter manifest has inconsistent completed-run accounting"
        )
    if (
        ...
        raise ValueError(
            "release adapter has inconsistent held-out model-selection accounting"
        )
    if early_stopped:
        if (
            ...
            raise ValueError(
                "release adapter has inconsistent early-stopping provenance"
            )
    elif epochs != requested_epochs or (
        early_stopping_patience > 0 and bad_epochs >= early_stopping_patience
    ):
        raise ValueError(
            "release adapter provenance requires completion or a valid early stop"
        )
    if resume_used and (
    ...
    if manifest.get("train_data_sha256") != expected_train:
        raise ValueError(
            "release adapter training-data fingerprint does not match --train"
        )
    expected_model = base_model_fingerprint(model_dir)
    if manifest.get("base_model_fingerprint_sha256") != expected_model:
        raise ValueError(
            "release adapter base-model fingerprint does not match --model-dir"
        )
    ...
    if manifest.get("adapter_bundle_fingerprint_sha256") != expected_adapter:
        raise ValueError(
            "release adapter bundle fingerprint does not match its Zig training manifest"
        )
    manifest_model = manifest.get("model_dir")
    if (
        not isinstance(manifest_model, str)
        or adapter_config.get("base_model_name_or_path") != manifest_model
    ):
        raise ValueError(
            "release adapter config and Zig training manifest disagree on the base model"
        )
    config_rank, manifest_rank = adapter_config.get("r"), manifest.get("lora_rank")
    config_alpha, manifest_alpha = (
        adapter_config.get("lora_alpha"),
        manifest.get("lora_alpha"),
    )
    config_dropout, manifest_dropout = (
        adapter_config.get("lora_dropout"),
        manifest.get("lora_dropout"),
    )
    config_targets, manifest_targets = (
        adapter_config.get("target_modules"),
        manifest.get("resolved_lora_targets"),
    )
    ...
    if (
        ...
        or any(
            ...
            for value in config_targets + manifest_targets
        )
        or config_targets != manifest_targets
    ):
        raise ValueError(
            "release adapter config and Zig training manifest disagree on LoRA metadata"
        )
...
def record_tasks(record: dict[str, Any]) -> set[str]:
    ...
    if legacy_entities is not None:
        ...
        if any(
            isinstance(item, dict) and has_value(item.get("text"))
    ...
    if isinstance(output, dict):
        ...
        if rows is not None:
            ...
            if any(
                isinstance(row, dict)
                and any(has_value(value) for value in row.values())
                for row in rows
            ):
...
def load_dataset(path: Path, max_records: int | None = None) -> DatasetContent:
    ...
    return DatasetContent(
        record_count, has_repeated_records, frozenset(texts), frozenset(tasks)
    )
...
def validate_release_data(
    ...
    *,
    ...
    if train.record_count < min_train_records:
        raise ValueError(
            f"training data has {train.record_count} records; require at least {min_train_records}"
        )
    ...
    if eval_ is not None and eval_.record_count < min_eval_records:
        raise ValueError(
            f"eval data has {eval_.record_count} records; require at least {min_eval_records}"
        )
    ...
    if len(train.texts) != train.record_count:
        raise ValueError(
            "training data contains repeated/cycled normalized input texts"
        )
    ...
    if train.texts & smoke_texts or (eval_ is not None and eval_.texts & smoke_texts):
        raise ValueError(
            "training or eval data contains checked-in smoke fixture content"
        )
    if require_full_task:
        ...
        if missing:
            raise ValueError(
                f"compared training slice is missing non-empty full-task coverage: {','.join(sorted(missing))}"
            )
...
def main() -> int:
    ...
    try:
        ...
        comparison_train = (
            load_dataset(args.train, args.comparison_records)
        ...
        if (args.model_dir is None) != (args.release_adapter_dir is None):
            raise ValueError(
                "--model-dir and --release-adapter-dir must be supplied together"
            )
