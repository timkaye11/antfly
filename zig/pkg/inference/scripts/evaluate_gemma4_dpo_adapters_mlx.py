...
RESULT_SCHEMA_VERSION = "antfly_gemma4_dpo_mlx_adapter_evaluation/v1"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    candidate_paths = [candidate.path.expanduser().resolve() for candidate in args.candidate]
    ...
    mlx_revision = dpo.require_source_revision(
        args.mlx_source_root, revisions["mlx"], "MLX"
    )
    mlx_lm_revision = dpo.require_source_revision(
    ...
    try:
        ...
        for candidate, candidate_path in zip(args.candidate, candidate_paths):
            if candidate_path.is_symlink() or not candidate_path.is_file():
                raise dpo.DpoBenchmarkContractError(
                    f"candidate {candidate.label} is not a regular non-symlink file"
                )
