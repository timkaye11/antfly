...
PREFILL_CHUNK_SIZE="${ANTFLY_GEMMA4_MTP_LONG_CONTEXT_PREFILL_CHUNK_SIZE:-64}"
...
# larger defaults can hit the 2,048-token encoder cap before the final question.
...
for value in "$TOKENS" "$PREFILL_CHUNK_SIZE" "$PROMPT_REPEAT" "$SPECULATIVE_K"; do
  case "$value" in
    ''|*[!0-9]*|0)
      echo "tokens, prefill chunk size, and prompt repeat must be positive integers" >&2
      exit 2
      ;;
  esac
done
...
if (( ring_gate_active )); then
  ...
  for ring_out in "$target_out" "$mtp_out"; do
    grep -Eq '^kv-write: .*layer=0 .*ring_pages=[1-9][0-9]*$' "$ring_out" \
      || fail "sliding layer did not use the SWA ring in $ring_out"
    grep -Eq '^kv-write: .*layer=5 .*ring_pages=0$' "$ring_out" \
...
grep -q '^gen_debug: executePrefill captured final logits and MTP hidden ' "$mtp_out" \
  || fail "long MTP prefill did not capture final logits and hidden state"
if grep -q '^gen_debug: executePrefill metal_prepared_tail_greedy ' "$mtp_out"; then
  fail "long MTP prefill incorrectly used the whole-prefill prepared tail"
fi

echo "metal Gemma4 MTP long-context token gate passed"
echo "prompt_tokens=$target_prompt_tokens prefill_chunk_size=$target_chunk output_tokens=$TOKENS"
