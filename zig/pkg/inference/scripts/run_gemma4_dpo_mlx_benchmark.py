...
RESULT_SCHEMA_VERSION = "antfly_gemma4_dpo_mlx_benchmark/v1"
...
def require_source_revision(root: Path, expected: str, label: str) -> str:
    try:
        checkout = locked.verify_source_checkout(
            root,
            expected,
            source_name=label,
        )
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if (
        args.first_update_adapter_output is not None
        and args.adapter_output is not None
        and args.first_update_adapter_output.expanduser().resolve()
        == args.adapter_output.expanduser().resolve()
    ):
        raise DpoBenchmarkContractError(
    ...
    if args.first_update_adapter_output is not None:
        destination = args.first_update_adapter_output.expanduser().resolve()
    ...
    if args.adapter_output is not None:
        ...
        adapter_output_path = str(destination)
    ...
    if args.evaluation_adapter is not None:
        comparison_path = args.evaluation_adapter.expanduser().resolve()
        if comparison_path.is_symlink() or not comparison_path.is_file():
            raise DpoBenchmarkContractError(
                "comparison adapter must be a regular non-symlink file"
            )
