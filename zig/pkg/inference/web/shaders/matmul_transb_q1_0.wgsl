...
// Tiled matrix multiplication with B transposed and Q1_0 dequantization.
// C[M,N] = A[M,K] @ dequant(B_q1_0)^T
//
// Q1_0 layout: blocks of 128 values, 18 bytes per block:
//   [0:2]  d   - f16 scale
//   [2:18] qs  - 16 bytes; 128 sign bits (LSB-first within each byte)
// Decoded value: (bit ? +d : -d).
...
fn dequant_q1_0(b_row: u32, k_abs: u32) -> f32 {
    let blocks_per_row = params.K / 128u;
    let block_idx = k_abs / 128u;
    let in_block = k_abs % 128u;
    let block_byte = (b_row * blocks_per_row + block_idx) * 18u;

    let d = read_f16(block_byte);
    let byte_val = read_byte(block_byte + 2u + (in_block / 8u));
    let bit = (byte_val >> (in_block % 8u)) & 1u;
    return select(-d, d, bit != 0u);
}

@compute @workgroup_size(TILE_M, TILE_N)
fn matmul_transb_q1_0(
    ...
    for (var t: u32 = 0u; t < num_tiles; t++) {
        ...
        for (var stage: u32 = 0u; stage < 2u; stage++) {
            ...
            if (g_col < params.N && g_k < params.K) {
                tile_b[idx] = dequant_q1_0(g_col, g_k);
            } else {
                tile_b[idx] = 0.0;
            }
        }
