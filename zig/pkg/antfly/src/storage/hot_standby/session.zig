...
test "storage.hot_standby session replicates primary records to standby and updates slot progress" {
...
test "storage.hot_standby session reports durable receive progress when apply fails" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby session records progress update failures" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby session resumes after primary and standby reopen" {
    const alloc = std.testing.allocator;
