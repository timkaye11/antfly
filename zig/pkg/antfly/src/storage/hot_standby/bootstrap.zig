...
test "storage.hot_standby bootstrap verifies manifest files and starts standby at checkpoint" {
...
test "storage.hot_standby bootstrap rejects bad copied files and wrong identities before changing standby" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby bootstrap catches up from primary backup end record" {
    const alloc = std.testing.allocator;
