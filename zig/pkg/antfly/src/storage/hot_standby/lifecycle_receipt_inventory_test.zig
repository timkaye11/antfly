...
test "storage.hot_standby lifecycle ledger is durable idempotent and survives generation gc" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby lifecycle ledger serializes concurrent publication and rejects identity mutation" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    const root = try tmp.dir.realPathFileAlloc(std.testing.io, ".", alloc);
...
test "storage.hot_standby lifecycle ledger reports bounded cursor truncation and collection gaps" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    const root = try tmp.dir.realPathFileAlloc(std.testing.io, ".", alloc);
...
test "storage.hot_standby lifecycle ledger fails closed on a torn final record" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    const root = try tmp.dir.realPathFileAlloc(std.testing.io, ".", alloc);
...
test "storage.hot_standby lifecycle receipt route is authenticated read only fenced visible and restart durable" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    const root = try tmp.dir.realPathFileAlloc(std.testing.io, ".", alloc);
...
test "storage.hot_standby lifecycle receipt route rejects ambiguous or malformed cursor queries" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    const root = try tmp.dir.realPathFileAlloc(std.testing.io, ".", alloc);
