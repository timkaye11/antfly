...
test "storage.hot_standby commit gate acknowledges async and satisfied remote write" {
...
test "storage.hot_standby commit gate waits rejects or degrades by failure policy" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby commit gate fail closed append rejects before local wal" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby commit gate ignores paused or reseed-required slots" {
    const alloc = std.testing.allocator;
