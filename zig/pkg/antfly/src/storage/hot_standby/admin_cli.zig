...
test "storage.hot_standby admin cli parses slot lifecycle commands" {
...
test "storage.hot_standby admin cli parses standby seed commands" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby admin cli rejects padded and invalid HA strings" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby admin cli parses primary status with sync policy" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby admin cli treats ALL sync policy as all named standbys" {
    const alloc = std.testing.allocator;
    var plan = try parse(alloc, &.{
        "status",           "primary",
        "--sync-mode",      "remote-apply",
...
test "storage.hot_standby admin cli parses operator plan command" {
    const alloc = std.testing.allocator;
    var plan = try parse(alloc, &.{
        "operator",                             "plan",
...
test "storage.hot_standby admin cli renders versioned json command plan" {
    const alloc = std.testing.allocator;
    var plan = try parse(alloc, &.{
        "--prometheus",
        "status",
...
test "storage.hot_standby admin cli parses stream ack commit and read checks" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby admin cli parses fenced promotion request" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby admin cli parses standby upstream and follow commands" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby admin cli parses former primary rejoin assessment" {
    const alloc = std.testing.allocator;
