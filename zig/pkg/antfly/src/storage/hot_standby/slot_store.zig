...
test "storage.hot_standby slot store persists slot progress across reopen" {
...
test "storage.hot_standby slot store tracks transient replication error until progress" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby slot store decodes v1 slot events without last error" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby slot store decodes v2 slot events with safe reads at applied lsn" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby slot store migrates v3 slot events to streaming lifecycle" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby slot store rejects invalid slot state on replay" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby slot store computes retention floor from active slots" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby slot store marks slots for reseed when lag cap is exceeded" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby slot store marks active slots at restart lsn for timeline" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby slot store drops slots and releases retention" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby slot store persists pause and resume state" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby slot store allows backup pin ahead of standby progress" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby slot store persists inactive seeding retention until explicit activation" {
    const alloc = std.testing.allocator;
