...
test "storage.hot_standby replication api identifies primary and creates restartable slots" {
...
test "storage.hot_standby replication api rejects invalid slot identifiers" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby replication api starts replication with record and byte batching" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby replication api pauses resumes and drops slots" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby replication api persists standby status updates" {
    const alloc = std.testing.allocator;
