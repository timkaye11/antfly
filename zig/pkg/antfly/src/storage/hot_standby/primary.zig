...
test "storage.hot_standby primary rejects invalid open identity" {
...
test "storage.hot_standby primary allows whole instance shard table identity" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary appends streams and persists standby status" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary opens from promoted standby handoff and continues writes" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary adoption serializes standby ownership transfer" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary rejects duplicate slot creation without regressing progress" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary rejects future slot creation at storage boundary" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary rejects invalid slot and sync standby identifiers" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary begins base backup with slot retention pin" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary recovers reservation durable before backup start" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary advances orphaned reservation after intervening WAL append" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary retention cap can expire an inactive seeding slot" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary marks oldest slots when retained byte cap is exceeded" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary marks oldest slots when retained age cap is exceeded" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary rejects base backup over healthy existing slot" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary allows base backup to reset reseed-required slot" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary escapes backup start json payload fields" {
    const alloc = std.testing.allocator;
    const identity = testIdentity();
...
test "storage.hot_standby primary ends base backup with decodable manifest payload" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary rejects backup end from wrong identity or missing start" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary requires matching backup start record before backup end" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary requires backup slot retention before backup end" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary evaluates async remote write and remote apply policies" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary any policy reports nth standby progress" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary first priority policy waits for the first eligible standby" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary rejects impossible named synchronous quorum policies" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary refuses streaming from reseed or expired slots" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary rejects replication start beyond next lsn" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary scopes retention to the current timeline" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary rejects status updates for old timeline slots" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary rejects status updates for inactive or reseed slots" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby primary pauses and resumes slot streaming across reopen" {
    const alloc = std.testing.allocator;
