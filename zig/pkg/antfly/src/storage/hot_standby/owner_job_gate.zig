...
test "storage.hot_standby owner job gate runs owner jobs only on primary role" {
...
test "storage.hot_standby owner job gate disables standby jobs until promoted-primary handoff" {
    const alloc = std.testing.allocator;
