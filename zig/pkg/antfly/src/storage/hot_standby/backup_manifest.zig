...
test "storage.hot_standby backup manifest round trips backup bounds and files" {
...
test "storage.hot_standby backup manifest validates copied file contents" {
    const alloc = std.testing.allocator;
    const files = [_]FileEntry{
...
test "storage.hot_standby backup manifest rejects unsafe paths and invalid wal bounds" {
    const files = [_]FileEntry{
...
test "storage.hot_standby backup manifest accepts only canonical portable relative paths" {
    for ([_][]const u8{
        "catalog/manifest",
        "tables/1/sst-0001",
        "catalog/file..name",
...
test "storage.hot_standby backup manifest detects body and header corruption" {
    const alloc = std.testing.allocator;
    const files = [_]FileEntry{
        .{ .path = "manifest", .kind = .manifest, .size_bytes = 5, .crc32 = crc32("hello") },
    };
