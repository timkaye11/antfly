...
test "storage.hot_standby write gate allows current primary writes" {
...
test "storage.hot_standby write gate rejects primary writes after matching promotion fence" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby write gate rejects standby until promotion handoff is opened" {
    const alloc = std.testing.allocator;
