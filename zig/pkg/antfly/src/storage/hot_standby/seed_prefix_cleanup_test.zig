...
test "storage.hot_standby seed prefix cleanup purges every version and delete marker before receipt" {
...
test "storage.hot_standby seed prefix cleanup deletes the exact instance prefix and emits a bound receipt" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby seed prefix cleanup fails closed on mutated authority and is idempotent when empty" {
    const alloc = std.testing.allocator;
    var memory = object_storage.MemoryObjectStorage.init(alloc);
    defer memory.deinit();
    var client = memory.client();
...
test "storage.hot_standby seed cleanup excludes publishers and leaves a durable tombstone" {
    const alloc = std.testing.allocator;
    var memory = object_storage.MemoryObjectStorage.init(alloc);
    defer memory.deinit();
    var client = memory.client();
...
test "storage.hot_standby seed namespace hands active authority to a newer same-topology generation" {
    const alloc = std.testing.allocator;
    var memory = object_storage.MemoryObjectStorage.init(alloc);
    defer memory.deinit();
    var client = memory.client();
