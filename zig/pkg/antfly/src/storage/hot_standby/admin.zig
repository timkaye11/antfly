...
test "storage.hot_standby admin manages slot lifecycle and status" {
...
test "storage.hot_standby admin seeds standby from base backup workflow" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby admin exposes commit and read freshness decisions" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby admin acquires fence and promotes standby" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby admin rejects mismatched fence identity for promotion" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby admin assesses former primary rejoin workflow" {
    const identity = testIdentity();
