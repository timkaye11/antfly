...
test "storage.hot_standby read gate serves stale reads at safe read lsn" {
...
test "storage.hot_standby read gate waits for read after write freshness" {
...
test "storage.hot_standby read gate caps stale reads at metadata applied lsn" {
    const decision = try evaluateProgress(.{
        .received_lsn = 9,
        .applied_lsn = 8,
        .safe_read_lsn = 8,
...
test "storage.hot_standby read gate waits when metadata lags required snapshot" {
    const decision = try evaluateProgress(.{
        .received_lsn = 9,
        .applied_lsn = 8,
        .safe_read_lsn = 8,
...
test "storage.hot_standby read gate permits lower metadata requirement" {
    const decision = try evaluateProgress(.{
        .received_lsn = 9,
        .applied_lsn = 8,
        .safe_read_lsn = 8,
...
test "storage.hot_standby read gate routes primary consistency away from standby" {
    const decision = try evaluateProgress(.{
        .received_lsn = 10,
        .applied_lsn = 9,
        .safe_read_lsn = 9,
...
test "storage.hot_standby read gate rejects invalid progress or missing freshness target" {
