...
test "storage.hot_standby internal typed operation bypasses legacy request dispatch" {
...
test "storage.hot_standby internal http adapter sheds requests while state transition is busy" {
...
test "storage.hot_standby internal http adapter handles every generated HA replication route method" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby internal http adapter serves the legacy replication prefix as an alias" {
    const alloc = std.testing.allocator;
    var server = Server.init(alloc, null);
...
test "storage.hot_standby internal http adapter returns method errors for generated HA replication routes" {
    const alloc = std.testing.allocator;
    var server = Server.init(alloc, null);
...
test "storage.hot_standby internal http adapter serves replication pull and status update" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby internal http adapter reports seeding slot as a lifecycle conflict" {
    const alloc = std.testing.allocator;
