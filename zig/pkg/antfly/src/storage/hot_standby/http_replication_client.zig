...
test "storage.hot_standby http replication client rejects invalid local inputs before execution" {
...
test "storage.hot_standby http replication client releases transferred request URIs once on status errors" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http replication client falls back to the legacy route prefix once" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http replication client stays on canonical routes when the upstream serves them" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http replication client pulls applies and acknowledges standby progress" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http replication client drains bounded apply debt before receiving more" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http replication client replicates mixed tables for whole instance identity" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http replication client verifies upstream identity before streaming" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http replication client rejects frame metadata mismatch before receive" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http replication client rejects record identity mismatch before partial receive" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http replication client rejects previous lsn mismatch before partial receive" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http replication client rejects slot mismatch before receive" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http replication client rejects premature end-of-wal before receive" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http replication client verifies typed standby status acknowledgement" {
...
test "storage.hot_standby http replication client reports durable receive progress when apply fails" {
    const alloc = std.testing.allocator;
