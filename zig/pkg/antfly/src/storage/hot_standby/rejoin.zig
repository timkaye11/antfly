...
test "storage.hot_standby rejoin rejects former primary without a fence" {
...
test "storage.hot_standby rejoin rewinds compatible fenced former primary when WAL is retained" {
    const former = FormerPrimaryState{
        .node_id = "primary-a",
        .identity = parentIdentity(),
        .last_lsn = 10,
...
test "storage.hot_standby rejoin reseeds when timeline is incompatible or WAL expired" {
    const receipt = promotedReceipt();
...
test "storage.hot_standby rejoin treats current timeline as already joined" {
    const receipt = promotedReceipt();
...
test "storage.hot_standby rejoin requires explicit policy for forced-promotion rewind" {
...
test "storage.hot_standby rejoin reseeds a divergent former primary instead of hiding applied writes" {
    var receipt = promotedReceipt();
...
test "storage.hot_standby rejoin rewind installs the promoted timeline boundary" {
...
test "storage.hot_standby rejoin rewind defensively rejects a divergent log suffix" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby rejoin rewind rejects stale or non-rewind assessments" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby rejoin rewind requires retained fork record" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby rejoin rewind rejects fork record identity mismatch" {
    const alloc = std.testing.allocator;
