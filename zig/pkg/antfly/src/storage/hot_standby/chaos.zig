...
test "storage.hot_standby chaos crash during base backup preserves slot pin and catch-up boundary" {
...
test "storage.hot_standby chaos crash after receive replays durable WAL before streaming resumes" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby chaos rejects noncontiguous records and follows timeline switch across restart" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby chaos rejects out-of-order WAL without poisoning receive cursor" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby chaos crash during apply preserves remote write and blocks remote apply" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby chaos crash after apply before ack reports durable progress on resume" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby chaos primary restart preserves synchronous acknowledgement boundaries" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby chaos lag retention forces reseed and former primary cannot rewind expired WAL" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby chaos network partition requires fence before standby promotion" {
    const alloc = std.testing.allocator;
