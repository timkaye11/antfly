...
FULL_TASK_PARITY_FIXTURES = [
    ...
    pytest.param("gliner2_negative_smoke.jsonl", 1, True, id="negative"),
]
...
def test_gliner2_lora_all_task_multi_step_roundtrip(tmp_path: Path):
    ...
    Tolerances (lr=1e-3, steps=3, measured on the optimizer-parity runs):
      ...
      * per-step loss: 3e-3.  Step-2 loss is ~34.6 (count/structure loss on
        the all-task fixture); after schema-active negative supervision, the
        measured cross-implementation delta is ~2.8e-3 while optimizer parity
        still shows matching step counts and lr-scaled adapter drift. The
        1-step 1e-4 gate is kept only for the strict first-batch test above.
    """
