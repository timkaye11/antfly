...
def arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--segment-mode", choices=("single", "production"), default="single"
    )
    ...
    parser.add_argument(
        "--antfly-manifest", type=Path, help="manifest for --reuse-index"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--comparator-manifest",
        type=Path,
        help="manifest for a reused comparator index",
    )
    ...
    if (
        args.merge_max_segments_per_tier is not None
        and args.merge_max_segments_per_tier < 2
    ):
    ...
    if args.postings_chunk_size is not None and (
        args.postings_chunk_size < 64
        or args.postings_chunk_size & (args.postings_chunk_size - 1)
    ):
        parser.error("--postings-chunk-size must be a power of two of at least 64")
    if args.comparator_index_command and not (
        args.comparator_index_dir and args.comparator_query_command
    ):
        parser.error(
            "comparator indexing requires --comparator-index-dir and --comparator-query-command"
        )
    ...
    if (
        args.comparator_query_command
        and not args.comparator_index_command
        and not args.comparator_manifest
    ):
        parser.error("a reused comparator requires --comparator-manifest")
    if args.comparator_query_command and (
        args.merge_max_segments_per_tier is not None
        or args.merge_max_at_once is not None
    ):
        parser.error(
            "merge-policy overrides are Antfly-only engineering runs and cannot be used with a comparator"
        )
    return args
...
def assert_compatible_manifests(left: dict[str, Any], right: dict[str, Any]) -> None:
    for key_path in (
        ...
        if left_value != right_value:
            raise RuntimeError(
                f"index manifest mismatch at {'.'.join(key_path)}: {left_value!r} != {right_value!r}"
            )
...
def assert_settled_antfly_layout(manifest: dict[str, Any]) -> None:
    ...
    if manifest.get("segment_mode") == "single" and len(segments) != 1:
        raise RuntimeError(
            f"single segment mode requires exactly one segment, found {len(segments)}"
        )
    ...
    debt = {
        field: merge_stats.get(field)
        for field in debt_fields
        if merge_stats.get(field, 0) != 0
    }
    if debt:
        raise RuntimeError(f"Antfly index is not settled; merge debt remains: {debt}")
    if merge_stats.get("failed_merges", 0) != 0:
        raise RuntimeError(
            f"Antfly index recorded failed merges: {merge_stats['failed_merges']}"
        )
...
def parse_ps_cpu_time(raw: str) -> int:
    ...
    total_seconds = (
        days * 86_400 + int(hours) * 3_600 + int(minutes) * 60 + float(seconds)
    )
...
def percentile(values: list[int], fraction: float) -> int:
    ...
    return ordered[
        min(len(ordered) - 1, max(0, int((len(ordered) - 1) * fraction + 0.999999)))
    ]
...
def verify_analyzer(
    adapter: Adapter, fixtures: list[dict[str, Any]]
) -> list[dict[str, Any]]:
...
def main() -> int:
    ...
    index_argv = command(
        args.antfly_index_command,
        [str(bin_dir / "search_benchmark_index"), str(args.index_dir)],
        **values,
    )
    index_argv += [
        "--segment-mode",
    ...
    query_argv = command(
        args.antfly_query_command,
        [str(bin_dir / "search_benchmark_query"), str(args.index_dir)],
        **values,
    )
    query_argv += ["--bm25-k1", str(args.bm25_k1), "--bm25-b", str(args.bm25_b)]
    if args.merge_max_segments_per_tier is not None:
        merge_args = [
            "--merge-max-segments-per-tier",
            str(args.merge_max_segments_per_tier),
        ]
    ...
    if not args.reuse_index:
        ...
        with args.corpus.open("r", encoding="utf-8") as source:
            ...
            if sampler.last_resources:
                index_cpu_ns = (
                    sampler.last_resources["user_cpu_ns"]
                    + sampler.last_resources["system_cpu_ns"]
                )
        ...
        if args.refresh_reused_index_layout:
            ...
            if raw_layout == "UNSUPPORTED":
                raise RuntimeError(
                    "Antfly adapter does not support reused-index layout refresh"
                )
            ...
            if len(live_layout["segments"]) != 1:
                raise RuntimeError(
                    f"refreshed single index has {len(live_layout['segments'])} segments"
                )
            previous_elapsed = refreshed.get("index_elapsed_ns")
            if isinstance(previous_elapsed, int):
                refreshed["index_elapsed_ns"] = (
                    previous_elapsed + args.reuse_index_extra_elapsed_ns
                )
    ...
    if not index_manifest.exists():
        raise RuntimeError(
            "index manifest is required (reuse-index must point at an archived build)"
        )
    ...
    if args.comparator_index_command:
        ...
        with args.corpus.open("r", encoding="utf-8") as source:
            ...
            if proc.returncode:
                raise RuntimeError(
                    f"comparator indexer exited {proc.returncode}: {stderr}"
                )
            (args.output / "comparator-index-stderr.log").write_text(
                stderr, encoding="utf-8"
            )
            comparator_index_peak_rss = sampler.peak or None
            if sampler.last_resources:
                comparator_index_cpu_ns = (
                    sampler.last_resources["user_cpu_ns"]
                    + sampler.last_resources["system_cpu_ns"]
                )
        ...
        if not comparator_manifest.exists():
            raise RuntimeError(
                "comparator index command did not emit {comparator_manifest}"
            )
    ...
    comparator_index_data = (
        json.loads(comparator_manifest.read_text(encoding="utf-8"))
        if comparator_manifest.exists()
        else None
    )
    ...
    if comparator_index_data:
        if comparator_index_elapsed is None:
            comparator_index_elapsed = positive_manifest_metric(
                comparator_index_data, "index_elapsed_ns"
            )
        if comparator_index_cpu_ns is None:
            comparator_index_cpu_ns = positive_manifest_metric(
                comparator_index_data, "process_cpu_ns"
            )
        if comparator_index_peak_rss is None:
            comparator_index_peak_rss = positive_manifest_metric(
                comparator_index_data, "process_peak_rss_bytes"
            )
    ...
    for query in queries:
        ...
        if comparator:
            ...
            if other_raw == "UNSUPPORTED":
                raise RuntimeError(
                    f"comparator rejected declared query: {query['query']}"
                )
    ...
    antfly_correctness_path.write_text(
        "".join(json.dumps(row, sort_keys=True) + "\n" for row in correctness_antfly),
        encoding="utf-8",
    )
    if comparator:
        ...
        comparator_path.write_text(
            "".join(
                json.dumps(row, sort_keys=True) + "\n" for row in correctness_comparator
            ),
            encoding="utf-8",
        )
        verify_argv = [
            sys.executable,
            str(ROOT / "tools/verify_search_benchmark.py"),
            str(antfly_correctness_path),
            str(comparator_path),
            "--abs-score-tol",
            str(args.abs_score_tol),
            "--rel-score-tol",
            str(args.rel_score_tol),
            "--diagnostics",
            str(args.output / "correctness.json"),
        ]
    ...
    if not args.skip_diagnostics:
        ...
        for query in queries:
            ...
            if raw == "UNSUPPORTED":
                raise RuntimeError(
                    f"Antfly rejected diagnostic query: {query['query']}"
                )
            record = json.loads(raw)
            record["query"] = query["query"]
            record["class"] = query["class"]
            diagnostic_rows.append(record)
        (args.output / "diagnostics-antfly.jsonl").write_text(
    ...
    warm_count = 0
    while (
        warm_count < args.warmup_queries
        or time.perf_counter() - warm_started < args.warmup_seconds
    ):
    ...
    def measure(engine: str, target: Adapter) -> dict[str, list[int]]:
        ...
        for repetition, order in enumerate(orders):
            for index in order:
                query = queries[index]
                response, elapsed = target.ask(
                    f"TOP_{query['top_k']}\t{query['query']}"
                )
                if response != "1":
                    raise RuntimeError(
                        f"{engine} invalid timed acknowledgement: {response!r}"
                    )
                query_class = query["class"]
                samples.setdefault(query_class, []).append(elapsed)
                rows_by_class.setdefault(query_class, []).append(
                    {
                        "schema_version": 1,
                        "engine": engine,
        ...
        for query_class, rows in rows_by_class.items():
            ...
            (args.output / f"{prefix}-{query_class}.jsonl").write_text(
                "".join(json.dumps(row, sort_keys=True) + "\n" for row in rows),
                encoding="utf-8",
            )
        return samples
    ...
    def resource_profile(engine: str, target: Adapter) -> dict[str, Any]:
        ...
        def snapshot() -> dict[str, int | None] | None:
            ...
            try:
                ...
                raise RuntimeError(
                    f"invalid Antfly RESOURCES response: {response!r}"
                ) from error

        rows: dict[str, Any] = {}
        for query_class in sorted({query["class"] for query in queries}):
            class_queries = [
                query for query in queries if query["class"] == query_class
            ]
            before = snapshot()
            started = time.perf_counter_ns()
            count = 0
            with PeakSampler(target.pid) as sampler:
                while (
                    count < 100
                    or time.perf_counter_ns() - started
                    < args.resource_profile_seconds * 1_000_000_000
                ):
                    query = class_queries[count % len(class_queries)]
                    response, _ = target.ask(f"TOP_{query['top_k']}\t{query['query']}")
                    if response != "1":
                        raise RuntimeError(
                            f"{engine} invalid resource-profile acknowledgement: {response!r}"
                        )
                    count += 1
            ...
            rows[query_class] = {
                ...
                or (after.get("peak_rss_bytes") if after else None),
            }
        return {
            "enabled": True,
            "minimum_wall_seconds_per_class": args.resource_profile_seconds,
            "classes": rows,
        }
    ...
    write_json(
        args.output / "indexing.json",
        {
            "antfly-zig": {
                "elapsed_ns": index_elapsed,
                "cpu_ns": index_cpu_ns,
                "peak_rss_bytes": index_peak_rss,
                "database_bytes": directory_bytes(args.index_dir),
                "text_index_bytes": index_data["layout"]["total_bytes"],
            },
            **(
                {
                    "comparator": {
                        "elapsed_ns": comparator_index_elapsed,
                        "cpu_ns": comparator_index_cpu_ns,
                        "peak_rss_bytes": comparator_index_peak_rss,
                        "index_bytes": directory_bytes(args.comparator_index_dir),
                    }
                }
                if args.comparator_index_dir and args.comparator_index_dir.exists()
                else {}
            ),
        },
    )
    write_json(
        ...
        {"schema_version": 1, "engines": resource_profiles},
    )
    write_json(
        args.output / "memory.json",
        {
            "schema_version": 2,
            "engines": {
                "antfly-zig": {
                    ...
                    "query_peak_rss_bytes": max(
                        (
                            row["rss_peak_bytes"] or 0
                            for row in resource_profiles["antfly-zig"][
                                "classes"
                            ].values()
                        ),
                        default=0,
                ...
                **(
                    {
                        "comparator": {
                            ...
                            "query_peak_rss_bytes": max(
                                (
                                    row["rss_peak_bytes"] or 0
                                    for row in resource_profiles["comparator"][
                                        "classes"
                                    ].values()
                                ),
                                default=0,
    ...
    summary = {
        name: summarize(samples) for name, samples in sorted(class_samples.items())
    }
    comparator_summary = (
        {
            name: summarize(samples)
            for name, samples in sorted(comparator_samples.items())
        }
        if comparator_samples
        else None
    )
    write_json(
        args.output / "summary.json",
        {
            ...
            "engines": {
                "antfly-zig": summary,
                **({"comparator": comparator_summary} if comparator_summary else {}),
            },
        },
    )
    manifest = {
        ...
        "merge_policy_override": {
            "max_segments_per_tier": args.merge_max_segments_per_tier,
        ...
        "measurement": {
            "warmup_queries": warm_count,
            "warmup_min_seconds": args.warmup_seconds,
            "repetitions": args.repetitions,
            "shuffle_seed": args.shuffle_seed,
            "resource_profile_seconds": args.resource_profile_seconds,
        },
        "reopen_ns": reopen_ns,
