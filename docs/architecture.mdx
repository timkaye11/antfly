...
Antfly follows the BASE model for distributed operations:
...
### QUIC for Raft Transport

Antfly's Raft transport (`zig/pkg/antfly/src/raft`) recognizes QUIC as a peer endpoint protocol, backed by a native QUIC/HTTP-3 implementation (`zig/lib/httpx`), for node-to-node communication:
