...
- `writer owner narrow publication scaling benchmark` / `dependency certificate delta scaling benchmark`: writer-owned narrow publication scales with input count, and delta certification of one new overlapping run against an existing GC certificate stays cheap relative to a full re-certification.
- `lsm incremental manifest publication benchmark` / `lsm persistent directory and lazy cursor scaling benchmark`: incremental manifest journaling stays far smaller than repeated full snapshots, and the persistent run directory plus lazy cold-scan cursor setup scale sublinearly with run count.
