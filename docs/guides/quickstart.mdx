...
```bash
antfly inference pull ggml-org/gemma-4-e2b-it-gguf:gguf:Q4_K_M
```

<Callout type="info">
`ggml-org/gemma-4-e2b-it-gguf` runs locally via Antfly inference and supports the full quickstart RAG flow — classification, generation, reasoning, and follow-up questions. For alternative providers, see the [next step](#rag-with-alternative-providers).
</Callout>
...
- **`ggml-org/gemma-4-e2b-it-gguf`** — local GGUF inference via Antfly inference, no API key needed, supports all RAG features
